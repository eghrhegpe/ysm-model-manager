...
- 审核流水线 / 反模式 / 致命陷阱 / 治理红线 / 防御范式 → 已拆分至三处：治理红线 = `skills/governance-rules.md`（10 条前端规则手册，R1–R10）、致命陷阱 = `skills/pitfalls.md`（22 条事故教训手册）、审核流水线 + 三份 Checklist（代码审查 / 跨平台 / 前端 3D）= `docs/adr/ADR-109-code-review-checklist.md`。原 `docs/audit-framework.md` 已于 2026-09 文档调整中删除（内容拆分归位）。
