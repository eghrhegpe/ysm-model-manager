...
$ysmTopCommands = @('analyze', 'analyze-mmd', 'avatar', 'benchmark', 'cache-clear', 'cache-diag', 'cache-status', 'cache-verify', 'concurrent-bench', 'config', 'config-show', 'copy', 'creator', 'dedup', 'download', 'export', 'file-bench', 'gui-flow', 'health-report', 'install', 'instance', 'link-mode', 'list', 'move', 'perf-log', 'perf-snapshot', 'recycle', 'rename', 'repo-audit', 'resource-scan', 'resource-types', 'scan', 'scan-bench', 'scan-dir', 'search', 'single-bench', 'tags', 'toggle', 'verify', 'workshop')
$ysmSubs = @{
...
$ysmFlags = @{
  ...
  'concurrent-bench' = @('--help', '--workers', '--max-models', '--format')
  'config' = @('--help')
  'config-show' = @('--help')
  'copy' = @('--help', '--src', '--dst')
  'creator' = @('--help')
  ...
  'scan' = @('--help')
  'scan-bench' = @('--help', '--iterations', '--format')
  'scan-dir' = @('--help', '--dir', '--detail', '--output')
  'search' = @('--help', '--keyword', '--min-bones', '--max-bones', '--min-cubes', '--max-cubes', '--min-tex', '--max-tex', '--format')
  'single-bench' = @('--help', '--model', '--iterations', '--rtype', '--all-types', '--max-models', '--top-largest', '--baseline', '--save-baseline', '--threshold', '--format')
  'tags' = @('--help')
  'toggle' = @('--help', '--path')
  'verify' = @('--help')
  'workshop' = @('--help')
