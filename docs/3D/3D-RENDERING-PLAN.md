...
| `frontend/js/utils/model3d.js` | Three.js 3D 渲染：场景/相机/灯光/网格 | ~464 |
| `frontend/js/utils/model3d-spec.js` | JS 兜底 Spec 构建（Go 不可用时） | ~255 |
| `frontend/js/utils/model2d.js` | Canvas 2D 骨骼图：正交投影、旋转、热区 | ~556 |
| `frontend/js/components/app-preview/preview-litematic-3d.js` | 3D 全屏预览覆盖层 | ~212 |
...
2. frontend/js/utils/model3d-spec.js 全文
3. frontend/js/utils/model2d.js 全文
...
3. frontend/js/utils/model3d.js
4. frontend/js/utils/model2d.js
...
| DeepSeek V4 Flash | JS 兜底 `model3d-spec.js` 同步 | `frontend/js/utils/model3d-spec.js` |
| Qwen3.7 Plus | 2D 骨骼图修正 | `frontend/js/utils/model2d.js` |
...
| `frontend/js/utils/model3d-spec.js` | `buildSpecFromModel()` :8 | JS 兜底，必须与 Go 一致 |
| `frontend/js/utils/model2d.js` | `renderModel2D()` :15 | 2D 正交投影 |
| `frontend/js/utils/model2d.js` | `rot()` :64 | Y 轴旋转函数 |
| `frontend/js/utils/model3d.js` | `renderModel3D()` :36 | Three.js 场景构建 |
