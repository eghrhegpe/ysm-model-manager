...
- **例外（已备案）**：`real-data-fuzz.test.ts`（独立数据轰击，不镜像模块）、`core/i18n/locales-consistency.test.ts`（locales/ 禁放 .test.ts，放上级 i18n/）、`preview-3d/model3d-spec.test.ts`（模块名含 `-spec` 属源码命名，非 Playwright spec）。
