...
--bg:       /* 最底层背景 */ --surf: /* 表面背景（侧栏、顶栏） */
  ...
  --accent: /* 强调色（链接、hover 高亮、关键按钮、焦点环、左侧选中指示条） */ --txt: /* 主文字色 = 正文级：列表主名/数值读数/错误正文/主操作按钮文字/导航项 */
  --muted: /* 次要文字色 = 仅真次要：提示/说明/元信息/时间/占位/空态/禁用与非激活态/装饰图标；正文级禁用（2026-09 层级扁平口径） */ --bd: /* 边框色 */;
```
...
| 变量        | 值               | 使用场景                       |
| ----------- | ---------------- | ------------------------------ |
| `--fs-tiny` | 8px（基准缩放）  | 热力图、极小水印、版权信息     |
| `--fs-xs`   | 11px（基准缩放） | 通用最小字号                   |
| `--fs-sm`   | 12px（基准缩放） | 通用小字号                     |
| `--fs-base` | 13px（基准缩放） | 正文、卡片标题、列表项主要文字 |
| `--fs-md`   | 14px（基准缩放） | 强调正文、区段小标题           |
| `--fs-lg`   | 15px（基准缩放） | 分组标题                       |
| `--fs-xl`   | 25px（基准缩放） | 数据统计数字、大号展示         |
...
| `--fs-btn-secondary` | 12px（基准缩放） | `.sm-item-btn` 次要/行内按钮    |
| `--fs-btn-tool`      | 12px（基准缩放） | `.repo-bar-btn` `.btn-base.sm` 工具栏按钮 |
...
语义字号（`--fs-nav` 等）**派生自 `--fs-base-size`（真基准）** + `--fs-scale` 偏移；间距随 `--fs-scale` 缩放。改基准全盘跟随，改偏移只做用户级微调。
...
- **`--fs-scale` 支持 `rem`**：当前 `--fs-base-size: 13px` + `--fs-scale: 0px` 使用 `calc(13px + var(--fs-scale))`，所有基础单位是 `px`，不响应浏览器默认字号设置（如用户设为 120%）。未来可改为 `--fs-base-size: 0.8125rem`（等价 13px@16px） + `--fs-scale: 0rem`，使整体字号尊重浏览器基础设置。
...
| ---- | ------- | --------------- | -------------------------- |
| 0    | 0       | —               | 无间距                     |
| 1    | 4px     | `--sp-1`        | 图标与文字之间、按钮内边距 |
| 2    | 6–8px   | `--sp-2`（8px@基准） | 列表项间距、小元素间距     |
| 3    | 10–12px | `--sp-3`（12px@基准） | 卡片内边距、段落间距       |
| 4    | 14–16px | `--sp-4`（16px@基准） | 区块间距、大按钮边距       |
| 5    | 20–24px | `--sp-5`（24px@基准） | 页面主间距                 |

**规则**：不要使用 3px、7px、9px 等非标准值。要么 4 的倍数，要么用上述层级。
层级 2–5 的像素是**区间**、`--sp-*` 取区间上沿单值（`--sp-*` = 内容/列表/卡片间距，随 `--fs-scale` 缩放）；按钮/标签**垂直内边距**另走 `--pad-*` 档（语义不同轨，勿混）。定义见 `variables.css`「间距五档令牌」。
...
- **例外（已备案）**：`real-data-fuzz.test.ts`（独立数据轰击，不镜像模块）、`core/i18n/locales-consistency.test.ts`（locales/ 禁放 .test.ts，放上级 i18n/）。
