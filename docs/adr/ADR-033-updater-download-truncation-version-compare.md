...
- **相关**：`go/updater/update.go, frontend/src/features/version-updater.ts`
...
1. **Download 三层防护**：Content-Length 预检超限直接拒绝（省流量、防磁盘写满）+ `LimitReader` 限流 + **读满后再读 1 字节**的截断检测（无 Content-Length 的分块传输兜底）——任一命中即删文件报错；
...
- **决策落地**：commit `071d71a`（`fix(updater): 下载截断检测与版本比较加固`）+ `a74b066`（`fix(updater): 检查更新频次时机与重启职责说明`）；
