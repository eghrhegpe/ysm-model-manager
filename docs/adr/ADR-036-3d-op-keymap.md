...
- **相关**：`frontend/js/utils/model3d.ts`、`frontend/js/views/app-preview/preview-skeleton.ts`、`frontend/js/views/app-content/tpl.ts`、`frontend/js/views/app-content/community/settings.ts`
...
- 来源：`docs/knowledge/model3d.md`（3D 渲染层 spec 与 `renderModel3D` 句柄 API）、`frontend/js/utils/model3d.ts`（键位处理段第 317–389 行原 `e.key.toLowerCase()` 硬编码）、`frontend/js/views/app-preview/preview-skeleton.ts`（顶栏速度/旋转控件未持久化）、`frontend/js/views/app-content/community/settings.ts`（UI 偏好统一走 localStorage）。
