...
- **相关**：`frontend/src/utils/model3d.ts`、`frontend/src/views/app-preview/preview-skeleton.ts`、`frontend/src/views/app-content/tpl.ts`、`frontend/src/views/app-content/community/settings.ts`
...
- 来源：`docs/knowledge/model3d.md`（3D 渲染层 spec 与 `renderModel3D` 句柄 API）、`frontend/src/utils/model3d.ts`（键位处理段第 317–389 行原 `e.key.toLowerCase()` 硬编码）、`frontend/src/views/app-preview/preview-skeleton.ts`（顶栏速度/旋转控件未持久化）、`frontend/src/views/app-content/community/settings.ts`（UI 偏好统一走 localStorage）。
