...
| **scale** | `scale==0 → 不可见`（三轴全零才隐藏），普通 scale 组合 | spec LocalRotation/Scale | **前端动画管线支持 scale 通道**：`BoneChannels.scale` → `evaluateClip` 逐通道局部求值 → THREE 场景图层天然父子复合 → `ysm-animation-player.ts` 应用；`scale=0 → node.visible=false` 对齐上游 calculateBoneMatrix:213-215（注：`evaluateClip` 不做显式父子累积，父子复合由渲染场景图承担，51a7c5e1 重构后口径） |
...
| **发光骨骼** | `bone.glow` → 全亮 `LightTexture.pack(15,15)` | spec glow 通道 | **glow 通道落地**：Go `spec-bones.go isGlowBone` 检测 `ysmGlow` 前缀 + `BoneData.Glow`；前端 `SpecBone3D.glow` → `mesh-builder.ts` 用 `MeshStandardMaterial + emissive/emissiveIntensity/emissiveMap` 模拟全亮渲染（回归测试 `tests/verify-adr-042.ts`） |
