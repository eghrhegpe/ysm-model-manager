...
- **P5（2026-08-29 追加）→ 撤销 P4-B-3 对 `fill3DPanel` 的「保持逃生舱」定性**：人类首席架构师指出，fill3DPanel 的组件切换是「渲染通道不受控」的架构问题（新增面板可绕过数组系统直接拼 DOM），非「交互态不值得转」——这与 P4-B-3 的 morph/play/material 交互面板**不同性质**。已落地：`schema-registry.ts` 受控注册（registerSchema，builder 吃快照）+ `PreviewMenuNode.schemaId` 声明 + renderMenu `select` 分支（交互控件 bind 状态层）+ `ui.activeComponent` 响应式状态（preview-state 会话态）+ `buildYsmModelSchema` 取代 fill3DPanel 的裸函数拼 DOM。**逃生舱收编为「schemaBuilders → registry → children → renderCustom → action → fillers」衰退链的末段**，新增面板必须先注册。
- **P5-收尾（2026-08-29 二轮审计）→ morph/play 交互面板也声明式化**：人类首席架构师二轮审计指出「morph/play/material 保持逃生舱」定性已过时（perception/material 已迁）。已落地：`morphNodes`（mmd 表情 toggle，照 perceptionNodes 样板）+ `playNodes`（播放/暂停 toggle + 动作 select + 空态引导，三 adapter mmd/vrm/ysm 共用）+ renderMenu `select` 分支支持闭包 get/set（非状态层路径）。**交互面板的「运行时状态」由 toggle/select 的 get/set 闭包 + rmAppendToggle/rmAppendSelect 的即时 apply 解决**——不需 Capability 类。**最终逃生舱只剩两类真·复杂**：① 骨骼面板（makeBonePanelRenderer，3D 射线拾取 + 相机/场景实时对象）；② litematic 分层切片（直接持有 DOM 元素引用）。
