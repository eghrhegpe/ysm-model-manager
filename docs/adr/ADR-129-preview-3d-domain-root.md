...
本文是 3D 预览领域根升格的**总纲 ADR**，后续子刀已分别立项并归位：

| 子刀 | 内容 | 状态 |
|------|------|------|
| 第一刀 | 正类型（依赖倒置修复） | ✅ 已纳入本文 §2 |
| 第二刀 | 领域根提升 + 相对引用修复 | ✅ 已纳入本文 §2 |
| 第三刀 | 整目录搬迁 `utils/3d` → `features/preview-3d` | ✅ 已纳入本文 §2 |
| 第四刀 | 截图/离屏渲染领域归位 | ❌ [ADR-136](./ADR-136-screenshot-domain-homecoming.md) 已取代 |
| 第五刀 | YSM 解码子系统归位 | ❌ [ADR-137](./ADR-137-ysm-decoder-homecoming.md) 已取代 |
| 收尾 | 去 features 中间层，上提 `src/preview-3d` | ❌ [ADR-138](./ADR-138-preview-3d-to-src.md) 已取代 |

> 历史引用仍可指向各子刀文件，但决策以本文为准。
