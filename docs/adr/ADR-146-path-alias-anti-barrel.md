...
`check-path-hygiene`、`auto-import*` 家族、`check-layering`、`check-circular`、`check-tpl-refs` 等按路径文本解析的脚本（**含门禁脚本自身**），必须先支持别名解析，**然后才开闸启用别名**。顺序颠倒会让门禁集体失明——别名一上，所有基于相对路径的环检测 / 分层检测都静默失效。
...
- **同顶层内、不同子目录**（`features/a.ts`→`features/b/x.ts`）→ 也 `@/顶层/子目录/...`（2026-09-07 再收口 442 处）；绕回原目录的畸形相对（`../../top/sub/file`）→ 归正 `./file`（8 处）；
- **仅精确同目录**（`./` 兄弟文件，约 1.3k 处）→ 保留相对 `./`；
- **src 根上的叶子文件**（`bus.ts` / `theme-core.ts`）→ 文件级别名 `@/bus` / `@/theme-core`；
- **越 `src` 界**（`bindings/**`、仓库根 JSON）→ 维持相对 / `#root`，不并入 `@/`（R4 域，只减不增）；
- **新增文件** → 一行一律 `@/`（非精确同目录）；`./` 仅限精确同目录兄弟。
- **例外**：迁移时 3 个并行会话 dirty 文件（未提交）暂留相对，待其入库后顺手切。

迁移后 `check-path-hygiene` 的 R5 语义即：**任何非 `./` 精确同目录的相对深度（`../`、`../../`）都应写成 `@/` 或 `./`**——相对深度思考已全仓归零。
