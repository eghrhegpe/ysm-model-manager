...
2. **adapter renderCustom 逐面板退役**（修正后实际范围 = 3 个面板，前两臂已声明式）：camera / environment 照 lighting 的 cap 自报模式迁入 schema-registry（低风险机械活）；**bones 是唯一真决策点**，开刀前必须拍板其二选一：① 给 node 模型加声明式 `tree` kind（骨骼树数据化，raycaster/场景引用经状态层注入）——彻底但最重；② 明示 bones 为唯一永久例外，menu-graph 判定式为其单列 `sanctionedProcedural` 口径而非永远卡死 full——务实但留一个洞。拒绝默认「挂着不动」（那是烂尾路径）。**拍板结果（2026-09，人类首席架构师选②）**：bones 定为唯一永久例外；例外名单收敛为单一事实源 `menu/sanctioned.ts|SANCTIONED_PROCEDURAL_PANELS`，由审计门（`render-custom-audit`，构造点须与名单逐一相等）与 `collectMenuGraph().sanctionedProcedural`（报告显式列出）共同消费——兑现本节 §3「不可静默」，杜绝「报告宣称 full 却藏着豁免面板」。**选②后的收尾：该例外的假释条件（`exitWhen`）与「何时升级为选项①抽象提取」的触发条件见 [ADR-276](ADR-276-preview-menu-subscribe-tree.md)（豁免条目须自证 decidedBy + rationale + exitWhen 三件，审计门守护）。**
