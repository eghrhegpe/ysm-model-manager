...
- **关联修正（ADR-257 §6.4）**：本文档 §2.2「尺寸 uniform 驱动、不重建几何」在落地时漏了两处尺度换算——
  水平位移未 `/uSize`（世界量直接加进局部坐标）、解析法线未 `×uSize`（世界偏导直接当物体空间法线）。
  水面 mesh 是 `scale(uSize,uSize,1)` 的各向异性缩放，两处漏换算互为镜像。已于 2026-09-18 连同 §2.1
