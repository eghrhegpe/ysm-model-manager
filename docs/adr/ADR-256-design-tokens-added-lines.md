# ADR-256：设计令牌门禁改判真行级：判定与账本分离（行位移幻影 95% 实证）
...
- **幻影（误伤）**：任何行位移都会把下方存量违规换成新行号 → 判「新增」。
  baseline 报的 346 条「新增」中 **327 条（95%）**在父版本 ±80 行内能找到同 kind 近邻；
  阻断视角：24 次阻断里 **19 次（79%）真新增为 0**。典型全幻影提交：`88ddb1ec`（69 条）、
  `6cec6a18`（57 条）、`a94d3884`（52 条）——多为批量迁移 / 重排 / 图标替换类改动。
- **盲区（漏检）**：新增违规恰好落在旧违规的同行号上 → 键相同 → 判「非新增」。
  同期实测 **36 条真新增被漏**（同行替换同类违规，如 `color:#abc` → `color:#fff`）。
...
结论：这不是「严格 vs 宽松」的取舍，而是**双向失效 + 高噪声**；既拦的提交里 baseline 要求的
条数是真新增的 median 2.3×（mean 3.8×），且 pre-commit 必须扫全树（frontend/src + frontend/css）。
...
| 来源 | 结果 |
|------|------|
| `scripts/token-shift-audit.ts --window 120`（只读，可复查） | 样本 116 提交；added 346；位移幻影 327（95%）；真新增候选 19 |
| 同上（阻断视角） | baseline 会阻 24 次（21%），其中纯幻影 19 次（79%）；行级会阻 9 次（8%） |
| 同上（代价侧） | 两规则都拦时：baseline 条数 / 行级条数 = median 2.3× / mean 3.8× |
| 同上（盲区侧） | 行级命中而键设计漏判：36 条 |
| 现场 | 不含前端文件的 docs 提交被并行会话位移造成的 21 条幻影 exit 1（`tpl-settings.ts`） |
