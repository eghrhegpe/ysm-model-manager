...
- **`waterSize` 在 UI 无入口**：全仓唯一写入点是 `loadState` 的 `size` 恢复（+ 测试直写）。
  ...
  → **已收口（ADR-272，2026-09-19）**：`ground-water-size` 滑块落地（form 组，范围 10–300 m）。
- **放开 size 入口前须先解决法线重算**：film 改 size 会同步触发 `getNormalMap()` 全量重算
  ...
  → **已收口（ADR-271，2026-09-19）**：走了第三条方向——CPU 贴图链路整体移除，微细节法线改
  fragment 程序化（`uDetailStrength`）。本条前置条件自此解除，**仅剩上方「size 无 UI 入口」待做**。
  → 上一条亦已收口（ADR-272）。
- **同族遗留（ADR-272 登记）**：pool 的 `waterPoolHeight` / `waterPoolWallThickness` 变更仍走全量
  `rebuildWaterContainer`，而这两条**早已有 UI 滑块**（`ground-pool-height` / `ground-pool-wall-thickness`）
  ——即拖动时的重建风暴**今天就存在**，只是从未被登记。修法与 ADR-272 同构（墙高改 `scale.y`、
  壁厚改外壁 offset 数据），未并入该次改动。
- **`waterLevel` × 容器参数的组合自由度**：解耦后仍可把水面抬到墙顶之上 / 压到池底之下，
  维持 §3「刻意保留自由度，如需约束应走 preset」的既有结论。

### 6.4 波浪位移/法线的尺度换算缺一（**既有缺陷**，数值实证修正）

- **事实**：水面 mesh 是单位平面 × `scale(uSize, uSize, 1)` 的**各向异性缩放**，而波场定义在世界水平
  尺度上（`p = position.xy * uSize` 即世界水平坐标）。原实现把「世界量」的**水平位移**直接加进局部坐标
  （缺 `/uSize`）；本次新增的解析法线沿用了同一套「按世界尺度算、按局部坐标用」的写法，于是**物体空间
  法线**也缺 `×uSize`。两者互为镜像，必须成对修——只修一边等于换一种错法。
- **实证方法（可复现）**：node 脚本复刻 shader 数学（`waterSize=80`、`choppiness=0.5`、128 分段局部网格、
  121 个采样点，用相邻顶点有限差分求几何法线），与解析法线计算夹角：

  | 尺度约定 | 几何法线 vs 解析世界法线（平均 / 最大） |
  |---|---|
  | 原实现（位移未换算） | **94.05° / 179.28°** —— 法线大面积反向，即三角形已翻面/自交 |
  | 修正后（位移 `/uSize`、法线 `×uSize`） | **2.40° / 7.02°** —— 仅剩一阶近似残差 |

- **为何长期未被目视发现**：film 默认 `waterOpacity 0.25 × waterWetness 0.5 = 0.125` 的极淡水膜叠在地面上，
  几何自交产生的黑面/闪烁几乎不可辨；一旦用户提高 opacity 才会暴露。
- **修正**：`gerstner()` 水平位移项 `/uSize`、法线水平项 `×uSize`（高度轴 `scale.z=1`，两侧均不换算），
  并以结构断言「两处 uSize 因子必须成对出现」防止将来单边回退（`water-capability.test.ts`）。
- **顺带记录（未改，独立议题）**：`amp = min(0.6·0.82^i / freq, 0.5)` 的钳制使 wave0–wave4 的振幅全部顶到
  上限 0.5，ADR-255 §2.1 设计的「amp 几何级数衰减」实际被抹平（仅 wave5 为 0.373）——波谱与设计意图不符；
  改钳制会动波形观感，故仅登记。

### 6.5 数字勘误（2026-09-19，ADR-272 顺带实证）

- 本文档 §1「抬高 0.1 的水位要付『重建 **9 个** mesh』」与 §3「抬升水面从『重建 **9** mesh』降为……」，
  以及水面知识卡的「顶面 + 池底 + 4 组内外壁共 9 mesh」——**数字皆错，实为 10**：
  顶面 1 + 池底 1 + 4 组 ×（内壁 + 外壁）8 = 10。
- 实证：`water-capability.test.ts` 以 `collectMeshes(root)` 计数断言（ADR-272 新增用例），
  实测长度 10，与本文档不符即红。
- 性质：纯计数漂移，不改变本文档任何决策与结论（那个「9」从未参与推理，只是被反复抄写）。
  正文数字保留原样不改（历史记录），以本节为准；活文档（知识卡、代码注释）已同步改为 10。
