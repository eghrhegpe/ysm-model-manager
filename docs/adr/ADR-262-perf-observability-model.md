...
**目标集参数面正交化：`--target` × `--order` × `--max-models`（2026-09-18 修订，取代上文 4 个互斥 flag）**。上面那套面把三个**正交维度**压进一个 flag 集合，于是长出了它自己的债：
- **「选谁」**（selector）与 **「排序」**（order）与 **「取几条」**（limit）本是三件事，`--top-largest N` 却把 order 与 limit 焊死 → 它与 `--max-models` 语义重叠，只能靠互斥打补丁（旧守卫自证：「两者都表示取几条」）；
- `--all-types` 只是 selector 的一个取值，却单独立 flag；selector 四选一 + cap 共 5 个参数说一件事，守卫 8 条；
- **代价最终由用户界面支付**：一个数字控件承载两种含义（「每类上限」/「前 N 大」），标签只能跟着模式改义——`syncPerfCountLabel` 就是这张账单，它修的是症状。

修订后的形态：`--target model|rtype|all|repo`（selector）× `--order path|size`（排序）× `--max-models`（上限），**跨命令同名同义**，互斥矩阵消失（只剩「各 selector 只接受自己那个 payload 参数」的配对校验）。统一规则一条：**`--max-models` 的单位 = `--target` 的展开单位**——`model`=单条（显式传入上限即报错，不静默吞参）、`rtype`=该类型 N 条、`all`=每类型各 N 条、`repo`=全库扁平 N 条。`repo` 与 `all` 必须分开：前者是**全库扁平**（不按类型分组，前 N 大即此形态），后者是**矩阵口径**（每类型各取 N，用于类型间可比），二者共用 `all` 会互相踩。等价映射（逐字）：`--top-largest N` ≡ `--target repo --order size --max-models N`；`--all-types --max-models N` ≡ `--target all --order path --max-models N`；`--rtype X` ≡ `--target rtype --rtype X`。旧面表达不出的新组合随正交性免费获得：`--target all --order size --max-models N`＝「每类最重的 N 个」、`--target rtype --order size`＝「某类型按体量排序」。

**候选池枚举同时收成单点**：旧面有三份并行实现（`scanBenchTargets` / `scanTargetsGrouped` / `scanTopLargestTargets`，各自 `ScanEntries` + `classifyForScan` 各扫一遍仓库），三旋钮若照旧各写一份就会变成第四、第五份。现为**一个枚举 + 一个排序 + 薄 selector**：`collectPerfTargets`（唯一枚举，返回全库候选池 + 逐类型**未截断**总数）/ `orderPerfTargets`（唯一排序；`size` 时把体量**固化进 `Footprint`**，使排序键与回显值同源，避免二次统计自相矛盾）/ `groupPerfTargets`（矩阵口径按类型切分）/ `capFlat`（扁平上限），三个旧函数退化为这些原语的具名适配器（行为逐字不变，既有测试即回归证明）。**过滤权归命令，不归 selector**：`order=size` 只产出有序候选池，可分析性由命令能力决定——`single-bench` 的 repo 目标集含不可分析类型并标 `unsupported`（全库池），而 `concurrent-bench` 必须过滤到 CLI 可分析（它调 `AnalyzeBedrockModel`，空模型=垃圾数据）。同一个 selector 在不同命令上过滤不同，正说明「选与排序」和「过滤与上限」是两层职责。

**`rust_backend` Go/Rust 对照的落地形态**（2026-09-18，D3 最后一项）：Rust（`scan_fast`）是 win-amd64 / macOS / Linux **生产构建的主扫描路径**，Go 是兜底，故对照必须「同机、同 fixture、同一条生产管线只换引擎」。两处决策：① **归属靠记账而非推断**——`tryRustScan` 的 `handled=true` 分支递增 `rustHandledCount`（与既有 `walkCount` 配对），`scanner.ScanEngineStats()` 前后差即「这一次是谁处理的」；这样「Rust 在跑」与「运行期不可用而静默回退 Go」被区分开，否则报告会把 Go 的耗时记在 Rust 头上。② **对照要有可控的 Go 端**——`SetForceGoEngine(true)` 让 Rust 快路径短路，另一端走同一条生产 Go walk；它是**基准专用**开关（生产恒 false，且用完即复位），不是新的生产分支。**不引入 `--engine` 之类的生产开关**：生产该走哪端由构建与运行期可用性决定，给用户一个「强制 Go」的生产入口只会制造两台机器行为不一致。消费方是新命令 `scan-bench`：逐次先 `InvalidateCache()`（缓存命中那一次根本没走引擎，算进样本就是把「省下的时间」当引擎速度，故如实记 `skipped`），并逐条比对条目集合与 `Size/Hash/Ext/Name`（`parity`）。**未采集就说未采集**：Rust 未参与时 `used=false` + 原因 token（`unavailable` 构建期无 Rust / `fell_back` 运行期回退 / `cache_hit` / `interfered`），绝不填 0.00ms。
