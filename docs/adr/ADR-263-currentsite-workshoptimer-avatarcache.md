...
| 字段 | 谁写 | 谁读 | 语义归属 |
|------|------|------|----------|
| `currentSite` | `site/workshop-tabs.ts:84` | `site/workshop-site-opener.ts:104,114`、`init-workshop.ts`（注入链读写） | **工坊页私有** |
| `workshopTimer` | `site/workshop-tabs.ts:106` | `index.ts:120`（`_render` 开头清）、`state.cleanupTransient()` | **app 壳层跨切** |
| `avatarCache` | `site/workshop-avatar.ts:22`（整表替换）、`init-workshop.ts:167`（增量） | `init-workshop.ts` ctx 注入 → `site/*` 渲染链 | **跨页**（写入方是模块级下载队列） |

三个模块（tabs / opener / avatar）拿到的都是**整个 `AppContentState`**。接口宽度即权限：tabs 只需要读一个站点，却持有了 `root` / `pagePanels` / `resizeMove` / `workshopTimer` 的写权——而它**确实**顺手写下了 `workshopTimer`（上表第二行即是）。这不是某次手滑，是接口给错了。收窄到只含页语义的句柄后，越界写入直接编译不过。

### 1.2 `avatarCache` 的根因：写入方比页面长寿

`init-workshop.ts:163` 用 `addGlobalOnce("workshop:avatar-refresh", …)` 注册，而 `features/community/download-queue-store.ts:293-295` 的 `Events.On("queue:file-done", …)` 是**模块加载时一次性注册**（`_registered` 守卫，知识卡 `community-feature.md` 明示「页面切换不丢事件」）。下载队列由 Go 侧驱动，**离页照跑**，`avatar:refresh` 随时可能在非工坊页派发。
...
`index.ts:119-123` 在 `_render()` **开头**清定时器，`:142` 才 `page.init(this)`。这个次序不是随手写的：`index.ts:64-65` 那次初始 `nav:changed` 可能早于 `app-content` 加载（注释原文），面板常驻下切页也不重跑 init——只有「壳层在每次进入渲染时统一清」才覆盖全部进入路径。
...
2. **`initWorkshopTabs(host, refs, page)` / `bindSiteEvents(host, page)` 收窄签名**：tabs 与 opener 不再接收整个 `AppContentState` 可达的那个 `host` 语义面——它们只经 `page` 读写站点。`init-workshop.ts` 是唯一创建者，负责把**同一实例**下传。
...
- ③ **维持现状，只补注释** —— 注释拦不住越界：tabs 写下 `workshopTimer` 的那条路依旧敞着（同一个 `host.state` 可达）。收窄接口才是机制。
...
- `avatarCache` 仍借宿 `AppContentState`。**正解是上收为 community 层模块级 store**（与 `download-queue-store` 同寿命、同形态），届时工坊页只做消费者；本刀只标注、不改动。
- `workshopTimer` 仍借宿。本 ADR 的立场是**它就该留在壳层**；若未来订阅桶新增「切页」粒度，可重新评估。

## 4. 数据溯源

- ADR-261 §3「已知遗留」原文 → 本刀立项依据（§1 开篇）。
- `download-queue-store.ts:293-295`「一次性注册全部后端事件 / Wails 脚本加载时执行一次，页面切换不受影响」+ `:319-354`（`.ysm` 成功 + `[作者]` 前缀 → `bus.emit("avatar:refresh")`）→ §1.2 结论「写入方比页面长寿」。
- `init-workshop.ts:163` 注册走 `addGlobalOnce`（global 桶）；`subscription-bucket.ts` 文件头「globalUnsubs 连入注册、卸载清除，不随切页清空」+ `cleanupPage()` 注释「**仅** lang:changed 全量重建 / 卸载调用；切页不调」→ §1.2 场景链（切页后订阅仍在、缓存若清则增量丢失）。
- `index.ts:119-123`（`_render` 开头清）早于 `:142`（`page.init`）；`:64-65` 注释「app-content 经 app-modules.ts 动态加载，可能晚于 app-nav 派发的初始 nav:changed」→ §1.3「清理必须早于 init」结论。
