...
- **状态**：✅ 已采纳（Accepted）
- **衔接**：§2.5「只读扫描进即有引导空态 + 主按钮」的**落点**已由 [ADR-288](./ADR-288-diagnostics-scan-bar-persistent.md) 修正（启动按钮迁到常驻参数栏，空态留在结果区）；本节「进 tab 不跑进程」的**判据**不受影响，仍有效。
...
### 2.7 「共用」= 模式之外常驻，不是「两个模式都显示」；引擎对照退出模式轴（2026-09-20 补，**部分取代 §2.2 的实现形态与 §3 的「升格为一等模式」**）

用户锐评：三种模式看起来臃肿，且「并发与单模型均可选范围，反而应该是范围放第一个」。核实 CLI 后确认**前端做了错误的分类**：

- `perf_target_set.go:255` 原话：single-bench 与 concurrent-bench「两命令**共享同一套参数面（同名同义）**，差异只在默认值」——两者由**同一个** `registerPerfTargetFlags` 注册 `--target/--order/--rtype/--model/--max-models`。
- `scan-bench` 则**只有 `--iterations` + `--format`**（`scan_bench.go:338` 的 FlagSet 就这两个），根本没有目标集参数。它测的是「扫一遍仓库」（Go vs Rust 引擎对照），输入/阶段/可比对象与「解析一个模型」全部不同。

**决策**：

1. **公共区常驻**：`测什么 / 排序 / 最多模型数` 移出 `[data-perf-mode]`，置于控制条**最上**且模式无关。⚠️ §2.2 说「共用」但实现落在 `data-perf-mode="single conc"` 行内——那是「两个模式都显示」而非「模式之外常驻」，仍把公共参数埋进了模式块，正是臃肿与漂移的来源；本条把它改为真常驻，**是补齐 §2.2 而非推翻它**。
   ⚠️ **补注（ADR-285 收口）**：「最多模型数」**不在**公共区常驻——§2.3 明确它有 single/conc 两个真实口径（默认 5 / 20），故实装仍是两条按模式显隐（`tpl.ts` 里 `diag-perf-max` / `diag-perf-conc-max` 各挂自己的 data-perf-mode）；本 item 的「公共区」仅指「测什么 / 排序」。另：模式行位于公共区**之上**（先选命令再圈范围，再是参数），与「公共区置于最上」的字面表述以本节为准。
2. **引擎对照退出模式下拉，改为平级 tab**：模式轴只剩 single / conc——它们确实是「同一件事的两种跑法」（解析模型：串行 vs 并行）；scan 是**另一件事**，不是「第三种范围」。⚠️ 这**部分取代 §3「升格为一等模式」**：当时升格解决了「藏在别人工具条里」的发现问题，但把它混进模式轴又制造了「同类内不同参数面」的新误解；升格为 **tab** 同时解决两者。
3. **运行按钮仍并排**：CLI 是三条命令，命令名必须可见（合并为单一按钮会把「我在跑哪条」隐藏掉）。

**代价与边界**：§2.6 的「同控件跨模式改义」诚实层**减弱**——`#diag-perf-iter` 不再跨 single/scan 改义（scan 独立成 tab 后有自己的标签）；但 `PERF_UNREAD_MODES` 等门禁表需同步收缩 scan 项，`perf-mode.test.ts` 的穷尽护栏扫描域需从 bench body 扩到含新 scan tab 的 body，否则**护栏会从「覆盖少一格」变成「扫错文件」**（同本卡反复出现的「闸只看得见它被写死的那一类」）。
