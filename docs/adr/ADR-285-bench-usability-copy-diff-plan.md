...
- **状态**：🔄 部分采纳（2026-09-20 首轮拍板：P0-1/2/4、P1-1(方案A)、P3-1/2 已采纳并落地；P2-1 仅经 P0-4 落三语 label 终值；P0-3 被否（反转 ADR-278 §2.6 属中高风险）；P1-2/1-3、P2-2/2-3 留待后续）
...
| P0-3 | zh「最多模型数」→「取样上限」 | 零风险 | `zh-CN.ts:487` + 2 处注释 | 低（e2e 只比较三次读数相等） | ❌ **否**（会反转 ADR-278 §2.6 已评审结论；单测 `perf-matrix.test.ts` 的 `toBe("最多模型数")` 与多处注释需连带，方案未列全；见 §2.1 补注） |
...
| P3-2 | 知识卡补记本轮审核结论 | 文档 | `docs/knowledge/app_content_diagnostics.md` | 极低 | ✅ 采纳（待落地） |
...
**⚠️ 2026-09-20 用户拍板：P0-3 否决，不回滚方向**（执行前核实发现本条的「零风险」是误判）：
- 这会**反转 ADR-278 §2.6 已评审的结论**——当时「取样上限」被判定为黑话、三语标签统一改「最多模型数」，且 `perf-matrix.test.ts:610,615` 的 `toBe("最多模型数")` 是**有意保留的锁定断言**（知识卡 `LyG`「文案漂移附带订正」刚清理完「注释冒充当前标签名」的失真）。
- 方案只列了「zh-CN.ts:487 + 2 处注释」，**漏了必连带**的 `perf-matrix.test.ts` 6 处（`:9/:50/:600/:601/:610/:615` 的 fixture label、断言、注释）与 `perf-single-bench.ts:193`、`perf.ts:115` 等注释——不连带即留「产品与夹具失真」，连带则改动面远大于「零风险」。
- 若日后确有「最多模型数在 all/repo 口径下名不副实」的真实痛点，应**另立 ADR 走正常评审**并连带全仓字面量，而非在本 ADR 当零风险微调。
