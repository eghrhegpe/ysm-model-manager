...
> 架构决策日志，共 **308** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
| [ADR-307](./ADR-307-settings-page-p2-hardening.md) | 设置页 P2 硬化：tab 样式去重 / viewer 区块缺席告知 / 非 3D 设置项 schema 化 | ✅ 已采纳 |
...
| [ADR-303](./ADR-303-preview-persist-spec-single-source.md) | 3D 预览持久化偏好规格单一源 | ✅ 已采纳 |
| [ADR-302](./ADR-302-menu-node-kind-field-single-source.md) | 菜单节点类型不做一次性判别联合——以运行期字段表为单一事实源（类型层派生） | ✅ 已采纳 |
...
| [ADR-296](./ADR-296-linkmode-switch-relink-hardening.md) | 链接模式切换重链链路加固（锐评落地） | ✅ 已采纳 |
| [ADR-295](./ADR-295-combo-spacing-tokens.md) | 组合间距档 --sp-vh-*：为高频「垂直+横向」组合值补标准化令牌（承接 ADR-294 D3 存量收敛） | ✅ 已采纳 |
...
| [ADR-288](./ADR-288-diagnostics-scan-bar-persistent.md) | 诊断页布局词汇统一与扫描类参数栏常驻化 | ✅ 已采纳 |
...
| [ADR-259](./ADR-259-tab-rendertabs.md) | tab 结构单点产出——renderTabs 工厂与结构契约 | ✅ 已采纳 |
...
| [ADR-242](./ADR-242-preview-group-card-nav.md) | 3D 动作/模型组一级卡壳收纳（面板入口行 array，内容跳转后渲染） | ✅ 已采纳 |
| [ADR-241](./ADR-241-preview-dock-route-decl.md) | 3D dock 一级路由声明化（组定义 direct 判定收敛 core.ts 隐式分支） | ✅ 已采纳 |
| [ADR-240](./ADR-240-preview-menu-panel-fold.md) | 3D 菜单内容型 panel 统一折叠卡渲染（kind 形态脱钩） | ✅ 已采纳 |
| [ADR-239](./ADR-239-toolbar-menu-declarative.md) | 工具栏下拉菜单声明式收敛（对齐 ADR-021 菜单范式） | ✅ 已采纳 |
...
| [ADR-230](./ADR-230-genguard.md) | GenGuard 统一收敛：内建状态源为唯一出口，外部状态源模式退役 | ✅ 已采纳 |
...
| [ADR-214](./ADR-214-debug-remove-dynamic-backend-import.md) | debugGetSpec 控制台钩子搬离 debug.ts——职责纯度回归 | ✅ 已采纳 |
...
| [ADR-202](./ADR-202-go-registry-go.md) | Go 测试执行结构收敛：registry 并发安全 + 锁协议注入 + 包内并行解锁（测试税减负 Go 版） | ✅ 已采纳 |
| [ADR-201](./ADR-201-test-naming-convention.md) | 测试文件命名规范 | ✅ 已采纳 |
| [ADR-200](./ADR-200-cli-structured-output.md) | CLI 输出契约结构化：命令返回结果对象，--json 载荷去文本化 | ✅ 已采纳 |
| [ADR-199](./ADR-199-cli-bridge-gui-fork.md) | cli-bridge 进程内直调：移除 GUI 自 fork | ✅ 已采纳 |
...
| [ADR-177](./ADR-177-lightcapability.md) | 拆分 LightCapability 为编排器 + 协作子模块 | ✅ 已采纳 |
| [ADR-176](./ADR-176-go-installlock.md) | Go 端维持进程单例与 InstallLock,收敛测试注入,修文档/门禁漂移与吞错 | ✅ 已采纳 |
...
| [ADR-164](./ADR-164-node-wasm-avat.md) | Node+WASM 解码桥单实现收敛（avatar × wasm_decoder 双胞胎） | ✅ 已采纳 |
| [ADR-163](./ADR-163-content-page-tab-panel-persistent.md) | 主内容页 tab-panel 常驻化：替代整 DOM 重建 | ✅ 已采纳 |
| [ADR-162](./ADR-162-knowledge-card-symbol-anchor-strip-linenum.md) | 知识卡符号锚点去行号（行号减噪） | ✅ 已采纳 |
...
| ADR-307 | 设置页 P2 硬化：tab 样式去重 / viewer 区块缺席告知 / 非 3D 设置项 schema 化 | ✅ 已采纳 | 2026-09-25 |
...
| ADR-303 | 3D 预览持久化偏好规格单一源 | ✅ 已采纳 | 2026-09-24 |
| ADR-302 | 菜单节点类型不做一次性判别联合——以运行期字段表为单一事实源（类型层派生） | ✅ 已采纳 | 2026-09-24 |
| ADR-301 | 创作者频道与创意工坊命名轴收敛 | 📝 提议中 | 2026-09-24 |
...
| ADR-296 | 链接模式切换重链链路加固（锐评落地） | ✅ 已采纳 | 2026-09-22 |
...
| ADR-291 | B 轨快照探针入册门槛（双轨状态镜像的治理边界） | ✅ 已采纳 | 2026-09-21 |
| ADR-290 | 体积光锥驱动源 schema 化（lightVolumetricDriver） | ✅ 已采纳 | 2026-09-20 |
...
| ADR-259 | tab 结构单点产出——renderTabs 工厂与结构契约 | ✅ 已采纳 | 2026-09-17 |
| ADR-258 | 诊断页导航：左栏分段收敛为顶部统一 tab 范式 | ✅ 已采纳 | 2026-09-17 |
...
| ADR-242 | 3D 动作/模型组一级卡壳收纳（面板入口行 array，内容跳转后渲染） | ✅ 已采纳 | 2026-09-15 |
| ADR-241 | 3D dock 一级路由声明化（组定义 direct 判定收敛 core.ts 隐式分支） | ✅ 已采纳 | 2026-09-15 |
| ADR-240 | 3D 菜单内容型 panel 统一折叠卡渲染（kind 形态脱钩） | ✅ 已采纳 | 2026-09-15 |
| ADR-239 | 工具栏下拉菜单声明式收敛（对齐 ADR-021 菜单范式） | ✅ 已采纳 | 2026-09-15 |
...
| ADR-230 | GenGuard 统一收敛：内建状态源为唯一出口，外部状态源模式退役 | ✅ 已采纳 | 2026-09-12 |
...
| ADR-214 | debugGetSpec 控制台钩子搬离 debug.ts——职责纯度回归 | ✅ 已采纳 | 2026-09-14 |
...
| ADR-203 | 平台门控归位 backend——断 utils/dom→backend 依赖环 | 🔄 部分采纳 | 2026-09-07 |
| ADR-202 | Go 测试执行结构收敛：registry 并发安全 + 锁协议注入 + 包内并行解锁（测试税减负 Go 版） | ✅ 已采纳 | 2026-09-07 |
| ADR-201 | 测试文件命名规范 | ✅ 已采纳 | 2026-09-07 |
| ADR-200 | CLI 输出契约结构化：命令返回结果对象，--json 载荷去文本化 | ✅ 已采纳 | 2026-09-06 |
| ADR-199 | cli-bridge 进程内直调：移除 GUI 自 fork | ✅ 已采纳 | 2026-09-06 |
...
| ADR-177 | 拆分 LightCapability 为编排器 + 协作子模块 | ✅ 已采纳 | 2026-09-04 |
| ADR-176 | Go 端维持进程单例与 InstallLock,收敛测试注入,修文档/门禁漂移与吞错 | ✅ 已采纳 | 2026-09-04 |
...
| ADR-164 | Node+WASM 解码桥单实现收敛（avatar × wasm_decoder 双胞胎） | ✅ 已采纳 | 2026-09-03 |
| ADR-163 | 主内容页 tab-panel 常驻化：替代整 DOM 重建 | ✅ 已采纳 | 2026-09-03 |
| ADR-162 | 知识卡符号锚点去行号（行号减噪） | ✅ 已采纳 | 2026-09-02 |
