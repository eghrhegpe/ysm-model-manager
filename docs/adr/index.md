...
> 架构决策日志，共 **139** 篇。决策真相源 = 各 ADR 文件首部「状态」行；本页为登记表 + 规范索引（单文件承载全部）。
...
*登记表由 `gen-docs-index.mjs` 自动重写；一致性校验已接入：`node scripts/check-adr-health.ts`（状态值域 + 登记同步 + 技术债）+ `node scripts/check-doc-drift.ts`（编号连续性/漏登/幽灵）+ `node scripts/gen-adr-supersede.ts`（取代关系扫描）。*
