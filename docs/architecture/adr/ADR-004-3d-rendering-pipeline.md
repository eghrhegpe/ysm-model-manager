...
- **相关**：`go/threejs/spec.go` / `go/geometry/archive.go` / `go/ysm/extracted.go` / `frontend/js/utils/model3d.js` / `frontend/js/utils/model2d.js` / `frontend/js/components/app-preview/preview-wasm.js`
...
| `frontend/js/components/app-preview/preview-wasm.js` | WASM 解码 + ysm.json 解析 + 纹理排序 |
