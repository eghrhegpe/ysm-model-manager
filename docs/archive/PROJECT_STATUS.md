...
| ADR | 主题 | 状态 |
|-----|------|------|
| ADR-014 | 前端 TypeScript 渐进迁移 | ✅ 已采纳 |
| ADR-013 | 治理体系收敛 — 文档宪法对账与联邦基线对齐 | ✅ 已采纳 |
| ADR-012 | Wails Binding 调用路径一致性 | ⚠️ 已采纳（当前不一致，未修复） |
| ADR-011 | 前端路径拼接分隔符不一致 | ⚠️ 已采纳（违规未修复） |
| ADR-010 | 资源类型字面量硬编码治理 | ⚠️ 已采纳（违规未修复） |
| ADR-003 | 业务逻辑从 Binding 层下沉至纯 Go 包（Logic Sinking） | 🔄 部分采纳（P0/P1/P1.5 已完成，P2/P3 进行中） |

> 统计：🔄 进行中 1 · ⚠️ 遗留未修复 3 · ADR 总数 13（编号空缺：ADR-009）
<!-- /GEN: active-adr -->
