> ⚠️ **冻结区警告**：本文档已冻结（JS 时代历史快照）。当前项目已全面迁移至 TypeScript，路径已变更。
> **请以 [architecture.md](../architecture.md) 为唯一权威视图**，本文仅作历史参考。
>
> **已知过期内容**：
> - 路径 `js/` 现为 frontend/src/（真实源码树）
> - 路径 `js/components/` 现为 frontend/src/components/（真实源码树，由视图层重组为 views/ 下的组件域）
> - 文件后缀 `.js` 现为 `.ts`
> - 组件目录已按三层解耦重组

---

# YSM 模型管理器 — 前端架构规范（冻结快照）
