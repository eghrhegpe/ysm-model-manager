# YSM 模型管理器 — UI 翻译计划（草案，未实施）

> ⚠️ **状态：草案，仅供参考。当前未实施任何 i18n 工作。**
>
> 本文件由 AI 生成，部分内容基于错误前提（声称"界面包含大量英文"），实际项目 UI 已为中文原生。
> 术语统一工作见 `docs/TERMINOLOGY.md`，治理规则见 `docs/CLEANUP_RULES.md`。

## 概述

本计划概述了 YSM 模型管理器前端界面的翻译/I18n 实施方案。
**如未来有国际化需求，需基于实际代码重新评估，不可直接照搬本草案。**

## 背景

当前 YSM 模型管理器前端界面已为**中文原生**，UI 文本（按钮、提示、错误消息等）均为中文硬编码。
项目用户群以中文为主，暂无多语言需求。

**翻译目标（未来）**：如需要支持多语言，需先完成以下前置工作：
1. 提取所有硬编码中文字符串
2. 建立术语表（参见 `docs/TERMINOLOGY.md`）
3. 评估哪些字符串需要翻译（技术术语、错误消息等）
4. 基于 Wails 桌面应用架构设计加载方案（不能用 Web 的 `fetch` 加载 JSON）
...
**翻译优先级**：按组件重要性和用户可见性排序，优先翻译用户界面核心组件。
...
**实施建议**：采用集中式翻译系统，动态加载翻译文件，按需加载翻译内容，确保性能和用户体验。
