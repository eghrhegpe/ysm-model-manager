...
2. **environment-capability.ts:703 `halfToFloat` API 不存在（高置信）**：custom HDR 分支调 `THREE.DataUtils.halfToFloat`，three r185 只有 `fromHalfFloat`，该分支运行时必抛 TypeError。影响：加载自定义 HDR 后打开菜单，直方图控件崩溃。测试内以 monkey-patch 桥接锁定逻辑（environment-capability.test.ts:504-519），修复时撤桥即可转真实路径。
...
1. 待修清单 #2（halfToFloat）建议尽快修——一行 API 名替换 + 撤测试 monkey-patch，高置信必崩路径。
