...
| `typeFromWebDir()` | `frontend/src/backend/web-fs:75` | 从 /web/&lt;type&gt;/... |
| `scanWebModels()` | `frontend/src/backend/web-fs:84` | — |
| `readWebFile()` | `frontend/src/backend/web-fs:179` | 读文件（/web/&lt;type&gt;/&lt;rest&gt; → IDB → base64；wasm.ts 解码链零改动复用） 模型组 name 与组内 rel 在 file key 中无缝拼接（ |
| `scanAllWebModels()` | `frontend/src/backend/web-fs:620` | 扫描全部资源类型的模型（供标签聚合 / 子目录映射等全库操作） |
...
| `GroundParams()` | `frontend/src/utils/3d/caps/ground-capability:37` | — |
| `DEFAULT_GROUND_PARAMS()` | `frontend/src/utils/3d/caps/ground-capability:58` | — |
