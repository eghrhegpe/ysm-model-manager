...
- `parseBedrockAnimationJSON` 消费方：`features/preview-3d/decoder/wasm-decode.ts`（+`loader.ts`，WASM 解码出的动画 JSON，ADR-137 归位）
...
- **消费方文件名漂移已修正**（2026-08-09；2026-08-31 ADR-137 再迁 decoder/wasm-decode.ts）：`parseBedrockAnimationJSON` 消费方为 `features/preview-3d/decoder/wasm-decode.ts`（+loader.ts，旧文 preview-wasm.ts 已过时）；stagger 消费方含 `app-content/site/render.ts`（旧文 site-view.ts 过时）；测试文件均为 `.ts`（旧文 .js 过时）
