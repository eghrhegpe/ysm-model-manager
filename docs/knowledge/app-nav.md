...
quick_intents:
  ...
  - nav_page 恢复
quick_risk_lines:
  - nav:changed 事件必须仅由 app-nav 派发，其他页面禁止自派 nav:changed（跨页跳转应派 nav:changed 借道）
pitfalls:
...
`app-nav` 是应用的主导航菜单组件（Shadow DOM，渲染为左侧固定栏），列出模型仓库、整合包管理、创作者频道、创意工坊、诊断与冲突、设置 6 个入口，底部显示应用版本号。它是 `nav:changed` 事件的主要派发源（2026-08-17 起 `bus.ts` 契约统一为单事件 `nav:changed`，旧 `nav:change` 请求事件已删除——不再有「请求/响应」双事件模型），并在启动时恢复上次浏览的页面。
...
- 监听 bus：`nav:changed`（据此同步高亮与持久化）、`lang:changed`（语言切换时重渲染导航标签）
- 派发 bus：`nav:changed`（导航项点击；启动恢复页面）
- getApp 调用：`GetAppVersion`
- 导航项 id：`repository` / `instances` / `workshop` / `github` / `diagnostics` / `settings`

## 与其他子系统关系

- `app-content`、`PageStore` 是 `nav:changed` 的主要消费方：`app-content` 收到后切页整块重渲染（见知识卡 `app_content`、`page_store`）。`app-nav` 自己 emit 的 `nav:changed` 会被自身监听，用于高亮收敛（2026-08-17 前是 `nav:change` 请求 + `nav:changed` 响应双事件，统一后单事件自环）
- 其他模块（如 `app-sidebar` 底部路径按钮、`app-tree` 工具栏、`repo:search-creator` 流程）也通过派发 `nav:changed` 借道切页
