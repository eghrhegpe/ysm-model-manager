...
- `app-content` 是 `nav:changed` 的主要消费方：`app-content` 收到后切页整块重渲染（见知识卡 `app_content`、`page_store`）。`app-nav` 自己 emit 的 `nav:changed` 会被自身监听，用于高亮收敛（2026-08-17 前是 `nav:change` 请求 + `nav:changed` 响应双事件，统一后单事件自环）
