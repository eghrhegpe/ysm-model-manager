...
- **跨类型浏览歧义扩展名走 ext 兜底路由（2026-08-28 `0d496a84`）**：`switchPreview`/`switchToSession` 复用**当前会话 adapter** 的 build、不做类型探测——因此 PMX 必须先作为首个加载资源（会话为 mmd）才能组内切换；从 YSM/蓝图/投影/车万女仆会话跨类型浏览 `.pmx` 走 `openModel3DFullscreen` → `DetectResourceType`，而 `.pmx/.pmd` 同时声明于 EntityPlayer 与 SceneModel（`resource_types.json`），Go `ClassifyExt` 对多归属扩展名保守返回 `"other"` → `resolvePreviewKey` 查表失败误报「3D 预览暂不支持该类型」。修复：`types.ts` 新增 `resolvePreviewKeyByExt(path)`（从 variants 派生、取首个声明者的 preview key，如 `.pmx→mmd`，只做预览适配器路由派生、不参与类型判定），`openModel3DFullscreen` 仅当 routeKey 为 `""`/`"other"` 时兜底再查 `_openers`
