...
auto_fields:
  ...
  tests:
    ...
    - frontend/src/views/app-sidebar/app-sidebar.keyboard.test.ts
...
- `launcher-detect.ts` — 空态就地配 mcRoot 两入口（**自 settings/launcher-detection.ts 搬家，2026-08-29**；settings 版按钮 + MutationObserver 注入已删，`app-modules.ts` 不再注册）：`runMcSearch`（`GetMinecraftPaths` 扫常见安装位，多结果 `modalSelect` 选择）与 `runLauncherDetect`（`pickDirectory` 选启动器目录 → `DetectLauncherInstances` 解析 HMCL/PCL/Minecraft 多实例 → 弹层选实例 → 经 `views/config-write.ts|writeAppConfig({ mcRoot })` 写 mcRoot，勾选「用作 YSM 根目录」时 `SetResourceRoot("ysm", customDir)`，失败回滚 mcRoot）；成功后派发 `stats:refresh`（sidebar 防抖重载实例列表）。模块级 `_busy` 守卫两类入口并发（都在改 mcRoot）。实例选择弹层原为自建 overlay 骨架，已在 2026-08-29 审核修复中收敛为 `modalPicker`（复用统一弹窗脚手架：单例/焦点陷阱/Esc/退场动画），流程文案走 `launcher.*` i18n keys（见知识卡 `dialog_modal`）。⚠️ **配置落盘禁止手抄 `SaveAppConfig` 六位置实参**（ADR-313）：本文件曾自抄一份并硬编码主题字面量 `"dark"`，而 `THEME_VALID` 无此值 → 落盘后被 `normalizeTheme` 静默归一成 `system`（用户只改目录、主题却变「跟随系统」）。`saveMcRoot` 的第二参 `app?`（原为省一次动态 import）已随出口上移删除
- **整合包卡片拖拽导入**（2026-08-29，见知识卡 `import_queue` 的 `pack-dnd.ts` 节）：`index.ts` `connectedCallback` 调 `features/dnd/pack-dnd.ts` 的 `bindPackCardDnD(root, () => this._instances)`（document 层监听 + cleanup 存 `_packDndCleanup`），拖文件到实例卡片 = 先入仓库再推送进该实例；`.dnd-over` 高亮样式在 `sidebar-css.ts`
- **键盘导航（ADR-308 D2，2026-09）**：`index.ts` `connectedCallback` 绑定 `utils/dom/bind-roving.ts` 的 `bindRoving`（preset `list`、orientation vertical、容器 `#sidebar-instance-list`、项 `.instance-card`）——ArrowUp/Down 移动即激活、Home/End 跳端、Enter/Space 再激活；激活 = 在卡片头几何中心派发合成 click（`clientX/clientY` 供 `--ripple-x/--ripple-y` 涟漪计算）→ 完整复用 events.ts 委托 click 链（高亮/`package:selected`/去重/localStorage），键盘路径零重复逻辑。结构角色由模板承担：容器 `role=listbox` + `aria-label t("sidebar.instanceList")`（tpl.ts `listContainerHTML`，渲染后 render.ts 挂空态时移除 role），卡片 `role=option` + `tabindex=-1` + `aria-selected`（render.ts 逐卡设置）；原语只管交互态（roving tabindex / aria-selected 迁移 / 焦点）。`restoreSelectedCard` 的 rAF 内经 `SidebarHost.syncRestoredIndex?(idx)`（events.ts 新增可选成员）把恢复选中同步到 roving 锚点（不抢焦点）；`disconnectedCallback` 调 `_roving.dispose()`，卸载后键位全寂
- `sync-flow.ts` — 同步流程层（2026-09-08 从 `index.ts` 拆出，P1-3）：`runPush`/`runPull`（整包级 push/pull 执行链）、`bindSelectAll`/`bindSyncSelected`/`restoreCheckboxes`（勾选状态管理 + 全选/同步所选）；经 `getCheckedSets()` 回调读 `AppSidebar` 实例属性 `_checkedSets`（按 rtype 隔离的 Map，定义在 `index.ts`），实例级生命周期随组件 GC 回收，跨重渲染持久化勾选状态；`SYNC_TIMEOUT_MS`（30s 推送等待/兜底超时）；`index.ts` 仅保留 Web Component 生命周期与渲染编排
...
- 派发 bus：`package:selected`、`ctx:show`、`sync:download:missing`（含 token）、`toast:show`、`stats:refresh`、`tree:reload`、`nav:changed`（底部路径按钮跳设置页）
- Go 调用：统一经 `getApp()` 取绑定（ADR-012 红线，禁 `window.go.main.App.*`）——`LoadAppConfig` / `ListVersionInstances` / `GetResourceInstanceStatus` / `GetRepoRoot`（loader.ts）、`PullResourceFromInstance`（index.ts 拉取）、`LoadAppConfig` / `GetMinecraftPaths`（events.ts bindFooter 自动检测 MC 路径；**落盘不在此处直调 `SaveAppConfig`**，走 `views/config-write.ts|writeAppConfig`，ADR-313）；「推送到整合包」由 `features/context-menu/context-menu-file-handlers.ts` 的 `file.push-to-pack` 走 `InstallModelTo`（见知识卡 `context_menu`）
...
- 右键派发 `ctx:show`（type=instance）→ `features/context-menu/context-menus.ts` 转 `menu:show`（见知识卡 `context_menu`）
- 推送经 `sync:download:missing` 交给全局同步 handler（`features/sync.ts` 的 `registerSync`，由 `app-content` 的 `connectedCallback` 直调注册）；数据源对应 Go 端 `go/sync` 包与 `internal/app/app_install.go`
...
- **roving handle 实例级**（`_roving`，ADR-308 D2）：`disconnectedCallback` 必 `dispose()`，卸载后键位全寂（契约测试 `app-sidebar.keyboard.test.ts`「卸载后 keydown 死寂」锁定）；键盘激活走合成 click 复用委托链，**禁止**在键盘路径另写高亮/emit/去重逻辑
