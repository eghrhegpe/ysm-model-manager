...
- 样式走全局 CSS 变量（`var(--card)` / `var(--status-error)` / `var(--status-success)` / `var(--accent)`），层级走 `var(--z-toast)`
...
- **toast 的真实父节点是 `#c`（`.toast-container`），不是 `:host`**（2026-09 由 `css-layer-check` 报出后修）：`:host` 上写 `gap: 8px` 对**孙节点**无效——toast 之间从来没有间距。容器布局（`display:flex; flex-direction:column; gap:8px`）必须落在 `.toast-container` 上，`:host` 只留 `position:fixed` 定位与 `pointer-events:none`。
