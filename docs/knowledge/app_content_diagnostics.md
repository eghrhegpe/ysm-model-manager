...
- **类型矩阵前端接入 + 第三种目标集「前 N 大」**（ADR-262 D3，2026-09-17 / 2026-09-18）：单模型 tab 的控制条有类型选择器 `#diag-perf-rtype`（选项由 `perf-matrix-render.ts|populatePerfRtypeOptions` 从 `loadResourceRegistry()` 填充——**类型表单一事实源是 Go/`resource_types.json`，前端不写死**；注册表不可用时保留「单模型」项回落旧行为）与条数控件 `#diag-perf-max`。`perf-single-bench.ts|singleBenchReadMode` 派发**四态**：空值 → 单模型（需路径）、`__all__` → `--all-types`、`__top__`（`PERF_RTYPE_TOP`）→ `--top-largest`、类型 id → `--rtype`；三种目标集载荷**同形** `{spec, models[]}`，全部由 `perf-matrix-render.ts|renderPerfMatrix` 渲染（**不写第二套渲染器**）：类型汇总表 + 逐模型 identity 行，前 N 大模式额外回显「N + 口径 token + 口径人话」（`topLargestLine`）与逐条体量徽标 `formatBytes(footprint_bytes)`（0/缺席不渲染——单模型模式凭空多一行就是噪音）。**目标集三种互斥，参数绝不叠加**：前 N 大只发 `{top-largest, iterations, format}`（单测用 `toEqual` 锁**精确**参数集而非 `objectContaining`——多带一个键正是要防的回归）。**诚实红线在展示层同样成立**：`cli_analyzable=false` 的类型显示「未采集」而**不是 0.00ms**——空模型数据不得当实测。**控件复用必须连标签一起复用**：同一个 `#diag-perf-max` 在矩阵模式是「每类上限」、在前 N 大模式是 N，`syncPerfCountLabel` 在 `#diag-perf-rtype` 的 change 上按模式换文案（`perfTopLargestCount` ↔ `perfMaxModels`）——标签不换就是界面撒谎（用户会把「每类上限 3」理解成每种类型取 3 个）。Go 侧 `single-bench` 的 `rtype/all-types/max-models/top-largest` 已登记 ParamSpec（ADR-173，否则桥走 legacy 告警路径）。测试：`perf-matrix.test.ts`（分发参数 / 渲染 / 未采集诚实 / 空态 / 选项来自 registry / 前 N 大精确参数集 + 口径回显 + 体量徽标 + 标签随模式切换）+ e2e `diagnostics.spec.ts`（真实浏览器断言选择器选项来自 mock 注册表；**前 N 大用真 CLI 载荷 `TOP_LARGEST_REAL` 断言「N 与口径回显、体量逐条可见、models 顺序 = 载荷顺序」**）。⚠️ 连带修 `e2e/mock-data.ts` 的 `LoadResourceTypes`：此前用 `JSON.stringify` 返回**字符串**，违反 ADR-143 P0「去 string-JSON 化」（Go binding 返回结构体），导致 e2e 里 `loadResourceRegistry()` 恒判空注册表 → 所有注册表驱动 UI（含新选择器）在 e2e 中静默降级为 fallback。
