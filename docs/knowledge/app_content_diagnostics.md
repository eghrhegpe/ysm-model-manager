...
- `css/content-diag.ts` 的 **通用布局三件套 `.diag-pane` / `.diag-bar` / `.diag-bar-row` / `.diag-bar-hint`** —— 全页 tab 共用的「**上栏常驻 + 结果独立**」骨架（`.diag-pane` = 纵向容器、`.diag-bar` = 常驻控制栏、`.diag-bar-row` = 栏内语义行、`.diag-bar-hint` = 栏内说明行）。**2026-09-21 由 `.perf-wrap` / `.perf-controls` / `.perf-row` / `.perf-hint` 泛化而来（ADR-288 D1，规则逐字未变、纯改名）**：前缀 `perf-` 名不副实（`health` / `sync-conflict` 同样要用），而两者本就按日志工具栏（`.diag-log-bar` + `.diag-log-row`，保留——其 padding 与 subtabs/filter/search 子控件确属日志专属）已验证的范式写的。这类布局类在 2026-09 之前**在整个 `frontend/` 零 CSS 规则**（类名是空头支票），性能面板 11+ 控件靠 UA 默认 inline 流换行，功能分组不可见、窄屏折行语义全散，由 `css-layer-check` 报出后补。同批修 `.perf-conc-filerow`（补进 `.perf-conc-row` 选择器，原缺 flex 致行内 label 不伸展）与 `.log-copy`（原无规则 → 每个日志行里一个 UA 默认灰底按钮）。`.perf-mode-off` 是基准模式显隐的**唯一**机制（class），与查看器降级的 inline `display:none` 分工不重叠——inline 胜过 class，故模式接线不可能把查看器藏掉的桌面专属按钮又「显示」回来（ADR-278 §2.4）。闸为何曾漏检见 `docs/adr/ADR-274`；新布局**一律抄这四个类**
...
- `health.ts` — 仓库体检：`initHealthPanel(bar, list, esc)` 接常驻栏（ADR-288 D2/D3），`runHealthAudit(list, esc, btn?)` 调 Go 端 `RepoHealthAudit`（go/repoaudit 同源，GUI/CLI 消双轨），渲染分数环/完整性/缓存/资源/去重/警告到结果区；扫描期 `btn.disabled` 给可见反馈（替代旧「重复点击被 busy 守卫静默吞掉」的外观）
...
- **自动复扫的感知性绑定（2026-09 P2-2 收债）**：`repo:rtype-changed` 的自动复扫**仅面板可见时**触发（`!container.hidden`，bindTabs 以 hidden 翻转可见性）——`go/dedup` 无指纹缓存，每次全价 SHA256，隐藏面板的后台扫描是零价值纯成本。不可见时只更新 `init-pages.ts` 的 `dedupType`，补扫由 bindTabs 的 `TAB_ON_SHOW` 机制（initFn 返回 `{ cleanup, onShow }`，已初始化 tab 每次激活触发 onShow）在**切进面板**时按 `lastScannedType ≠ dedupType` 判定补扫；从未扫过（null）不自动开跑（维持手动开始语义）。记账在 `start` 内部 busy 置位后（`lastScannedType()` 接口）——busy 拦截的重入不改账，快速连切由下次激活兜底，修掉旧实现「dedupType 已新、结果仍旧」的显示失同步
...
- `conflicts.ts` — 同步冲突检测与解决（本文件现只剩这一条链）：
  - **`initSyncConflictPanel(bar, list, esc)`（ADR-288 D2/D3）**：初始化**常驻参数栏**——资源类型下拉（`RESOURCE_TYPE_LABELS` 声明序）+ 异步填充可用实例（`Exists` 过滤，默认选首项）+ 绑定「扫描同步冲突」。不可用态（无游戏目录 / 无可用实例 / 上下文读取失败）→ 原因落**结果区** + **禁用扫描按钮**（点不了好过点了报错）。
  - **`runSyncConflictScan(list, esc, target)`**：调 Go `DetectConflicts`（真比两端 SHA256，哈希不同→`ConflictContentModified`）→ 结果**只写结果区**；`DgCfScanTarget = { rtype, instance, scanBtn }` 把「本次扫描目标 + 按钮引用」带进解决流程，1.5s 自动复扫按**原目标**重跑（list 分离时作废迟到复扫），扫描期按钮禁用。
...
- 样式：`.diag-*`（含通用布局三件套 `.diag-pane` / `.diag-bar` / `.diag-bar-row` / `.diag-bar-hint`，ADR-288）/ `.log-row` / `.conflict-row` / `.scan-*` 动画定义在 `app-content` 样式层 `content-diag.ts`（跨子域共享，不随本卡迁移）
...
- **日志面板工具栏按语义分两行**（2026-09-17 版面收口，方案 A；搜索框 2026-09-28 上移）：行1 = 子 tab（操作 / 运行时）+ 搜索框 + 动作（刷新 / 复制 / 清空日志），行2 = 状态筛选 chips；两行容器为 `.diag-log-row`，`.diag-log-bar` 退化为纵向堆叠的框。立因：9 按钮 + 1 输入框挤单行时，`flex:1` 的 spacer 把「清空」（破坏性动作、且只清操作日志）与筛选 chips 划成一组、却把刷新 / 复制推到行尾，视觉分组 ≠ 功能分组，且窄宽下 spacer 随 `flex-wrap` 折行挤散动作组。清空恒排行尾；**搜索框后续上移行1 紧跟子 tab**（搜索按激活子 tab 分派过滤、属「视图范围」语义，与子 tab 同层），行1 搜索框 `flex:1; min-width:110px; max-width:320px` 吃掉子 tab 与动作组之间的空档，行2 只剩筛选 chips。类名 ↔ 规则同步由 `css/content-diag-classes.test.ts` 兜底（`.diag-log-row` 属布局类，有规则即通过）。
...
- **状态 chips 是横向（结果好坏）筛选，与搜索（含 Operation 纵向）正交相交**（2026-09-28 补全）：chips 五个档位 = 全部 / 成功 / 失败 / **警告（warn）** / 跳过，对应 Go `Status` 取值域 `success/failed/warn/skipped`（`go/types/types.go|StatusToLevel` 同源）。**与「成功/失败」不冲突**：四者是互斥的 Status 值，加「警告」是补洞而非改语义——此前 `Status="warn"` 的行（扫描断链、部分同步失败、error-diary 的 UI warn）在除「全部」外的任何 chip 下都不可达，只有「全部」能翻到。筛选与搜索是 **AND 交集**（`logs.ts|dgLsFilterDiagLogs`）：「搜索里筛状态」可精确到「成功+扫描」这类组合。筛算什么、怎么筛归前端（读 Go 已筛日志做展示层收窄），状态语义单一事实源 = Go `Status`+`Level`（前端不重判「状态该不该红」）。🔍 遗留：`ui`（界面）日志来自 `core/error-diary` 的 `DiaryStatus = "failed"|"warn"`，`warn` 已可筛出；但「成功+扫描」「失败+警告」这类**组合语义目前只能靠搜索+chip 交集实现**，没有独立的多维筛选 UI。
- **操作类型下拉是纵向筛选，与状态 chips 正交叠加**（2026-09-28 落地）：行2 chips 右侧新增 `<select id="diag-log-op-filter">`（选项 = 「全部操作」+ `OP_META` 七类：import/scan/download/sync/rename/delete/ui），与状态 chips、搜索框三者 **AND 交集**——互斥维度叠加不冲突（如「扫描 + 警告」可精确锁定扫描断链）。落点单点：`logs.ts|dgLsFilterDiagLogs` 的 filter 回调加一条 `(l.Operation ‖ "import") !== opFilter` 判断，复用既有分组键口径（与 `logs.ts|dgLsGroupByOp` 逐字一致）。**绑定与 chips 同口径**（`init.ts|dgInBindLogOpFilter`）：只作用于操作日志，运行时子 tab 下仅更新选中态、不回落拉列表。样式复用搜索框度量（`content-diag.ts|.diag-log-op-filter`，`margin-left:auto` 推行尾）；选项集在 `tpl.ts|diagnosticsHTML` 硬编码，与 `OP_META` 同源，**新增操作类型须两处同步**（`tpl.test.ts` 锁选项集）。
- **日志展示窗口容量单点化**（2026-09-28）：`logs.ts` 顶部 `DIAG_OP_WINDOW` / `DIAG_RUNTIME_WINDOW` 两个具名常量取代原 `slice(-500)` / `slice(-300)` 字面量，注释标明 Go 侧对应上限（操作 500 = `logs.go|logMaxEntries` 可配置；运行时 200 = `runtime.go|DefaultRuntimeCap` 固定）。**运行时后端恒 ≤200，300 是防御性冗余**。改 Go 容量须同步此处（不崩，仅影响窗口内检索范围）。完整能力面复盘见 `docs/plans/diagnostics-log-capability.md`。
- **运行时日志已结构化：chips 在运行时子 tab 下真正生效**（ADR-289，2026-09-20）：Go 捕获层（`go/logs/runtime.go|RuntimeBuffer.Write`）从 Message 推断 `Level` 并提取行首 `[tag]` 前缀写入 `Tag`（`types.RuntimeLog` 增 `Tag` 字段，绑定已重生成）。前端随之三变——① `logs.ts|dgLsFilterRuntimeLogs` 接入 chips 级别筛选与 Tag 搜索命中域；② `logs.ts|dgLsRuntimeStatusIcon` 按 Level 出图标（不再固定 joystick）+ `.log-tag` 徽标（CSS 在 `content-diag.ts`，与 `.log-op` 同范式）；③ `init.ts` 的 chips 绑定在运行时子 tab 下改为 `loadRuntimeLogs`（**仍不回落拉操作日志**，既有断言不变）。⚠️ **chip 值（Status 词汇 success/failed/skipped）与 Level 词汇（info/error/debug）不同族，须显式映射**：failed↔error、success↔info、skipped↔debug；warn 档是**级别阈值**（收 warn+error+fatal，与操作日志的精确 Status 匹配语义不同）。⚠️ CSS 走模板字符串，注释里写反引号包裹的 `[tag]` 会触发 ReferenceError（实测踩到）。
...
- **tab 的「进入语义」必须统一：进入即有内容**（2026-09 补齐，起因是评审实测：诊断页九个 tab 混了**三套**范式——`log` 进即加载（`loadDiagnosticsLogs`）；扫描三兄弟（conflict/health/sync-conflict）进即有**引导空态 + 主按钮**（`scanHint`/`healthHint` 写在 tpl 初始 HTML 里）；而**性能五兄弟**（single/gui/conc/hist/trace）进去是**裸空 div + 工具栏一个孤零零的按钮**，用户得先去别处「制造数据」再回来手动刷新——`trace` 的文案甚至自己写着「加载模型后自动记录，点击刷新查看」，即**必须先去 3D 预览转一圈**）。现统一为**两级**：
  ...
  - **需要跑进程/CLI 的 → 引导空态 + 显式按钮**：`single`/`gui`/`conc`/`scan-bench-out`/`hist` 的输出容器在 `tpl.ts` 预置 `diagnostics.perfIdle` 引导空态（照抄扫描三兄弟的 `stat-row` 内联配方，**进入即有话说**），按钮仍留在工具栏；**不做「进 tab 自动 spawn CLI」**——与扫描三兄弟同口径（进 tab 不跑进程，跑不跑由用户点）。
    - **ADR-288 修正（2026-09-21）**：起初「引导空态 + 主按钮」**同住结果容器**，结果整块 `innerHTML` 替换就把启动按钮一起抹掉（`app-content` 面板按页缓存 ⇒ 该入口会话内不复活，见 `conflicts.ts` 条目的 dead-end）。现两块分居：**按钮进常驻参数栏**（`#diag-health-bar` / `#diag-sync-bar`），**空态留结果区**。同步冲突的 rtype/instance 参数也随之上栏，进 tab 即有参数（初始化只做 `LoadAppConfig` + `ListVersionInstances` 两次轻量读，**不是**跑进程，§2.5 判据不变）。
...
- **诊断页性能分组按「动作 / 产物」两轴（ADR-278，2026-09）**：9 → **7 个 tab** —— `log` / **`bench` 跑基准** / `gui` 端到端 / **`record` 性能记录** / `conflict` / `health` / `sync-conflict`。起因是三件实测事实：① 性能五兄弟并存**三套进入语义**（已先收口为两级，见上条）；② tab 按「哪段代码跑」（**机制**）切而非按「想干什么」（**意图**）——`single` 与 `conc` 是**同一个基准**的串行/并行两态却各摆一遍参数，第 4 个基准（引擎对照）藏在 `single` 的工具条里当按钮，`trace` 的数据全来自 3D 加载管线却挂在性能组；③ **ADR-262 自己写了却没落地的意图**——`tpl.ts|VIEW_TESTIDS` 注释声明「并发 tab 复用同一套目标集 / 排序控件……不写第二份」，实现却写了 `diag-perf-conc-target` / `-order` 两份 DOM（`perf-concurrent.ts` 的注释同样自称「不另立第二套」）。落地要点：
  ...
  - **同日五修（大刀）：门禁单源化 + 登记面穷尽护栏**：四修只拆了 rtype 路径的雷，apply 循环里残留同型第二颗——它按表直写 `el.disabled = unread.includes(mode)`，对基准三件套（表登 [conc,scan] 不读）在 single 下会**无条件启用**，盖掉目标集维（single+全库勾选基准照样可点）。修法：新增导出 `BASELINE_CONTROL_IDS` 单点名单（现居 `perf-common.ts`——最初放 `perf.ts` 使 perf ↔ perf-single-bench 构成模块环，两边只在函数体内读绑定靠 TDZ 注释走钢丝，2026-09-23 迁至共享叶断环，见「断环」条），apply 遇名单成员跳过、统一末尾委托 `syncPerfBaselineControls` 兼并判定；同时补**穷尽性护栏**（此前「未登记 = 所有模式可读」是静默宽容，新控件忘登记永远绿）：`perf-mode.test.ts` 静态扫 tpl.ts bench tab body 模板串（起点 `id: "bench",` 后首个反引号、终点 `id: "record",`；happy-dom 下 `import.meta.url` 是 http:// 不可用 `new URL`，读文件走 `process.cwd()`）内全部 input/select，每个控件必须有归宿：**行归属（所属 perf-row 开标签带 data-perf-mode→缺席即整行隐藏）∨ 不读表/基准名单（共用行可见但置灰），二选一即可**；两者都没=可见可改不被读，当场红。两个实现坑：行归属不能拿控件自身所在行（run+model 同行合排，data-perf-mode 挂在 div 上），也不能拿「向前最近 <div>」（row-label 嵌套误抓）——取控件前最后一个 perf-row 开标签再沿 <div/</div> 配平验证仍开着；抽样断言防正则空转假绿。变异检查：往 tpl 注入裸 `<input id="diag-perf-newprobe">` 恰好红该例。
...
- **可分析性事实源迁入 `resource_types.json`（`cliAnalyzable`）+ 目标集选择器改为只列可分析类型**（2026-09-21，审核修复）：审核发现目标集 selector 渲染 registry 全部 15 类，其中 13 类 CLI 无解析链路（PMX/PMD/VRM/FBX/GLTF 的解析器只在前端 3D adapter）、`resourcepack`/`shaderpack` 更不是模型——用户选中即被 Go 判 `unsupported`，是「渲染出必然失败的选项」的诚实语义反面（ADR-278 §2.6）。
  - **为什么不是前端本地过滤**：判定归 Go 是回归红线（前端只读不判），此模块历史一贯做法是「Go 拥有事实、前端消费、再加机检护栏」（`1c5a654d9` 登记 maid-model、`5aaba80f3` 单源化 `BASELINE_CONTROL_IDS`（2026-09-23 起名单住址 = `perf-common.ts`，断 perf↔perf-single-bench 环）、`8a76da041` 加「登记面穷尽」护栏）。前端自写一份「哪些类型能跑基准」= 第二份真值。
