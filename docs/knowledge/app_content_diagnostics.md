...
- **类型矩阵 `--rtype` / `--max-models`**（ADR-262 D3，2026-09-17 切片 C）：`go/cli/perf_targets.go|scanBenchTargets` 从 `scanner.ScanEntries`（**发现权单点**）取条目、用 `classifyForScan` 过滤类型、**路径字典序**排序取前 N（Walk 顺序依文件系统而变，测试/AI 断言需可复现）。载荷 `{spec:{rtype,max_models,iterations,analyzed,unsupported,cli_analyzable}, models:[单模型载荷…]}`，与 `--model` 互斥、仅 `--format json`（text 模式明确拒绝而非静默降级）。**发现白名单 ≠ 可分析白名单**：`cliAnalyzableRtype` 当前仅 `ysm`（PMX/PMD/VRM/FBX/GLTF 的解析器只在前端 3D adapter，CLI 拿到是空模型）→ 这类类型在矩阵里**只出身份、`stages` 为空**并附解释性 hint，绝不拿空模型阶段数据冒充实测。`scanFirstModel` 同时补认目录式入口（`IsYsmEntryJSON`），纯解包目录仓库不再「未找到模型」。测试 `go/cli/bench_matrix_test.go`（fixtures 跑 2 个真实目录样本 + 不可分析类型不伪造 + text/互斥拒绝）。
