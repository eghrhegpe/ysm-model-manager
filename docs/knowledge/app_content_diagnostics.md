...
- **两个日志窗口容量是双轨，改 Go 容量前端不自动同步**（2026-09-28 复盘）：Go 侧操作日志上限 = 500（`go/logs/logs.go` `logMaxEntries`，可配置）、运行时缓冲 = 200（`go/logs/runtime.go` `DefaultRuntimeCap`）；前端各自 `slice(-500)`（`logs.ts` 操作日志搜索前）与 `slice(-300)`（运行时消息过滤前）。**运行时后端恒 ≤200，前端 `slice(-300)` 是防御性空操作**。两处口径各写各的——若将来调整 Go 容量，前端 slice 易漏同步（不会崩，只影响「窗口内检索」的范围与「无匹配」占位的准确性）。**`skipped` 状态非预留**：`go/sync/sync_push.go` 推送遇同哈希/同名真实写 `Status:"skipped"`，该 chip 有效。复盘全文见 `docs/plans/diagnostics-log-capability.md`。
