...
- **延迟编排（2026-10 方案 A 起，见不变量「顶层入场编排 = stgUnits 表驱动」）**：顶层单元一律经 `stg-card.ts|stgUnits` 有序单元表按声明顺序自动派生起始延迟（单卡/行组 +60、卡组 + (n-1)×cardStep+60），渲染函数签名带 `startMs`、内部对卡组再经 `stgCards(startMs, step)` 派生组内阶梯。**新增顶层卡/组 = 在对应 tab 的 stgUnits 表加一项，禁止再写 delayMs/startMs 裸值或自建编排**（历史：曾散落 0/60/90/…/300 十一档手填字面量且两套步长混用，2026-10 前收口为命名档表，现统一为表驱动规则）。折叠区（解析/鸣谢 details）内部例外：默认收起、展开才播，`STG_BAND.preview3d` + `stgCards` 自定内部节奏。
