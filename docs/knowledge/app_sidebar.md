...
category: ui
source_files:
  - frontend/js/views/app-sidebar/index.ts
  - frontend/js/views/app-sidebar/tpl.ts
  - frontend/js/views/app-sidebar/data.ts
  - frontend/js/views/app-sidebar/loader.ts
  - frontend/js/views/app-sidebar/render.ts
  - frontend/js/views/app-sidebar/events.ts
  - frontend/js/views/app-sidebar/sidebar-css.ts
use_when:
  - 侧边栏
  - 整合包列表
  - 版本卡片
...
- Go 调用：静态 import `LoadAppConfig` / `ListVersionInstances` / `GetResourceInstanceStatus` / `GetRepoRoot`（loader.ts）；动态 import `PullResourceFromInstance`（拉取）、`SaveAppConfig` / `GetMinecraftPaths`（bindFooter 自动检测 MC 路径）；「推送到整合包」由 `core/context-menus.ts` 的 `file.push-to-pack` 走 `InstallModelTo`（见知识卡 `context_menu`）
