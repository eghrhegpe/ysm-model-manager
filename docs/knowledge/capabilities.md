...
- **`VIEWER_PURE_ACTIONS`** — 纯前端动作集（`batch.copy-paths`/`batch.export-list`/`file.copy-path`），不调 Wails binding，viewer 模式恒可达。`noop` 假动作已退役（menu-defs kind 判别后标题项不占 action 空间，不入集）。
