...
- `download-queue.ts` 族（原单文件超 100 行红线，ADR-040 拆为 tasks/store/progress/ui 四件；ADR-208 D2 再拆 web 入队分支至 `download-queue-web.ts`）：
  ...
  - **`download-queue-progress.ts`（99% 卡进度守护）**：`createProgressGuard` 状态机分两档——小文件（`total ≤ 100KB`）从 `<10%` 直跳 `≥99%` → 锁 99%，**300ms** 后补写 100%；大文件（`total > 1MB`）同样条件 → 锁 99%，**2s** 后转「refresh SVG + …」菊花动画（`_dotTimer`/`_dots` 每 400ms 加一个点，**2026-09-20 R8 整改后状态收进守卫闭包 CmPgCtx，不再挂 pctEl DOM 节点自定义属性**；图标位走 `UI_ICONS.refresh`+`innerHTML`，不再是 `⏳` emoji）；`forceFileDone` 在 `file-done` 到达时强制把卡在 99% 的进度覆盖为 100%（fail 显示 `UI_ICONS.error` 并复位锁定），失败汇总行同走 `${UI_ICONS.warning}` 前缀；3s `completeTimer` 收口与队列结束双路互斥（`beginQueueEnded`，防重复 onAllDone/cleanup）
