...
quick_intents:
  ...
  - 菜单行为执行、ctx:show
quick_risk_lines:
  - 菜单结构声明在 menu-defs.ts（唯一事实来源），行为在 core/context-menu-handlers.ts（HANDLERS 表）
  - 禁止 view 层手写菜单项
pitfalls:
