...
- 调用方：app-tree 高级筛选入口（结果交 `toolbar-search.ts` 的 `runFilter`：调 Go `SearchModels`，命中路径写入 `AppTree._filterPaths` 集合驱动树过滤；无独立 bus 事件——旧文所称 `filter:results` 事件从未存在，已移出契约，见 `UI-Design.md` D6 记录）
