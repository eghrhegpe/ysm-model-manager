...
pitfalls:
  ...
  - readEntries 分页：Web 标准 API 单次最多返回 100 条 FileSystemEntry，必须循环调用直到返回空数组才读完目录——单次调用会静默漏掉第 101+ 个文件（`features/import/collector.ts` 已收敛为 `readAllDirEntries` 循环读取，`1cd8e305`）
use_when:
  - 拖拽导入
  - DnD 文件收集
  - 文件夹整组分组
