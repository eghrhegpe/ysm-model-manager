...
- ✅ **刀㉝ padding 立闸：454 处硬编码 padding 入账 + 判定层补齐 + 间距体系断层定位**（2026-09，承接刀㉙ 遗留「22 种 padding 归 5 档」）：
  ...
  - **实施进度（2026-09-22，ADR-294 落地后）**：路线图 ①（补 `--sp-1..5` 间距档 + 判定层 `--sp-*` 优先）已在提交 `b2b5f4fa9` 落地；**首轮机械收债**：`--fix` 值等价归位 **46 处**（单值精确命中：`4/8/12/16/24px→--sp-*`、`3px→--pad-btn-tool`、`6px→--pad-nav`；字体/圆角/阴影/过渡全值等价），基线 463 → **417 条**。**甄别经验（机械档的安全边界）**：`--fix` 的 padding 就近档对 `10px→--sp-2`（实 8px）、`14px→--sp-3`（实 12px）、`2px→--sp-1`（实 4px）这类**非精确值会位移 ±2px**，且 `10px` 按 §5 层级3（10-12px）**语义上该归 `--sp-3` 而非 `--sp-2`** —— 属 D3「按元素语义归档」的人工区，**不交给机械档**（本轮已回退 7 处，只保留精确命中）。**路线图 ③（基线收缩）是真的渐进**：剩 417 条里绝大多数是组合值（`4px 8px` 等）与孤品，需逐元素人工归 `--btn-padding-*`/`--sp-*`。
  - **组合值波次（2026-09-22，子代理聚焦 35 处）**：从剩 417 条过滤出**数值精确命中** `--btn-padding-*` 的 35 处组合值（`3px 6px→sm`×4 `4px 8px→md`×14 `4px 12px→filter`×16 `5px 12px→lg`×1），子代理逐个看元素上下文：**归位 8 处真按钮/tab/chip**（`.mc-pick-cancel`/`.ysm-3d-popbtn`/`.preset-chip`/`.slide-back`/`.cr-preset-btn`/`.cr-add-area button`/`.gh-action-btn`/`.pv-tab`），**留 27 处**（tooltip/容器/网格/输入框/表格单元格/下拉项——值相同但不是按钮语义，归 `--btn-padding-*` 是语义冒充；`--sp-*` 才是它们的归位方向，留后续波次）。基线 417 → **409 条**。**经验**：组合值归 `--btn-padding-*` 是「先值对、再语义对」双重闸——数值命中只是候选，元素角色必须真是按钮/tab/chip 才换；网格/表格/输入框即使值相同也绝不硬套（本轮 27 处全留）。
  - **内容间距波次（2026-09-22，主模型脚本执行）**：从剩 409 条清出**四类明确内容间距**共 **32 条**：`0 4px`→`0 var(--sp-1)`（tag/角标/日志 chip 横向，值等价）×17；`4px 0`→`var(--sp-1) 0`（工具条/错误行垂直）×9；单值 `10px`→`var(--sp-3)`（§5 层级3 10-12px，+2px 归档）×5；单值 `14px`→`var(--sp-4)`（§5 层级4，+2px 归档）×1。基线 409 → **377 条**。**关键经验**：① **`0 4px`/`4px 0` 换 `0 var(--sp-1)` 混合形态能出债**——判定闸 `design-tokens.ts` ⑥ padding 段只对「纯 px 组合」报，值含 var() 即豁免，而 `0`+`--sp-1`=基准 `0`+`4px` 完全值等价；② 块注释中段行（`css.ts:100` 那种 `（display:flex;...padding:4px 0;…）＝作者本意`）`isCommentLine` 行首判定识别不了、被闸当违规报——**不能为清债改注释文本**，留着（每波 1-2 条）；③ **剩 377 条全是无值等价档的中高频组合**（`2px 8px`×24、`4px 10px`×16、`8px 12px`×15…）——它们语义是「按钮/标签/输入」或「内容间距」，归 `--sp-*` 需按角色，`--btn-padding-*` 值不对，D4 组合档是唯一省力路径但需 ADR 批准。
  - **组合档波次（2026-09-22，ADR-295 落地，主模型执行）**：沿 ADR-294 路线图走到「D4 组合档」十字路口，量化确认剩 377 条里**前 13 高频值占 185 条（52%）**，且共同规律 = **垂直分量全对齐既有档、横向分量无档可归**（10/6/8px 是 §5 层级**区间下沿**，而 `--sp-*` 取的是上沿）。立 **ADR-295**（取代 ADR-294 D4「明拒不建组合档」，保留防膨胀精神）：
    - **新增档**：`--sp-vh-btn: var(--pad-nav) 10px`（6px 10px 内容间距组合）；`--btn-padding-tool-lg: var(--pad-btn-tool) 8px`（3px 8px）；`--btn-padding-filter-lg: var(--pad-filter) 12px`（4px 12px）。
    - **判定层**：新增 `COMBO_PADDING_TOKENS` 表（源组合值 → 令牌名，**只精确命中不建议**，与垂直表「近档 ≤2px」哲学不同轨）+ `suggestPaddingToken`/`fixLineTokens` 支持组合值 → `--fix` 可机械清组合债。
    - **清债结果**：`--fix` 一次清 **67 条**（`2px 8px`→tool-lg×25、`6px 10px`→sp-vh-btn×13、`4px 12px`→filter-lg×12、`4px 8px`→md×12、`3px 6px`→sm×3 + 单值 2px×1），基线 377 → **311 条**。
  ...
  - **按钮族 2px 垂直档波次（2026-09-22，承 ADR-295 D1 续建，用户拍板「补 2px 垂直档」）**：剩 284 条中 270 条 padding，可建议率此前 0%（无值等价组合档），本波**新增 2 个组合档 + 补 3 个建档遗漏**，`--fix` 机械清 **50 条**，基线 284 → **234**：
    - **新增档（值等价，防膨胀口径）**：
      - `--btn-padding-std: var(--pad-btn-secondary) 10px`（4px 10px）——标准按钮/输入载体（`.cr-back`/`.cr-search-input`/`.te-add-btn`/`.tb-btn`/`.sb-btn`/`.batch-item`/`.ws-back-repo`/`.cc-btn`/`.undo-btn` 等），**×16**；
      - `--btn-padding-xs: 2px 4px`（2px 4px，全字面量不随字号缩放）——紧凑输入/选择器（`.cr-input-type`/`.diag-log-op-filter`/`.af-inp`/`.ysm-ovl-select`/`.stg-select` 等），**×14**。
    - **补建档遗漏**：`3px 8px`×6 是既有 `--btn-padding-tool-lg` 的**精确展开值**，但因表键只收录源值 `2px 8px`，同展开值的 `3px 8px` 写法从未入表 → 6 处漏网。补键即 **+6** 可清。同理 `2px 6px`×14（全按钮）补 `→ --btn-padding-sm`（垂直 2→3px，+1px 映射按钮最小档）。**教训：组合档表是「源组合值 → 令牌」双向映射，应同时收录「同展开值的不同源写法」**，否则同值不同键会漏网。
    - **方法（先画像再建档，防误挂）**：上轮曾踩 `4px 4px` 前缀误挂 `--sp-vh-btn` 的坑——**短值组合必须逐处看上下文定语义**，不能只看值的前缀命中。本波对每个候选簇先拉全量 `file:line` 画像、按选择器归语义（按钮/输入/面板/列表行），确认**单一语义**才建档。`4px 6px`×17 因语义三分（输入 `.dlg-input*`/按钮 `.sb-btn`/面板 `.dlg-preview`/列表 `.cr-scroll`）**不统一建档**，留人工逐处。
    - **清债**：`--fix` 50 处全部落在 4 个目标令牌（`--btn-padding-std`×16 / `--btn-padding-sm`×14 / `--btn-padding-xs`×14 / `--btn-padding-tool-lg`×6），零 `--pad-*`/`--sp-vh-*` 误挂。逐条 diff 审后**语义全部正确**。
    - **验收**：`test_design_tokens` 全绿（新档展开值对账）✅ / contract-tests 103/103 ✅ / vite build ✅ / typecheck ✅ / biome（--write 清 3 处存量 noNonNullAssertion + 正则字面量 sugar）✅ / 基线 284 → **234**。
    - **方法论沉淀**：① 组合档表补键时**同时查「同展开值的不同源写法」**（本次揪出 3px 8px 漏网）；② **值等价优先于映射既有档**——横向 10px 曾有归 `--btn-padding-filter-lg`(12px) 的提议（+2px 横向位移），因「既有组合档全值等价」哲学否决，改新建 `--btn-padding-std` 值等价。
  - **内容间距组合档续建波次（2026-09-22，承 ADR-295 D1 续建，用户拍板「B：建 2 档，5px 8px 留人工」）**：剩 214 条前（当时 234），按覆盖率画像 `6px 12px`×9 与 `8px 10px`×11（全 2D/3D 混合栏/卡片行），建 **2 个 `--sp-vh-*` 值等价档**，`--fix` 清 **20 条**，基线 234 → **214**：
    - `--sp-vh-hdr: var(--pad-nav) 12px`（6px 12px）——栏/头部/配置行容器（`.cr-browser-bar`/`.gh-creators-list`/`.hdr`/`.diag-config-item`/`.ysm-ovl-bar`/health×3/`.mdli-trunc-warn`），**×9**；
    - `--sp-vh-card: var(--sp-2) 10px`（8px 10px）——内容卡片行/菜单行（`.cr-creator-card`/`.cr-local-card`/`.pv-card`/`.nav-item`×2/`.cm-row`/`.fr-empty-note`/`.fr-unload-row`/error-note/section/`.mc-pick-item`），**×11**。
  ...
  - **性能诊断图表容器档波次（2026-09-22）**：剩 205 条中 `8px 2px`×5 全部同源（`perf-trend`/`perf-trace`/`perf-concurrent`/`perf-single-bench` 诊断子系统图表容器），内联 `style="padding:8px 2px"` 包裹 ganttSvg/bars。垂直 8px=`--sp-2` 对齐、横向 2px 为**图表贴边刻意极窄**（图表内部自带间距）。建 `--sp-vh-perf: var(--sp-2) 2px` 值等价，`--fix` 清 **5 处**，基线 210 → **201**。**经验：诊断域专属横向极窄值（2px）也是正当语义——「横向非标准」不等于该降级，只要同源语义统一 + 值等价，就值得建档**。
  - **纯垂直呼吸档波次（2026-09-22，用户拍板「A：建 2 纯垂直档」）**：剩 201 条中 `2px 0`×14 + `1px 0`×6 = 20 处**方向残缺**（横向 0），语义统一 = hint 文本/列表行/错误行的「上下内边距 + 横向贴边 0」纯垂直呼吸。**打破「方向残缺值不建档」的 ADR-295 条目先例**（覆盖率高 + 值等价），建 `--pad-v-2`(2px 0px)/`--pad-v-1`(1px 0px)，`--fix` 清 **20 处**，基线 201 → **181**。
...
- ✅ **刀㉝ 创作者频道（community）锐评修复轮：P0 四项 + 死码一轮清**（2026-09，用户「锐评前端创作者菜单设计」→「尝试修复」）：
  - **锐评总判**：数据面骨架干净，脏的全在 UI 表达层——命名三体（community/workshop/github 互指）、鬼面板与死 CSS、语言串流进数据面、desc 双语义吞原文。本轮只动 P0 + 纯删除项，P2 结构性（tier 预计算/favs 缓存/清理槽代际/定时器）留立项。
  - **P0-1 空态引导反向**：`workshop-tabs.ts` 无站点空态原为「上传图标 + `t("workshop.exportSite")`」= 让用户**导出**空数据，而注释写的是「允许手动导入站点配置」——改 `UI_ICONS.import` + `t("workshop.importSite")`，图标/文案/恢复路径三者对齐（回归：`workshop-tabs.test.ts` 断言含 `importSite` 且不含 `exportSite`）。
  - **P0-2 i18n 串流入数据面（两处落盘污染）**：①`site/edit.ts|eeBindCreatorsEdit` 的 `cr-add` 原以 `t("workshop.newCreatorName")`/`t("workshop.newCreatorDesc")` 当默认 name/desc，保存即**把语言包文案写进 creators.json**（三语同名不同值）；改落空值交给编辑卡 placeholder 引导，并在保存路径**过滤空名条目**（防未命名占位行成垃圾数据）。②`features/community/community-data.ts|mergeLocalAuthorsInto` 本地作者 `desc: la.desc || t("community.fromLocal")` 同因落盘；改落 `""`，「来自本地仓库」提示**上收视图层**按 `_fromLocal` 现取当前语言（`site/render.ts|createCrCard` 卡片 + `site/events.ts|cmCrBuildDetailHtml` 浮层）。连带删除 `workshop.newCreatorName`/`newCreatorDesc` 两键（三语包同步，`frontend/public/locales/*.json` 由钩子重生成）。回归：`edit.test.ts` 第 8/9 例、`community-data.integration.test.ts` 空 desc 例、`render.test.ts` 第 14/15 例、`events.test.ts` 浮层本地提示例。
  - **P0-3 `desc` 双语义吞原文**：`workshop-data.ts|parseDescTags` 原切分字符类含 **ASCII 逗号**（英文描述 "cool models, fast updates" 被碎成 2 个 #chip），且单段描述也返回片段 → `site/events.ts|cmCrBuildDetailHtml` 的 `!descTags.length ? esc(desc) : ""` 分支**丢弃全文只显 chips**。改：切分只认顿号/全角逗号；新增「标签式描述」判定（≥2 段且每段 ≤12 字符，否则回 `[]`）——单段/长句自动回退全文展示，内容不再丢失。回归：`workshop-data.test.ts` 5 例（含 ASCII 逗号段内保留、超长段回退）。
  - **P0-4 筛选行泄露裸 role id**：`site/render.ts|buildSiteTagFilterRow` 动态标签原直接 `esc(tag)`——ja/en 用户看到裸 "vup"/"oc"/"repo"；改走 `getCreatorIdentity({ role: tag }).label`（身份 i18n 单源，未知 tag 回退 YSM 创作者），`data-tag` 仍为原始 id（过滤键 = 数据语义）。回归：`render.test.ts` 第 3 例锁 label 与 data-tag 分离。
  - **P1-5 鬼面板 `#ws-creator-view` 拆除**：tpl 第二创作者面板（`#ws-cr-list` / `.ws-creators-list` / 未翻译字面量 `creators/`）JS 恒 `display:none`、零填充者、CSS 无规则；删模板块 + `creatorView` 句柄链（`RenderSiteViewCtx` / `SiteViewState` / `init-workshop.ts` 装配与早退判据 / 4 处测试夹具），页面收敛为单内容区 `#ws-search-results`。
  - **P1-6 死 CSS 两轮清（证据化）**：脚本比对「CSS 选择器 × 全仓非 CSS 源码引用」得候选，再人工剔除模板拼接假阳性（`cr-tag-${…}` / `gh-row${…}` / `gh-progress-fill${…}` 是动态构造，保留）。删：`content-gh.ts` 的 `.gh-creator-*` + `.gh-cr-*` 两整块（GitHub 侧栏创作者列表/编辑行早已不存在）；`content-creator.ts` 的 `.cr-left`/`.cr-grid`/`.cr-browse-repo`/`.cr-toggle`/`.cr-browser-bar`/`.cr-back`/`.cr-url`/`.cr-creator-icon|-body|-name|-desc|-action`/`.cr-action-btn-muted`/`.cr-fetch-failed`/`.cr-model-count`/`.cr-local-count`/`.cr-detail-row-platforms`/`.cr-error-page` + `.cr-back-repo`；连带清 `content-util.ts` 响应式里给死类 `.cr-left` 配的规则、`content-layout.ts` 共享选择器组里的 `.cr-back-btn`/`.cr-back-repo`。
  - **P1-7 同选择器重复定义合并**：`.cr-section`（`margin-bottom:8px` 与 flex/padding 两处并存）、`.cr-tag-filter-row`（`margin:0 0 8px` + `padding-bottom:8px` **双叠加**成 16px）、`.cr-section-title-lg`/`.cr-section-sub`/`.cr-action-btn-accent` 各两处——合并为单点定义，**间距按合并前实际计算值保留**（本轮不改视觉）。
  - **踩坑与护栏背书**：合并注释时写了「（`gh-left-head*/foot` 仍在用）」——注释体内 `*/` 提前闭合，被 `content-css.test.ts`「注释体内不得出现 `*/`」哨兵当场拦下（正是 2026 年吞掉 `@keyframes fadeSlideUp` 的同款事故载体）。**教训：CSS 内写「A*/B」这类简写会破注释，写全称。**
  - **验收**：`app-content` + `features/community` 全域 vitest **843/843 绿**（56 文件）；`src/views/app-content/css` + `tpl*` + `init-workshop` 72/72；`npm run typecheck` ✅；`npx vite build` ✅；`node scripts/check-biome.ts --files <17 个改动源文件>` ✅。
  - **明拒/留后续（不在本轮）**：`content-gh.ts` 另有 ~30 个无引用 `gh-*` 类（`gh-card-icon/-label/-desc/-external`、`gh-popup*`、`gh-preset-*`、`gh-section-*`、`gh-error-*`、`gh-back-btn` 等，疑似旧「站点视图」残骸）——属 GitHub 域，单独一轮清；P2 结构性（`createCrCard` 每卡 O(N log N) 算 tier + 无图例色条、`loadFavs` 每卡 JSON.parse、`_repoEventsCleanup` 槽未接 `createLoadGuard`（ADR-230 明确要求）、`WS_TAB_LOAD_DELAY_MS=100` 魔法定时器、`extractAvatars` 每次进页全盘扫、`community`/`workshop`/`github` 命名三体归一）待立项。
  - ⚠️ **本条 P0-2 / P0-3 / P0-4 / P1-7 的落地情况经独立复核被修正**（P0-2 改错侧、P0-3 仍丢尾、P0-4 半途、P1-7 名不符实）——**读本条务必连带读下条刀㉞**，勿把本条当已收口的证据。
- ✅ **刀㉞ 创作者频道复核回炉：P0-2 治本到源头 + 「假锁」重铸**（2026-09，刀㉝ 提交后按仓库闭环派子代理**只读独立复核**，结论「有 P1 级问题，建议回炉」——主模型核验关键事实后**采信并回炉**）：
  - **复核核心（刀㉝ 的 P0-2b 改错了侧，对生产数据无效）**：本地作者 `desc` 的唯一生产者永远给非空值——`go/scanner/scanner.go` 与 `frontend/src/backend/web-community.ts|scanWebLocalAuthors` 都是硬编码中文字面量「来自本地仓库」，故 `community-data.ts|mergeLocalAuthorsInto` 的 `|| ""` 分支与 `render.ts` / `events.ts` 的语言兜底在生产**恒不可达**（en/ja 用户仍见中文，只是来源从 TS `t()` 变成 Go 字面量）。**病根在 Go 侧字面量，不在前端 `t()`**——这条记入 `app_content_site.md` 不变量，防下次又只改下游。
  - **治本动作**：界面文案从源头撤出数据面——`go/scanner/scanner.go` 与 `web-community.ts` 两处同口径落空串；被钉进契约的断言同步改为 `""`（`scanner_hotspots_extra_test.go` 原断言 `== "来自本地仓库"`，`browser-adapter.test.ts`、`browser-adapter.contract-b3.test.ts` 同）；展示层按 `_fromLocal` 现取当前语言（刀㉝ 已就位的前端兜底至此才真正生效）。**可达性边界**：该批条目 `type` 为 rtype（与站点 id 集无交集），被站点过滤挡在渲染与落盘之外——「写进 creators.json」当前不可达（潜伏），「语言串存在于数据面」是实的。
  - **P1 残余（`parseDescTags` 仍会丢尾）**：`slice(0, 6)` 让第 7 段起在浮层静默消失（而 `content.descPlaceholder` 教的正是顿号标签串，写满即命中）→ 改为**段数在 2..6 且每段 ≤ `TAG_STYLE_MAX_SEG_LEN` 才判标签串，段数超上限整体回退全文**（不截断）。ja 的「、」本属常规句读，短片段多段仍会被渲染成 chips——词面不丢、只丢标点，判可接受（长句由单段长度上限挡住）。
  - **P1 半途（tag 展示文案）**：未知 tag 曾回退 `t("workshop.roleCreator")`（「YSM 创作者」）而 `data-tag` 是原值 → 按钮文案与过滤语义不符；且卡片/浮层仍显裸 role id（同一 tag 两种呈现）。抽出 `workshop-data.ts|getTagDisplayLabel`（已知身份 → i18n label；**未知 tag 原样返回**）作为筛选行/卡片/浮层唯一出口；`data-tag` 与 `cr-tag-<role>` class 恒为原始 id。
  - **P2 一并处置**：刀㉝ 删鬼面板时顺手造出新死键 `workshop.activeCreators`（三语包已删）；`.cr-tag-filter-row` 的「重复定义合并」名不符实（只把属性抄到后段、旧定义未删，净效果为零）→ 真正单点化；`web-community.ts|SaveWorkshopCreatorsBySite` 内 `const t = c.type` 遮蔽 i18n `t`（回调里写 `t("...")` 会静默崩）→ 改名 `typeSeg`。
  - **「假锁」教训（本轮最值钱的一条）**：刀㉝ 的多处新测试在父提交即绿——`events.test.ts` 把 `parseDescTags` mock 成 `[]`（旧实现本就在空片段时渲染全文，故测不出「丢原文」）、`community-data.integration.test.ts` 用的是生产不可达的输入形状、`render.test.ts` 的 desc 兜底两例锁的是**早于本轮**的提交已存在的 `descText`、`workshop-tabs` 只断言文案不断言图标、`render.test.ts` 整体 mock 掉 `getCreatorIdentity`（只锁「调用过它」，真 bug 面零覆盖）。**口径：把真值源整段 mock 掉的测试 = 没锁；锁回归必须让真实现跑通链路。** 重铸：新建 `site/events-desc.integration.test.ts`（不 mock `workshop-data`，真 `parseDescTags` → 浮层分支，覆盖 ASCII 逗号全文 / 顿号 chips / 超上限回退全文 / 本地空 desc 语言提示）；`render.test.ts` 改用 `vi.importActual` 取真身份函数（连 `workshop-icons` 的 `ICONS` 一并保真，真 `getCreatorIdentity` 依赖它），锁「vup → VTuber 创作者」与「未知 tag 原样」；`workshop-tabs.test.ts` 改按 polyline `points` 属性断言图标（整串 SVG 经 happy-dom 规范化后自闭合标签变形，整串比对必误报）。
  - **验收**：`go build ./...` ✅ + `go test ./go/scanner/...` ✅；`site` + `community` + `backend` 全域 vitest 全绿（含新集成锁）。
