...
| views/app-content                                 | 3.4  | 9.3k  | 已非最低分（2026-09 复核回升 ≈4.0）：perf-cli.ts God Object ✅ 已拆为 7 件（28-147L）；dedup 竞态 ✅ 已消（2026-09-03 会话工厂化）；6 个死代码转发壳 ✅ 已删（见 frontend_design_critique 刀⑮）                                                                                                                                                                                                          |
...
2. ~~app-content/perf-cli.ts 535L God Object（趋势图+single-bench+诊断面板三合一）~~ ✅ **已拆**（2026-09 复核）：`perf-cli.ts` 文件已不存在，拆为 `perf.ts`(28L facade) + `perf-common.ts`(146L) + `perf-single-bench.ts`(147L) + `perf-gui-flow.ts`(130L) + `perf-log.ts`(99L) + `perf-trace.ts`(115L) + `perf-trend.ts`(103L)（以上为**含空行**的统一口径），无一 >150L；`perf.ts` 头部注释逐条记录拆分去向
...
5. ~~detail-3d.ts 多个 300-350L show 函数~~ ✅ **已闭环**（2026-09 复核）：现 485L，抽出 `CardShowConfig` + `showCard` 统一卡片渲染器，6 个 show 函数共享同一模板（invalidate → 骨架 → fetch → stale 检查 → 内容 → wire FAB），各 36-92L、无一处 ≥100L
