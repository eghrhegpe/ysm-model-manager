...
全局 handler 注册入口在 ADR-188 中从 `core/handlers/global.ts` 汇编壳收敛为 **app-content 直接编排**：`connectedCallback` 依次调 `registerPageStore`/`registerSync`/`registerContextMenus`/`registerInstanceOps`/`registerAndroidEvents`，把页面状态、右键菜单、同步、实例操作、Android 事件五组 handler 的 unsub 函数收回，另单独调用 `registerResourceManagerGlobal()` 收资源管理器全局 handler，`disconnectedCallback` 统一释放。页面切换时 handler 不丢、不重复累积。仓库页 DnD（ADR-060）已收敛为 `app-tree` 组件级绑定，不再注册 document 级全局 DnD。

## 核心职责

- **注册编排（ADR-188）**：app-content `index.ts` 直接注册 5 组（registerPageStore/registerContextMenus/registerSync/registerInstanceOps/registerAndroidEvents），unsub 统一收 `globalUnsubs`；core 不再设壳层；`registerResourceManagerGlobal` 由 app-content 单独编排（index.ts registerResourceManagerGlobal 调用点，文件头注释自述「features/views 层注册由 app-content 编排」）——功能单点注册不变量仍成立（app-content 单点注册/单点释放）
