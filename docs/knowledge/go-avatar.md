...
- `SetNodeJS` 未注入时 `DecodeYSMData` 直接返回 nil，不 panic
...
- **R32 修复链（2026-08-31）**：
  ...
  - P3-1 `DecodeYSMData` 重复解码：批量 `CacheAvatarsFromModel` 时每个作者重复触发一次 `extractAvatarFromYSM`→`DecodeYSMData`，同一 .ysm 被解码 N 次。待后续优化（一次解码 + 一次遍历缓存所有作者）。
