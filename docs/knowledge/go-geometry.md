...
- **R29 修复链（2026-08-31）**：
  - `classifyFileInventory` OOM 封顶（P2-1）：恶意归档塞入数十万微小条目可导致 FileInventory 占用数 GB 内存。修复：加 `maxClassifyEntries=10000` matched 条目数封顶，超限 `inv.Truncated=true` + log 标记 + break。`FileInventory` 新增 `Truncated bool` 字段，调用方据此向用户披露「清单可能不全」。
  - `selectBestMaidCandidate` 空切片 panic（P2-2）：`candidates[0]` 在空切片时 panic。修复：函数开头加 `if len(candidates) == 0 { return maidNsCandidate{} }`。
  - code_review P3-1 修正：原实现 `classified++` 计所有非 dir 条目，10000 个 .bin 垃圾条目会耗尽配额。改为仅计 `appended`（matched）条目，dir 在计数前 `continue` 跳过。
