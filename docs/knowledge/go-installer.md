...
- `installDirRecursive` 单个文件失败不中断整棵树，逐条记日志并聚合成一条错误返回；扩展名白名单**注册表驱动**（`types.InstallExtsFor(rtype)`，来自 `resource_types.json` 的 `installExts`）：`ysm` → `.json/.png/.jpg/.jpeg`；`EntityPlayer` 等在无 `installExts` 时不设白名单，全部非可执行文件放行（仅 `.exe/.bat/.dll/.cmd/.scr/.pif/.com/.msi/.ps1/.vbs` 黑名单拦截）。**白名单预计算透传（2026-09 落地）**：`registry.InstallExtsFor(rtype)` 在 `installer.go|callInstallDirRecursiveWithRollback` 顶层算一次、沿 `installDirRecursive`→`installSingleDirEntry`→`isAllowedEntryName` 递归链透传（`isAllowedEntryName` 收 `installExts []string`，不再收 rtype）——原实现对每个 `DirEntry` 都经 `InstallExtsFor` 走 `LoadRegistry` 加锁 + 线性扫描 + 三份 slice 深拷贝，整树递归放大为 N 次锁竞争；rtype 整树恒定，透传语义完全等价
