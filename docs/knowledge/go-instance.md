...
- `BuildInstanceStatusCounts(instances, rtypes, filesRoots) []types.InstanceStatus`（`status_counts.go`，**ADR-310 侧栏计数薄入口**）— 逐实例折 `BuildSyncItems` 的**顶层展示单元**（不递归 children，避免面板父+子重复计数）：`synced`→`Synced`；`missing | diverged`→`MissingCount`（学面板 `tabStatus` 把内容分叉折叠进红）；`optional | legacy`→`Extra`（legacy 在容器聚合里同样算 hasPull）；`disabled`→`Disabled`（中立，不驱动红/橙）。**清单分成两路且一律排序去重**：`Missing` 恒为**仓库侧文件级绝对路径**（缺夹按 `children` 递归展开／无 children 的目录走 `DiffFolderContents`，供 `runDownloadMissing` 逐条 `Install` 的一键安装契约；**绝不吐目录**——目录路径经 `InstallResourceToInstance` 会触发 `InstallDir(父目录)` 过度安装（把兄弟模型包整棵装进实例），对 fileLevel 类型则必判「不支持的文件类型」）；夹内**无任何可逐文件安装的受支持文件**时（如只含 `pack.mcmeta` 的资源包夹、内容全不被 `IsTypeModelFile` 命中的 maid-model 夹）整夹进 `MissingDirs`，由一键安装调用 folder-aware 的 `PushSingleResourceToInstance`（`sync_push.go` 的 `os.Stat().IsDir()` → `InstallDirLocked(该夹)`，装的是这个夹本身）——**每个 `MissingCount` 单元因此都有对应安装路径**，不再出现「徽章恒红、一键安装无动作」。`Extra`（实例侧独有单元）/`Disabled`（`.ban` 单元，实例侧或仓库侧；不推送、不展开）为单元路径。去重的必要性：面板链「混合夹」会把同一平铺文件重复列示（实测容器子项 `flat.pmx` 出现 2 次），不去重会重复安装同一文件。`CustomDir` 单类型时 = `FindInstDir` 解析的实例子目录（与 `processOneResourceType` 同源）。零新增判定逻辑：diff／禁用／聚合全部复用面板链，30s 缓存随之命中；`GetResourceInstanceStatus` / `GetInstanceStatus` 两绑定换线到此处，签名不变
