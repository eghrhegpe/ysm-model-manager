...
- **DiffFolderContents 只比存在性不比内容**（正确性）：两侧同名同相对路径的文件一律标 synced，**不做哈希对比**（sync_dirlevel.go 注释明示）→ 实例侧文件被修改/损坏后仍显示 ✅ 已同步。若治理（D2′-c，待排期）：复用 `contentDiffers` 内容级口径（哈希优先、Size 兜底），与 `ResourceDiff`（D2′-a 已落地）对齐，不必全量 SHA256
