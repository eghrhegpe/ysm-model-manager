...
- **空 faceUV 对象必须回退 box UV（真实库审计抓出的双端分歧，2026-09）**：cube 声明 `"uv":{}` 或 `{"east":{}}`（零个有效面）时，解析层产物 `FaceUV="{}"` + `UV=[0,0]` 零值；`parseFaceUV` 必须带 **parsed 布尔**（零有效面返回 false）→ `parseUV` 回退 `expandBoxUV([0,0])`。TS 旧实现恒返回 true → 六面全 0（纹理角点拉伸），已对齐 Go。真实库实证：15_kluonoa main.json mingpai 薄片（全库 24707 cubes 中唯一 1 例；75 几何文件 box=2815/per-face=21892/per-face+mirror=0/坏 JSON=0）。双端锁定：Go `TestParseUV_EmptyFaceObjectFallsBackToBoxUV` + TS 同构用例。**真实库回归工具**：`go/threejs/uv_realworld_audit_test.go`（opt-in：`$env:YSM_UV_AUDIT_ROOT="<files-root>"; go test ./go/threejs -run TestRealWorldUVAudit -v`，默认 skip 不进 CI）——WalkDir 全量 JSON，每个 cube 三层校验：①独立 Blockbench/GeoCube oracle 48 UV 逐值对拍（box=BB 全负编码 canonical、per-face=GeoQuad q 表 + 顶点序映射，mirror+per-face 跳过逐值仅验尺寸）②box 输出 texel 绝对宽高==|size| 几何维度 ③per-face 输出矩形==声明 uv/uv_size。改 UV 链后必跑。
