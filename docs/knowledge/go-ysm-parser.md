...
- 解析入口设大小上限（zip 内 mods.toml 1MB / geoJSON 5MB / ysm.json 50MB，limit+1 探测截断拒绝，ADR-033；`ysm.json` 50MB 走 `types.MaxReadLimit` 全仓单点）；裸 ysm.json 同 50MB 上限（2026-09-05 锐评 P1#7 修 TOCTOU：原 `os.Stat`+`os.ReadFile` 两步改为 `fsutil.ReadLimitedEntry` 一步，消除 Stat 与 ReadFile 之间文件被替换为超大文件的竞态窗口）
