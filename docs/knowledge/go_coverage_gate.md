...
- `DEFAULT_THRESHOLDS` / `SKIP_PACKAGES`：阈值表与豁免表。
  豁免表只收**测试不可达**的包（进程入口 / 代码生成器 / 测试基建，共 5 项）。
  2026-09 从 14 项收紧到 5 项——被移除的 10 项实测均 ≥50% 门槛（最低 `go/cli` 57.3%），
  豁免它们只会让真实回归隐形。**新增豁免前先确认该包真的达标不了**，别把豁免当降级阀。
...
- 入口包与代码生成器进 `SKIP_PACKAGES`，不以 0% 论失败；**其余包一律按阈值判定**。
