...
- 导出：`initImportQueue(app: ImportQueueHost): () => void`（返回清理函数，内部 unsub `import:history-changed` 等 bus 订阅）、`interface ImportQueueHost`（依赖宿主 `_root`/`_esc`）
...
- 与 [global_handlers](./global_handlers.md) 分工：全局 DnD 在仓库页内直接走 `import-executor` 全局执行器（`directImport`），导入完成后经 `import:history-changed` 驱动本模块刷新已导入列表
