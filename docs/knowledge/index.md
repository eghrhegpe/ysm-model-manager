...
- **mount-preview-module-singleton-race**（mount3D 并发竞态（已闭环 — _gen 代际守卫））：**已闭环**。`mount-preview-core.ts`（现位于 `preview-3d/adapters/`）L249 声明模块级 `let _gen = 0`，`mount3D` 入口`const myGen = ++_gen` …
