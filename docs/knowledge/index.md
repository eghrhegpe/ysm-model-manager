...
| 🍃 ik_solver | CCD IK 求解器 ik-solver / 足部锚地 mmd-foot-ik | leaf | — | IK 求解, 骨骼 IK, 足部锚地, foot IK, 极向量 / pole, CCD |
...
- **preview_3d_migration**（preview-3d 领域根迁移）：ADR-129 第三刀：把 `frontend/src/utils/3d/`（227 文件）整编搬迁到 `frontend/src/features/preview-3d/`。纯改名、收益最低、但暗礁最多。三刀顺序不可逆：第一刀正类型（依赖…
...
- **bone-tools**（跨格式骨骼工具层 bone-tools）：`frontend/src/features/preview-3d/bone-tools.ts` 是 ADR-072 落地后新增的**跨格式骨骼工具层**，屏蔽 YSM spec 扁平 bones 声明与 VRM humanoid Obje…
...
- **model3d**（3D 预览渲染 model3d）：前端 Three.js 3D 渲染层（`frontend/src/features/preview-3d/`），**单会话架构**：场景/相机/渲染器/控制器由统一预览核心 `mount3D`（ADR-066）持有单实例，模型内容经适配器（…
...
- **perception**（3D 感知系统 perception）：`features/preview-3d/perception/` 是实现模型「自主生命感」的感知层子系统：让 Minecraft 角色自动眨眼、呼吸、注视、对口型、随音乐律动。
