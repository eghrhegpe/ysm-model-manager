...
- **global-handlers**（全局事件处理 global-handlers）：全局 handler 注册入口在 ADR-188 中从 `core/handlers/global.ts` 汇编壳收敛为 **app-content 直接编排**：`connectedCallback` 依次调 `registerPageS…
