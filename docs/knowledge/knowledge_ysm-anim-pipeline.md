...
source_files:
  ...
  - frontend/src/utils/3d/ysm-adapter.ts
  - frontend/src/utils/animation/molang-bridge.ts
...
| 模块 | 文件路径 | 职责 |
|------|---------|------|
| **动画玩家** | `utils/3d/ysm-animation-player.ts` | 完整的状态机：时间推进、Clip 切换、控制器管理。导出符号 `createYsmAnimPlayer`。 |
...
| **适配器桥接** | `utils/3d/ysm-adapter.ts` | 将解码数据喂入 Player，并挂载到 `renderModel3D` 实例。 |
