...
- **解码拷贝链实测口径与前提更正**（2026-09 专项调研，V8 13.6 / N=64MB 实测）：L1 base64 串(1.33N) + L2 `atob` 串(1.0N) + L3 `charCodeAt` 拷贝(1.0N) + `HEAPU8.set`(1.0N) = **4.33N**——「3-4×」是 **N_in 口径的下界**，尚未计 C++ 内部 `m_buffer`/`m_binaryData`/`m_decrypted`（各 ~N_in）与输出侧 N_out（.ysm 是压缩格式，N_out 可能显著大于 N_in）。前提更正：① **MEMFS 的 `node.contents` 是 JS 侧数组、不在 WASM 线性内存**（Emscripten 的 MEMFS 是 JS 实现）；② `atob` 产出 **Latin-1 one-byte string（1 字节/字符）**，非 UTF-16 的 2 字节；③ `new Uint8Array(arrayBuffer)` 是**视图（别名）**非拷贝——拷贝须用 `new Uint8Array(typedArray)`。已落地的优化：审查 C-1（`sniffTexSize` 复用 `f.data` 免每张纹理整拷贝 −N_tex、内存直解路径补 MEMFS 收集后清理）+ **ADR-228 网页版消除 base64 往返**。⚠️ **比峰值更隐蔽的风险：WASM 线性内存只增不减**（上限 2GB），HEAP 高位常驻整个应用生命周期。**已排除路线**（勿重复论证）：Go binding 直接返回 ArrayBuffer（Wails 传输只有 JSON）、分块流式喂解析器（`YSMParserFactory::Create` 要求完整缓冲）、Web Worker 降峰值（dedicated worker 与页面同渲染进程，只改卡顿不改内存）；**桌面传输层**仍需资产服务器二进制路由（ADR-228 范围外）
