...
- **MMD zip**（`mmd-controls.ts` `mmdModelInfoNodes`）：候选 = `zipModelCandidates`（虚拟路径，`content-bridges.ts|MmdBottomNavCtx.zipModelCandidates` 字段，由 `mmd-build-load.ts` 组装暴露）；get 保持 basename 匹配（`modelName` = 虚拟路径 basename）；set → `ctx.switchTo(虚拟路径)`
