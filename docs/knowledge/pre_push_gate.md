...
**前端域**（plan.frontend）：`check-layering`（R0/R1/R2/R5/R6/R7 零容忍 + R3/R4/R8 防回退基线）→ `check-path-hygiene`（ADR-146：反桶/深度/上跳/跨边界冻结/双写一致性）→ `check-menu-health`（ADR-085：菜单表 id/labelKey/i18n/dockGroup/kind/render·run 完备）→ `check-ctx-menu-i18n`（扫右键菜单文件里字面量 `t("key")`/`tr("key")` 须存在于 zh-CN 基准包；**2026-09 修复「路径失效致空转报绿」事故**——清单曾整体指向已不存在的 `frontend/src/core/*`（代码迁到 `features/context-menu/` 未同步），叠加「文件不存在即跳过」的容错 → 闸扫零文件却输出「✅ 全部 0 个 key 均存在」。更隐蔽的是本条曾记作「tr 根除后 0 命中恒绿——休眠闸，防 tr() 回潮才重新咬人」，**把真因（路径漂移）写成了假前提**：即便 tr 回潮也照样扫不到。现修法三重 = 重指真实路径 + 正则并覆盖 `t()`/`tr()` + **空域 fail-loud（0 文件 / 0 键 → exit 2）**，实测扫描面 5 文件 / 45 键。结构教训：**硬编码路径清单 + 「文件不存在就跳过」= 静默归零，必须显式断言非空**）→ `check-mock-paths`（ADR-224：vi.mock 路径静态校验，M1 内部 spec 失效硬拦、M2 裸包漂移默认 WARN、M3 .js→.ts 兜底 INFO）→ npm 三件套并行（`vite build` ∥ `tsc --noEmit`）→ `vitest run --maxWorkers 8` 串行在后
...
**2026-09-15 scripts typecheck 双安装点第二例（TS2688，修 main CI 连续多笔同点红）**：`pre-push-gate --static` 的 scripts typecheck 在 CI 长期阻断（34990867684 / 34988991645 等连续红），报 `error TS2688: Cannot find type definition file for 'node'`，本地却恒绿。根因与 `_lib/tool-bin.ts` 头注释记载的**同族第一例**（硬编码根 `.bin` 致 CI 找不到 tsc 二进制）同源，但落在**另一层**：`_lib/tool-bin.ts|resolveToolBin` 修好了**二进制**解析，`scripts/tsconfig.json` 的 `types: ["node"]` 却仍依赖 `typeRoots` 默认向上走到 `<ROOT>/node_modules/@types` —— 而 CI（`.github/workflows/test.yml`）只在 `frontend/` 跑 `pnpm install`，根 `node_modules` 根本不存在；本地因曾根跑 `npm i`（`@types/node` 26.x）而恒绿。注意 frontend 有自己的 `@types/node`（24.x），故**类型不缺失，只是解析路径没探到第二安装点**。修法 = 同款双根显式 `typeRoots: ["../node_modules/@types", "../frontend/node_modules/@types"]`（两处任一在即满足，无需版本对齐——已实证 frontend 的 24.x 满足 scripts 全部类型检查）。**复现手法**（专治「本地绿、CI 红」）：把 `scripts/`（含 `_lib/`）复制到无根 `node_modules` 的临时树、仅 junction 软链 frontend 的 `@types/node` → `npx tsc --noEmit -p scripts/tsconfig.json` 即得同形 TS2688；补 `typeRoots` 后同树 exit 0，旧配置对照组 exit 1。**教训**：本仓「双安装点」（根 npm i / frontend pnpm install）的**每一处隐式解析**——二进制、类型、配置——都必须显式双根，**修一处不等于修一族**。附带辨误：本次红与「Wails 绑定未重新生成」无关，该步 `git diff --exit-code -- frontend/bindings` 在同一 run 中**通过**（工作流继续执行到后续 adr-check），勿据表象误判病根。
...
- **性能退化门禁（`compareSingleBenchBaseline`）必须有**两层**噪声保护**（2026-09-15 补齐第 2 层；2026-09-18 基准特性独立成文件，判定/呈现现位于 `go/cli/bench_baseline.go`——`evaluateBaseline` 只算不说、`compareSingleBenchBaseline` 只管 text 渲染）：① `base` 与 `now` 双双 ≤ `benchNoiseFloorMs`(1ms) → 跳过；② **绝对增量** `now-base` ≤ 下限 → 跳过。只留 ① 是**不对称保护**——`base` 亚毫秒作分母时，`now` 只要刚过下限（0.5 → 1.4ms）相对百分比即被放大成 +180% 假退化，**pre-push 与 vitest 并跑时 `go/cli` 整包必 FAIL**（实测报「7 个阶段相对基准退化超过 50%」，而该测试单跑 3/3 PASS——典型负载 flaky）。真实退化不受影响（0.001 → 50ms 增量 49.999ms，仍被拦）。测试锚点：`bench_concurrent_test.go` 的 `TestRunSingleBenchJSON_SaveAndCompareRoundTrip` 同时守住两侧（亚毫秒+小增量不报 / 0.001→50ms 必报），改判定逻辑别把任一侧断言删掉
