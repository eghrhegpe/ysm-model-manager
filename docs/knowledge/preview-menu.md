...
- **`preview-3d/caps/scene-capability.ts`** — cap 生态：`controls` 节点直持 `PreviewControlDef[]`（增量2a 起复杂件专用：button/image/timeline/histogram/preset-thumb），渲染委托 `renderCapControls`（复杂件唯一渲染器；简单件走节点原生 kind + `renderCap*` 直渲）。
