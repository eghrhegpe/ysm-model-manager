...
- **`PreviewMenuNode`**（node-types.ts）— 声明式菜单节点类型契约（纯类型叶，零运行时依赖）；`PreviewMenuNodeKind` 16 种节点类型；`PreviewControlSpec` 控件绑定规格。folder 节点支持 `headerToggle`（对齐 MikuMikuAR PopupRow.headerToggle）：「功能=本 folder」时能力总开关放 folder header 一眼可见、免展开即切换；`createHeaderToggle` 内置 stopPropagation，开关点击不触发折叠；声明后 folder body 不得再含同源开关。env.ts `envCapFolder` 据此把 cap 自报的 `getMasterToggle`（SceneCapability 可选接口，fog/environment/reflector 实现）升为 folder header，并从 body 剔除同源控件（`restControls` 先剔再分区，base 组剔空则不产生空组 folder）。
