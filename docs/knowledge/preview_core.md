...
- **`SharedInfra` 窄出口（2026-09-22 死字段清算）**：`buildSharedInfra` 带出的 cap 引用只限「装配现场之外仍有消费者」的（lightCap/shadowCap/environmentCap + postProc），曾带 skyCap/groundCap/waterCap/fogCap/reflectorCap/postProcCap 六枚只写不读字段已删。cap 间协作的**唯一合法通道**是构造注入 `ctx.caps` 查询器 / `getTypedCap(registry,…)` 现场取（ADR-168/216）——往 infra 出口加 cap 字段即造第二条暗线，勿为省一次查询器就加字段。
