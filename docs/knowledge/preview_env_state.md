...
- **saveState**（写）：`persistState(capId, { this.enabled(能力级私有) + envState 参数字段 })` —— 从 envState **摘键**，不存全量。
  ...
  （**能力级开关收口后的键形**：fog/water/shadow/reflector 已删私有 `enabled`，`saveState` **只写 schema 键**（含 `{fog,water,shadow,reflector}Enabled`），不再落无前缀 `enabled` 幽灵键；旧存档的 `enabled` 由 `loadState` 回填进对应 schema 键。仍留旧键形的：ground/environment（见「不变量」末条）。）
...
2. **恢复语义丢失**——envState 全量恢复经 `setEnvState` 只能写参数，写不了能力级 `this.enabled`（`isStateLoaded` 置位均 cap 独占；fog/water/shadow/**reflector** 私有 enabled 已收编为 schema 键，其开关即 schema 字段 `{fog,water,shadow,reflector}Enabled`；environment/ground 的私有态除外）。
...
- 能力级 enabled 原则上不入 schema（是否挂载是装配态）；**例外**：fog（首例）、water（2026-09-22 跟进）、shadow（2026-09-22 锐评 F-1 并入）、reflector（同日 F-1 二度收口并入）已把「能力启停」并入 schema 单门——`setEnabled/isEnabled` 收敛为 `envState.{fog,water,shadow,reflector}Enabled` 别名，`SceneCapability` 接口不变。运行时态（customHdrTex/currentPreset/manualPreset）仍留 cap 私有。
  （原 `volumetricEngine` 运行时态已随 ADR-246 D1 删除——postprocess 空壳引擎移除。）
  （**并入判据 = 私有默认值与 schema 默认值是否一致 + 冲突时哪方有佐证**：shadow 两侧同默认 `true`，直接并；reflector 两侧**相反**（私有 `true` / schema `false`），但 schema 侧三重佐证（注释/邻座援引/不变式测试）而私有侧零佐证，故以 schema 为准并入——**「默认值相反」不是不能并，而是并之前必须先判定谁是对的**。排查同族时务必比对两侧默认值，不要只看「有没有 UI 写口」。）
...
- **[同批复核] 同族病灶余项（2026-09-22 扫描结论；含 F-1 二度收口后更新）**：`sky`/`environment`/`ground` 仍留私有 `this.enabled`（`light` 经本轮核实**已收口**，见末条），**逐个核实后严重度不同**——
  ...
  - **`sky` = 真私有门且**已有 UI 写口**，是与 shadow 同级的**最高危余项**（未修，登记待办）**：`sky-capability.ts` 声明 `private enabled: boolean`（构造 `opts.enabled ?? true`，registry 不传 → 恒 `true`），`setEnabled/isEnabled` 直读写它，`sky-menu.ts|skyEnabledNode` 的 toggle `get/set` 即绑这对方法——**写口存在**（这点与 shadow 同、与 ground 不同）。同时 `saveState` 落**无前缀** `enabled: this.enabled`、`loadState` 用 `enabled.boolean((v) => { this.enabled = v; })` 把它读回**私有门**（不写任何 schema 键）——即 shadow 收口前的**双键形态原样保留**。且 schema 中**不存在 `skyEnabled` 键**（sky 组 14 键全为参数，无能力开关）。
    - **为何判「高危」而非已发作**：与 shadow 不同的是**它的私有门有真写口**，`saveState`/`loadState` 又双轨自洽，故当前**用户可见行为正确**（开关读写一致、存档往返一致）。但它缺的是**单门收口的其余三项保障**：①`loadState` 走 `manual` 时代遗留的私有门回填路径（非 schema 键，故 `_writeSource` 纪律管不到它，`isStateLoaded` 之外的守卫无从施加）；②`getParams()` 与 UI 之间仍是私有态直读，任何「只写 envState 的旁路」（如 `applyModelPreset`/氛围快照/未来程序化调用）都**无法**改变天空开关——**这正是 shadow 病灶的同型结构**，只是当时代码恰好没引入第三条写路。③无 schema 值域钳制（布尔无值域，风险低）。
    - **收口前置条件（已核实，结论：可收，但收益是「防将来」而非「治现在」）**：①sky **不在 `MODEL_DEFAULTS`**（ADR-284 大气与类别解耦）→ 无同轨模型写对手；②**`ATMOSPHERE_PRESETS` 不携任何天空能力开关键**（五档只有 `skyTimeOfDay`/`skyCloudCoverage`/`skyForceEnv`，已逐档核实）→ 氛围预设**不会**也无法拨动天空开关，故收口后氛围链路行为不变；③结论：sky 与 shadow **同型**（有 UI 写口 + `saveState` 落无前缀 `enabled` 幽灵键 + `loadState` 回填私有门），但**尚未发作**——因为它恰好没有第三条写路，且存档双轨自洽。**它是本族最后一个「结构上已具备病灶、只差一次改动就会发作」的余项**，值得优先收口（改动面比 reflector 小：无默认值冲突、无邻座联动）。
  - **`light` = 已收口（本轮核实补记）**：`light-capability.ts` 用 **getter 别名**形态收口——`private get enabled(): boolean { return envState.lightEnabled; }`（不持字段，注释明言「本 cap 不再持有 enabled 字段」），`opts.enabled` 若传入则转写 `setEnvState({lightEnabled})`。故 light **不应**再列入「仍留私有 `this.enabled`」名单——上文 water 收口条末尾那句「sky/ground/shadow/light/reflector/environment 仍留私有」在 light 项上已是化石。
