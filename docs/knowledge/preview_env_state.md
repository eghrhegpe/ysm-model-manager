...
- **saveState**（写）：`persistState(capId, { this.enabled(能力级私有) + envState 参数字段 })` —— 从 envState **摘键**，不存全量。
  ...
  （**能力级开关收口后的键形**：fog/water/shadow 已删私有 `enabled`，`saveState` **只写 schema 键**（含 `{fog,water,shadow}Enabled`），不再落无前缀 `enabled` 幽灵键；旧存档的 `enabled` 由 `loadState` 回填进对应 schema 键。仍留旧键形的：ground/reflector/environment（见「不变量」末条）。）
...
2. **恢复语义丢失**——envState 全量恢复经 `setEnvState` 只能写参数，写不了能力级 `this.enabled`（`isStateLoaded` 置位均 cap 独占；fog/water/shadow 私有 enabled 已收编为 schema 键，其开关即 schema 字段 `{fog,water,shadow}Enabled`；reflector/environment 的私有态除外）。
...
- 能力级 enabled 原则上不入 schema（是否挂载是装配态）；**例外**：fog（首例）、water（2026-09-22 跟进）、shadow（2026-09-22 锐评 F-1 并入）已把「能力启停」并入 schema 单门——`setEnabled/isEnabled` 收敛为 `envState.{fog,water,shadow}Enabled` 别名，`SceneCapability` 接口不变。运行时态（customHdrTex/currentPreset/manualPreset）仍留 cap 私有。
  （原 `volumetricEngine` 运行时态已随 ADR-246 D1 删除——postprocess 空壳引擎移除。）
  （**并列例外须逐案拍板语义**：shadow 能并入是因为其 schema 键 `shadowEnabled` 默认 `true` 与私有门默认一致；`reflectorEnabled` 默认 `false` 而私有门默认 `true`，合一会改变用户可见默认态，故 reflector 的私有门**仍在**，待单独定夺——见下方同批复核。）
...
- **[同批复核] 同族病灶余项（2026-09-22 扫描结论）**：`sky`/`environment`/`ground`/`reflector` 仍留私有 `this.enabled`，**逐个核实后严重度不同**——
  - **`reflector` = shadow 的活体孪生（唯一真·同级病灶，待办）**：`buildReflector` 是**双门** `if (!this.enabled || !envState.reflectorEnabled) return;`，`getParams().enabled` 返回 `this.enabled && envState.reflectorEnabled`。今日症状被掩盖，只因 `saveState` 把两键都落盘、`loadState` 把两键都恢复 → 二者恒同步；但**只要存档缺 `enabled` 键（旧档/手改档/未来只写 schema 键的档），私有门保持构造默认 `true` 而 `reflectorEnabled` 已恢复**，菜单开关就会显示 OFF 而 schema 键是 ON。收口须先拍板**语义**：`reflectorEnabled` 默认 `false` 而私有门默认 `true`（与 shadow 相反），合一是取「默认关」还是「默认开」，改的是用户可见默认态，故未随 F-1 一并动手。
  ...
  - **`environment` = 同类但开关本就私有语义**：`getMasterNodeId` 返回 `"env-enabled"`，开关明确设计为不入 envState（`environment-capability.ts` 注释「能力总开关（不入 envState…）」）——ADR-196「刀5」段所记「能力级 enabled 不入 schema 红线」的原教旨形态，非漏网。（该红线与 fog/water/**shadow** 的并入例外并存，判据见上条「并列例外须逐案拍板语义」。）
