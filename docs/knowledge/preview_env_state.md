...
- **[同批复核] 同族病灶余项（2026-09-22 扫描结论；含 F-1 二度收口后更新）**：`sky`/`environment`/`ground` 仍留私有 `this.enabled`，**逐个核实后严重度不同**——
  ...
  - **`environment` = 同类但开关本就私有语义**：`getMasterNodeId` 返回 `"env-enabled"`，开关明确设计为不入 envState（`environment-capability.ts` 注释「能力总开关（不入 envState…）」）——ADR-196「刀5」段所记「能力级 enabled 不入 schema 红线」的原教旨形态，非漏网。（该红线与 fog/water/**shadow**/**reflector** 的并入例外并存，判据见上条「并列例外须逐案拍板语义」。）
