...
- **`skyForceEnv` 是活键，不是死字段（ADR-292 D8 更正）**：它承载**天空内部的**阈值门控意图——`false` 时按 `PMREM_ELEVATION_THRESHOLD` 判断是否需要重烤（防昼夜循环每帧全量 PMREM 的 GPU 熔炉），`true` 时无条件重烤。收口后它**不再承担跨 cap 契约**：重建请求改由 env 侧直接调用 `bakeEnvironmentTexture({ force })` 表达，sky 的 `requestEnvironmentRefresh()` 判定「自己装载 or 转交 env」。旧卡曾记为「死字段」（依据是五 cap 挑参表均不读它）——那是**只看挑参表的采样偏差**，真实读点在 sky callback 的 `maybeRegenerateEnvironment` 与云量重建分支。
