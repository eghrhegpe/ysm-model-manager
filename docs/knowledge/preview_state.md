...
setStateValue(path, v, { notify?: false }) // 写（v 类型 = PathInput<path> = PathValue[path] ∪ number|string|boolean；滑块高频传 notify:false）
isPathAvailable(path)                     // cap 派生项在 cap 缺席时 false
previewSnapshot()                         // { [K in PreviewStatePath]: PathValue[K] }（全键有值、无黑洞键；旧 Record<…,unknown> 的结构超集），供 visibleWhen 纯函数谓词消费
