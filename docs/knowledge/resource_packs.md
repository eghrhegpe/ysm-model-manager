...
`resource-packs.ts` 是一个薄 wrapper：把仓库页的各类资源包 tab（资源包/光影包/蓝图/MMD/VRC/投影）统一委托给 `<app-resource-manager>` 组件渲染。文件本身不含业务逻辑，仅负责动态导入并挂载组件、以 `rtype` 属性区分资源类型，是「tab 页 → 通用资源管理组件」的适配层。

## 核心职责

- `initResourcePacks(container, host, rtype?)`：`await import("../components/app-resource-manager/index.js")` 确保组件已注册，随后向容器写入 `<app-resource-manager rtype="...">`
...
- 依赖常量：`RESOURCE_TYPES`（frontend/js/utils/resource-types.ts）

## 与其他子系统关系

- 由 [app_content](./app_content.md) 的 `_bindTabs` 在首次切到对应 tab 时懒加载调用，返回的 cleanup 收进 `_unsubs`
