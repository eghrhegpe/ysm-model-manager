...
| geometry、基岩版、bedrock、模型解析、zip、7z、纹理、动画 | [Geometry 存档 go/geometry](./go-geometry.md) ⚠️歧义（另见 go-container.md等） | `go/geometry/` 包解析 Bedrock（基岩版）`minecraft:geometry` 模型：既支持单个 geometry JSON，也支持从 ZIP/7z 存档中按 `ysm.json` 清单合并多个模型文件、提取纹理与动… |
