...
- **采用率全景口径 = scripts/ 侧**（分母为脚本数，不含 `_lib` 内部互引）。扫描面已含 `_lib`，但统计不并入分母——否则「被引用脚本数 / 脚本总数」的百分比被稀释，读数漂移。
- **豁免精确到文件**：`selfFile = "_lib/" + rule.lib` 全等判定，不做目录级 / 前缀级放行。
- **判定不依赖文案**：采用特征锚定**真实 import 语句**（行首 `import` + 引号路径），而非文本中出现模块名——否则 RULES 里的 advice 字符串、模块头部注释提及自身名都会被误判为「已接入」。
