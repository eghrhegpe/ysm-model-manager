...
- **检查 6（跨层存在性）**（2026-09，ADR-275）：shadow 域模板用到的类，若在「所有 shadow 域 CSS ∪ document 层 `frontend/css/*.css`」都无定义 → WARN（豁免走既有 `KNOWN_NO_CSS_CLASSES` 单点，无第二处白名单）。它补的是检查 3 的**对偶盲区**：检查 3 问「本域命名空间里用了却没定义」，它问「**哪儿都没定义**」。首跑 9 条 / **0 假阳性**，把 ADR-274 写明的 42 类盲区**全部归因收口**——30 类 document 层对话框模板（边界正确）、**7 类 `lt-*` 真缺陷**（`litematic-meta.ts` 的色块是空 span 无尺寸 ⇒ 恒不可见，已补 `app-preview/css.ts` 真规则）、9 类豁免（`heatmap-bar-*` 靠内联容器+bar 的 height/color 承载、`br-preset`/`br-file-cb` 是共类+JS 钩子、`gray` 是设计缺口）。**对偶检查（定义了没人用）已实测否决**（naive 版 617 条 / 真死 0，绝大多数是 `classList` 运行时类），别重走。同批删 `.hm-*` 13 条死 CSS + 孤儿令牌 `--hm-0..4`（全仓 src 与 e2e 零消费者，与 `.heatmap-bar-*` 构成错名孪生体）。
