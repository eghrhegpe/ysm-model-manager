...
- `stubLogWarn()` / `stubConsoleWarn(impl?)` / `stubConsoleError(impl?)`（`mock-log.ts`）— 项目告警通道 `logWarn` 与 `console.warn/error` 的 spy 工厂（mock 复印机收敛，对齐 `stubBlobUrls` 范式；收敛前 16 个文件各自手写同构 spy）。返回 spy 供 `toHaveBeenCalled*` 断言；console 两枚默认静默实现屏蔽噪声、可传自定义 impl。清理由调用方 `afterEach` 的 `vi.restoreAllMocks()` 统一承担（工厂不接管 restore）
