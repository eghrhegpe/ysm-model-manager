...
- `summaryCardHTML(summary, header, basename?): string` — 主入口：双空 → 空态引导卡（「点击左侧仓库文件查看详情」）；无 summary 且 header.isYsm → 头部简约卡（🔒 加密提示 + 格式/加密版本号）；否则 → 完整卡片（免费/付费徽章由 hasFree/isFree 驱动；图标走 SVG——免费=`UI_ICONS.tag`、付费=`UI_ICONS.lock`，2026-09 收 ADR-238 债，原 `🆓` emoji 退役）。**方案 A 去重（2026-08-28）**：完整卡片不再渲染作者行（含文件名 `[作者]` 回退）与纹理尺寸行——作者（头像+角色）与纹理由统计卡 `buildStatsCard`（skeleton-render.ts）统一承载并挂详情卡底部 `#preview-stats`，摘要卡保留唯一性，消除信息重复。**资源行删除（2026-08-28）**：「📦 贴图/模型/动画」行整体移除——贴图/模型计数（Go `extractFileStats` 只数 `files.player.*` 清单条目）与统计卡实际文件数口径不一致且重叠；动画计数并入动画分组标题（`🎬 ${name}（${items.length}）`，如「其他动画（7）」）。**顶部标题去重（2026-08-28）**：`model-detail-title`（📄 模型详情）两处（header-only 卡 + 完整卡）移除——外层 tab 已表达「📄 详情」层级，卡内不再重复标题；统计卡 `pv-card-title` 的「📊 模型概览」文字同步移除（该类已无渲染点，仅 CSS 规则残留）。**解码器徽标归位（2026-09-18）**：`ysm-badge` 一度挂在 `summaryCardHTML` 的 `<h3>` 标题行（与免费/付费徽标挤同一行，且 `margin-left:auto` 在非 flex 的 `h3` 上失效），现改由 `statsCardHTML` 的「文件信息」橙卡渲染——与 `.ysm`/`.json` 格式信息同展，标题行只留许可徽章。徽标值 = `_decodedBy` **来源码**（`utils.ts` 的 `DECODE_SOURCE`），由**解码器盖章**：`wasm-decode.ts` 在 `cacheSet` 前写 `result._decodedBy`，故 `index.ts` 纹理预览等旁路写缓存也随对象传播（此前不盖章 → 缓存命中丢标记，`.ysm` 徽标不显示的根因）；展示层映射图标（`parser`/`package`）+ i18n（`preview.decodedBy.*`），未识别的码不渲染。**幽灵空行修复（2026-08-28）**：`configHtml` 内部原自带 `<div class="md-divider">`，与渲染处 `animGroupHtml` 的前导 divider 同时存在时连续两个 divider 产生幽灵空行——统一为「divider 只在渲染处按区块存在性加一次」（scale/animGroup/config 各自 `?` 前导），区块内容自身不再带
...
- 徽章/卡片颜色全走 CSS 变量（--accent / --status-success / --status-error / --surf / --txt / --muted）
