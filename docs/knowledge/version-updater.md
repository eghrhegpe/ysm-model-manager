...
- 频次限制：`CHECK_KEY = "ysm_lastUpdateCheck"` + `CHECK_INTERVAL_DEFAULT_MS = 6h`（已 export，跨域副本须与 views|settings-schema|UPDATE_CHECK_DEFAULT 保持相等；`version-updater.test.ts`「缺省回退间隔 = 6h 数值语义」断言 21600000 + 频次边界行为护栏，2026-10 子代理审核 P1-1 收口），`canCheck()`/`markChecked()` 基于 localStorage 实现
