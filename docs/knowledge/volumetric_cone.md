...
pitfalls:
  ...
  - 重建成本：只有 enabled/angle/penumbra 影响几何；color/intensity/distance/decay 走 updateUniforms，误加回「任意 spotlight 变更即 rebuild」会恢复拖滑块抖动
quick_groups:
  - preview-3d
quick_intents:
  - 改体积光外观 / 加锥体参数
...
quick_risk_lines:
  ...
  - frontend/src/preview-3d/caps/light-capability.ts|SPOT_GEO_CHANGES
invariant_anchors:
...
- 按 `SpotlightParams`（angle/penumbra/enabled/color）+ `VolumetricParams`（opacity/fogPower/edgeFade/baseStrength/tipStrength）产出可见锥体。
...
| `syncPosition(spotlightPos, spotlightDir?)` | 只同步位置/朝向（`setTarget` 路径）；缺省沿用上次方向 |
...
- **`LightCapability`**：拥有聚光灯与靶点，计算射束方向 `getSpotDir()`（光源 → 靶点），并决定何时重建（`SPOT_GEO_CHANGES = {lightSpotEnabled, lightSpotAngle, lightSpotPenumbra}`）与何时走 uniforms 快路径。
- **envState / env-dispatcher**：参数变更经 `setEnvState` 派发，`onEnvChanged` 分派到锥体。
- **`SpotLightHelper`**（ADR-246 D3）：线框锥是聚光灯的角度参照；体积光锥是视觉光柱本体，二者同开关显隐。
...
3. **未双开不产锥**：`spotlight.enabled && volumetric.enabled` 同时为真才产出 group（挂载态另有 `attach/detach` 语义，重建后需回挂）。
...
5. **重建触发面**：非几何字段（color/intensity/distance/decay）只走 `updateUniforms`；同批 spot+vol 变更只重建一次。
