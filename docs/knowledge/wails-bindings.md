...
- **反射注册机制与「不成为 binding」的唯一正解**（2026-09 实证，Wails v3 alpha2.105）：`application.NewService(appStruct)` 经 `bindings.go|getMethods()` 反射**注册 `*App` 的所有导出方法**——**无任何签名合法性校验**。故在 `*App` 上加一个导出方法 = 自动成为前端可调用 binding。三条推论：
  1. **唯一按名排除**（运行时 + codegen 双生效）是 `ServiceName`/`ServiceStartup`/`ServiceShutdown`/`ServeHTTP` 四个固定名；`//wails:ignore` **只管 codegen 不管运行时**（照样注册，前端可调 → `binding-check` 报 `missing_in_js`）；`//wails:internal` 生成但不 `export`，运行时**仍绑定**。后两者不可用作排除手段。
  2. **带不可序列化参数的方法是「假 binding」**：如 `func (a *App) X(w http.ResponseWriter, ...)` 会注册成功、codegen 只打 Warning 并照常生成包装；JS 传 null → 接口为 nil → 处理器 panic → 被转成 422。**既非报错退出也非静默跳过，是一个永久坏死的入口。**
  3. **需要「非 binding 的能力」时用自由函数**，不要加 `*App` 方法：`scripts/binding-check.ts` 只扫 `func (\w+ \*?App) X(`，自由函数（如 `func NewXxxHandler(a *App) http.Handler`，守卫仍走包内私有方法）**完全不被注册也不被扫**——这是「既复用包内私有守卫、又不污染 binding 面」的落点。**改门禁白名单来放行是下策**（`binding-check` 是 pre-push 硬闸，动它本身会被治理审查盯上）。
  - 基线核对：`node scripts/binding-check.ts` 应报告 `go_functions == js_functions` 且 `issues: 0`；新方法数须以生成 `frontend/bindings/` 的 `app.ts` 为权威。
