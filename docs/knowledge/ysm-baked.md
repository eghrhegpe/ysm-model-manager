...
1. **游戏模组（OpenYSM / ModernYSM 等）**：`YSMBinaryDeserializer` 直读烘焙字节流 → `RawYsmModel`。二进制里 **bone 层的 pivot/rotation 仍然保留**（`YSMBinaryDeserializer.java` bone 解码段），cube 层只剩 `RawFace`（`RawYsmModel.java`）；渲染直接消费两者（`YSMClientMapper.java` → `NativeModelRenderer`），**不经反推**，所以游戏内模型外观总是正确的；
...
- `RawYsmModel.RawCube.faces`：**cube 的几何**被烘焙成顶点面，每面存 4 顶点 xyz + 1 法线 + 4 组 float u/v，读取见 `YSMBinaryDeserializer.java` cube 解码段。**bone 的 pivot/rotation 并未丢失**——`YSMBinaryDeserializer.java` bone 解码段保留，渲染端真实使用（`YSMClientMapper.java` 塞进 GeoBone，矩阵应用见 `NativeModelRenderer.java`）。
...
- 上游参照：`upstream/ModernYSM-1.20.1-forge`（开源版）二进制反序列化 + 渲染直接吃烘焙数据，能力对标 YSMViewer（**该 upstream 目录未纳入本仓库**，`YSMBinaryDeserializer.java` / `RawYsmModel.java` 仅作类名级外部参考，不在本卡 source_files 中）
