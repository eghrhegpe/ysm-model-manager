...
1. **章首标题**：vol-4+ 用 `# 第 N 区域 · 第 M 章 · 标题`；创业三部曲用 `# 第 X 幕 · 第 M 章 · 标题`（格式见 SKELETON 第七节）
2. **真实事件/代码改动标注**：`> 对应真实事件/代码改动：xxx`
3. **叙事结构**：四段式（惊醒→追查→真相→教训）+ 章末「边界悖论」隐喻 + `---` 教训摘要（≤20 字），硬约束见 [SKELETON.md 第七节](SKELETON.md)
...
1. 先读 `SKELETON.md`（世界观/角色/冲突）与 `AGENTS.md`（区域总表/决策链路），再读对应区域已有章节，最后动笔
