...
| 包 | 用途 |
|----|------|
| `avatar/` | 创作者头像提取与缓存 〔源码 4: avatar.go avatar_decode.go avatar_extract.go avatar_zip.go · 测试 6〕 |
...
| 包 | 用途 |
|----|------|
| `app/` | Wails Binding 入口（app.go / resource_bindings.go） 〔源码 41 · 测试 24〕 |
...
| `utils/` | 工具函数（display / fmt / dom / icon / summarize / model3d） 〔源码 6: array.ts gh-links.ts main-thread-watch.ts module-loader.ts safe-error-msg.ts types-re-export.ts · 测试 4 · 子目录 10: 3d/ animation/ async/ cache/ core/ debug/ dom/ format/ icon/ resource/〕 |
