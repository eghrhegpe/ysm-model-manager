...
| 🟡 中 P3 | [index.ts#L22-L34](../frontend/src/views/app-preview/index.ts#L22-L34) | **evict 回调不回收头像 blob URL**：decoder/wasm-decode.ts 为作者头像 `URL.createObjectURL`（[wasm-decode.ts#L80-L83](../frontend/src/features/preview-3d/decoder/wasm-decode.ts#L80-L83)）存入 `authors[].avatarUrl` / `avatars`，淘汰回调只释放 `geometry.textures` / `texture`，缓存流转 50 条后头像 blob 持续累积 | evict 回调补扫 `val.authors[].avatarUrl` 与 `val.avatars` 中的 `blob:` 前缀并 revoke |
...
| 🟢 低 P4 | [wasm-decode.ts#L110-L127](../frontend/src/features/preview-3d/decoder/wasm-decode.ts#L110-L127) | 同文件已有 `devLog`，却残留 3 处裸 `console.log`（生产环境仍输出） | 换 `devLog` 或删除（按约定需先请示） |
