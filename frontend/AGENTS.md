...
- **R5 / R6（2026-09 补强，`check-path-hygiene`）**：
  - **精确同目录**禁用别名，写相对 `./xxx`（R5）；跨顶层 **或** 同顶层内不同子目录，一律 `@/`——别手算 `../` 深度。
  - **测试文件禁 import 桶入口**（裸 `@/目录聚口` 或以 `/index` 结尾），必须引具体叶（R6），防「一根测试拉起一整个模块」。文件级别名 `@/bus`/`@/theme-core` 是具体叶，不触发。
- **迁移策略（2026-09-07 已全量收敛）**：相对深度 `../`/`../../` 已全仓归零。**非精确同目录**写 `@/<顶层>/具体文件`，**仅精确同目录**写 `./`；src 根文件用 `@/bus`/`@/theme-core`；越 `src` 界（bindings/根 JSON）保持相对或 `#root`。