...
- **R5 / R6（2026-09 补强，`check-path-hygiene`）**：
  - 同目录内**禁用别名**，写相对 `./xxx`（R5）——别手算 `../` 深度；跨目录才用 `@/`。
  - **测试文件禁 import 桶入口**（裸 `@/dir` 或以 `/index` 结尾），必须引具体叶（R6），防「一根测试拉起一整个模块」。
- **迁移策略（2026-09-07 已全量收敛）**：跨顶层相对引用已全部切为 `@/`；**跨顶层**写 `@/<顶层>/具体文件`，**同顶层目录内**写相对 `./`/`../`，src 根文件用 `@/bus`/`@/theme-core`；越 `src` 界（bindings/根 JSON）保持相对或 `#root`。