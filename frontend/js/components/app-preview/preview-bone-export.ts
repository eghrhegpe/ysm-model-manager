// ===== 骨骼名导出 =====
// 从 index.ts _loadModel2D 拆分：导出骨骼名按钮
import type { BedrockGeometry } from "./utils.ts";

/**
 * 在容器底部添加 "📋 导出骨骼名" 按钮
 * @param container - 父容器
 * @param model - BedrockModel 对象（含 boneCount, cubeCount, texWidth, texHeight, bones[]）
 * @param modelPath - 模型文件路径（用于生成文件名）
 */
export function setupBoneExport(
