...
export async function showCreatorManager(sites) {
  return new Promise((resolve) => {
    ...
    box.style.cssText = "background:var(--surf);border:1px solid var(--bd);border-radius:10px;padding:16px;width:580px;max-height:80vh;box-shadow:0 8px 24px rgba(0,0,0,.5);display:flex;flex-direction:column;gap:6px;overflow:hidden";
    ...
    // 平台标签颜色
    const TAG_COLORS = {
      bilibili: '#fb7299', afdian: '#ff6600', github: '#6cc644',
      mcmod: '#7c83ff', curseforge: '#f16436', modrinth: '#1bd96a'
    };
    const tagColor = (t) => TAG_COLORS[t] || 'var(--accent)';

    const loadData = async () => {
      try {
        const { LoadWorkshopCreators } = await import("../../wailsjs/go/main/App.js");
    ...
    // 渲染智能标签
    const renderTags = () => {
      const allTypes = [...new Set(data.flatMap(cr => (cr.type||'').split(';').filter(Boolean)))];
      if (!allTypes.length) { tagsEl.innerHTML = ''; return; }
      tagsEl.innerHTML = '<span style="font-size:9px;color:var(--muted);align-self:center">🏷️ </span>' +
        allTypes.map(t => '<span class="cm-tag" data-tag="' + esc(t) + '" style="display:inline-block;padding:1px 8px;border-radius:10px;font-size:10px;cursor:pointer;background:' + tagColor(t) + '22;color:' + tagColor(t) + ';border:1px solid ' + tagColor(t) + '44' + (tagFilter === t ? ';box-shadow:0 0 0 2px ' + tagColor(t) + '66' : '') + '">' + esc(t) + '</span>'
      ).join('');
    ...
    const renderList = () => {
      ...
      let filtered = data;
      if (tagFilter) filtered = filtered.filter(cr => (cr.type||'').split(';').includes(tagFilter));
      if (kw) filtered = filtered.filter(cr => cr.name.toLowerCase().includes(kw) || (cr.type||'').includes(kw));
      countEl.textContent = data.length;
      if (!filtered.length) {
        listEl.innerHTML = '<div style="color:var(--muted);font-size:10px;padding:12px;text-align:center">' + (kw||tagFilter ? '无匹配' : '暂无创作者') + '</div>';
        return;
      }
      listEl.innerHTML = filtered.map(cr => {
        const i = data.indexOf(cr);
        const types = (cr.type||'').split(';').filter(Boolean);
        const tags = types.map(t => '<span style="display:inline-block;padding:0 5px;border-radius:8px;font-size:9px;background:' + tagColor(t) + '22;color:' + tagColor(t) + ';margin-right:2px">' + esc(t) + '</span>').join('');
        const hasSearch = sites && sites.some(s => types.includes(s.id) && s.searchUrl);
        return '<div style="display:flex;align-items:center;gap:4px;padding:4px 6px;border-radius:4px;border:1px solid var(--bd);font-size:11px">' +
          '<span>🎨</span>' +
          '<input class="cm-fld" data-idx="' + i + '" data-fld="name" value="' + esc(cr.name) + '" style="flex:1;min-width:40px;padding:2px 4px;border-radius:3px;border:1px solid transparent;background:transparent;color:var(--txt);font-size:11px">' +
          '<input class="cm-fld" data-idx="' + i + '" data-fld="desc" value="' + esc(cr.desc||'') + '" style="flex:1;min-width:40px;padding:2px 4px;border-radius:3px;border:1px solid transparent;background:transparent;color:var(--muted);font-size:10px">' +
          '<span style="flex-shrink:0">' + tags + '</span>' +
          (hasSearch ? '<button class="cm-search" data-idx="' + i + '" style="padding:1px 5px;border-radius:3px;border:1px solid transparent;background:transparent;color:var(--accent);cursor:pointer;font-size:11px" title="查看模型">🔍</button>' : '') +
          '<button class="cm-del" data-idx="' + i + '" style="padding:1px 5px;border-radius:3px;border:1px solid transparent;background:transparent;color:#e5534b;cursor:pointer;font-size:10px">🗑️</button></div>';
      }).join('');

      listEl.querySelectorAll('.cm-fld').forEach(inp => {
        inp.addEventListener('focus', () => { inp.style.borderColor = 'var(--bd)'; inp.style.background = 'var(--surf)'; });
        ...
        btn.addEventListener('click', () => {
          ...
          for (const s of sites) {
            if (types.includes(s.id) && s.searchUrl) {
              window.open(s.searchUrl.replace(/\{\{q\}\}/g, encodeURIComponent(cr.name)), '_blank');
              return;
            }
          }
        });
      ...
      listEl.querySelectorAll('.cm-del').forEach(btn => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.dataset.idx, 10);
          if (data[idx]) { data.splice(idx, 1); renderList(); renderTags(); }
        });
      });
    };
    ...
    const showQuickAdd = () => {
      ...
      modal.innerHTML = '<div style="background:var(--surf);border:1px solid var(--bd);border-radius:8px;padding:14px;width:320px;display:flex;flex-direction:column;gap:6px">' +
      ...
      modal.querySelector('#qa-save').addEventListener('click', () => {
        ...
        if (!name) { alert('请输入作者名'); return; }
        data.push({ name, type, desc: modal.querySelector('#qa-desc').value.trim() || '' });
        modal.remove();
        renderTags();
        renderList();
        // 滚动到底部
      ...
      // 回车保存
      modal.querySelector('#qa-name').addEventListener('keydown', (e) => { if (e.key === 'Enter') modal.querySelector('#qa-save').click(); });
    ...
    box.querySelector('.cm-export').addEventListener('click', async () => {
      try {
        const { SaveWorkshopCreators, ExportWorkshopCreatorsJSONFile } = await import('../../wailsjs/go/main/App.js');
    ...
    box.querySelector('.cm-merge').addEventListener('click', async () => {
      ...
      inp.onchange = async (e) => {
        ...
        try {
          ...
          const { MergeWorkshopCreatorsFromJSON } = await import('../../wailsjs/go/main/App.js');
    ...
    box.querySelector('.cm-replace').addEventListener('click', async () => {
      if (!await window.showConfirm?.('⚠️ 覆盖导入将完全替换！已自动备份。确定？')) return;
      const inp = document.createElement('input'); inp.type = 'file'; inp.accept = '.json';
      inp.onchange = async (e) => {
        const file = e.target.files?.[0]; if (!file) return;
        try {
          const text = await file.text();
          const { ReplaceWorkshopCreatorsFromJSON } = await import('../../wailsjs/go/main/App.js');
          alert('✅ 已导入 ' + (await ReplaceWorkshopCreatorsFromJSON(text)) + ' 个（备份已创建）');
          await loadData();
        } catch (err) { alert('❌ 导入失败: ' + err); }
      }; inp.click();
    });
    ...
    box.querySelector('.cm-cancel').addEventListener('click', () => { overlay.remove(); resolve(null); });
    loadData();
  });
}
