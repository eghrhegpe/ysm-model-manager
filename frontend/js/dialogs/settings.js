...
async function openSettingsDialog() {
    ...
    dialog.style.cssText = 'width:320px;background:var(--surf);border:1px solid var(--bd);border-radius:10px;padding:16px;display:flex;flex-direction:column;gap:12px';
    dialog.innerHTML = '<div style="font-size:13px;font-weight:600;color:var(--txt)">⚙️ 设置</div>' +
        ...
        '<option value="copy"' + (mode === 'copy' ? ' selected' : '') + '>📄 复制（占用高）</option>' +
        '<option value="hardlink"' + (mode === 'hardlink' ? ' selected' : '') + '>🔗 硬链接（省空间）</option>' +
        '<option value="symlink"' + (mode === 'symlink' ? ' selected' : '') + '>🔗 符号链接（灵活）</option>' +
        '</select></div>' +
    ...
    dialog.querySelector('#settings-link-mode').onchange = async (e) => {
        ...
        st.textContent = '✅ 已切换为' + (e.target.value === 'copy' ? '复制模式' : e.target.value === 'hardlink' ? '硬链接模式' : '符号链接模式');
        await saveConfig();
    };
}
