...
function showFolderMenu(ti) {
    const items = [
        ...
        { label: "📁 新建文件夹", action: async () => {
            ...
            // 推算当前文件夹在仓库下的相对路径
            const firstFile = ti.nextElementSibling?.querySelector(".ti[data-path]");
            let relDir = "";
            if (firstFile) {
...
function showFileMenu(ti) {
    ...
    const matched = matchedPath ? entries.find(e => e.Path === matchedPath) : null;
    if (!matched) {
        contextMenu.innerHTML = "<div style=\"padding:6px 12px;font-size:10px;color:var(--muted)\">未找到文件信息</div>";
        return;
    }
    const items = [];
    // 第一项：解析名称（点即复制文件名）
    items.push({ label: "📋 复制文件名称", action: () => copyToClipboard(matched.Name) });
    ...
    items.push({
        ...
        action: async () => {
            try {
                const newState = await window.go.main.App.ToggleModelEnable(matched.Path);
                showToast(newState ? "✅ 已启用: " + matched.Name : "⛔ 已禁用: " + matched.Name);
                entries = await window.go.main.App.ScanModelEntries(repoRoot);
                buildTree();
    ...
    // 哈希信息
    if (matched.Hash) {
        const duplicates = entries.filter(e => e.Hash === matched.Hash && e.Path !== matched.Path);
        if (duplicates.length > 0) {
            items.push({ label: "🔗 发现 " + duplicates.length + " 个重复文件", action: () => {} });
        } else {
    ...
    // 文件操作
    items.push(
        { label: "🔑 复制文件哈希", action: () => copyToClipboard(matched.Hash || "无哈希") },
        { label: "📂 打开所在文件夹", action: () => {
