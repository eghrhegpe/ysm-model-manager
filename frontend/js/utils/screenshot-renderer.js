...
import { GetModel3DSpec, SaveScreenshotFile } from "../../bindings/ysm-model-manager/internal/app/app.js";
import { loadTextures } from "./model3d-loader.js";
import { buildSceneMesh } from "./model3d.js";

// renderMultiAngle 透明背景多角度截图
export async function renderMultiAngle(modelPath, texUrls, opts = {}) {
  const size = opts.size || 512;
  const spec = JSON.parse(await GetModel3DSpec(modelPath));
  ...
  for (const mg of spec.models || []) {
    ...
    for (const md of mg.meshGroups) {
      ...
      const mat = mt
        ? new THREE.MeshBasicMaterial({ map: mt, alphaTest: mti > 0 ? 0.5 : 0.02, transparent: true, side: mti > 0 ? THREE.BackSide : THREE.DoubleSide })
  ...
  for (const { name, theta } of angles) {
    camera.position.set(center.x + Math.sin(theta) * dist, center.y, center.z - Math.cos(theta) * dist);
    camera.lookAt(center);
    renderer.render(scene, camera);
    results.push({ name, base64: renderer.domElement.toDataURL("image/png").split(",")[1] });
  }
...
export async function batchRepoScreenshots(repoRoot, outputDir) {
  const { ScanModelEntries, SaveScreenshotFile } = await import("../../bindings/ysm-model-manager/internal/app/app.js");
  if (!repoRoot) {
    try {
  ...
  if (!repoRoot) { console.warn("[batch] 请传入仓库路径: __batchRepoScreenshots('C:/path/repo')"); return; }

  const entries = await ScanModelEntries(repoRoot);
  const models = Array.isArray(entries) ? entries.filter(e => {
    const p = e.Path || e.path || e.Name || e.name || "";
  ...
  for (const m of models) {
    const fullPath = m.Path || m.path || m.Name || m.name || "";
    const normalized = fullPath.replace(/\\/g, "/");
    const base = normalized.split("/").pop().replace(/\.\w+$/, "");
    console.log("[batch] 截图:", base);
    try {
      const { AnalyzeBedrockModel } = await import("../../bindings/ysm-model-manager/internal/app/app.js");
      let texUrls = [];
      try {
      ...
      } catch (e) { console.warn("[batch] 取纹理失败:", e); }
      const results = await renderMultiAngle(fullPath, texUrls, { size: 512 });
      if (!results) { fail++; continue; }
      const dir = outputDir || (normalized.includes("/") ? normalized.slice(0, normalized.lastIndexOf("/")) : ".");
      for (const r of results) {
        await SaveScreenshotFile(dir + "/" + base + "_" + r.name + ".png", r.base64);
      }
      ok++;
    } catch (e) { console.error("[batch] 失败:", fullPath, e); fail++; }
  }
