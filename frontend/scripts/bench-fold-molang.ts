 ...
 *
 * 运行：cd frontend && node scripts/bench-fold-molang.ts
 */

import { foldMolangConstant } from "../src/utils/animation/animation.ts";
import { compileMolang } from "../src/utils/animation/molang.ts";
...
const foldNs = bench(
  "foldMolangConstant（6 样本轮转，含命中/不命中）",
  () => {
...
const compFoldNs = bench(
  "compileMolang（同 6 样本，若不做折叠全走编译）",
  () => {
