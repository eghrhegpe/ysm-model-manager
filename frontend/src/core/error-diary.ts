...
// core 不感知 Wails：落盘通道 DiarySink 由装配层注入（backend/diary-sink.ts 适配
// AddOpLog）；本模块只持净化/去重/截断策略。设计沿革见 docs/knowledge/core-error-diary.md
import { bus, type ToastPayload } from "@/bus";
import { setLogSink } from "@/utils/base/log.ts"; // ADR-189 D4：core→utils/base 允许（纯函数层）

/** 日记状态（与 go/logs status 枚举对齐） */
export type DiaryStatus = "failed" | "warn";

/** 净化后的日记条目：title/detail 均已剥离 emoji 前缀与内部路径段并截断
 *  （title→AddOpLog modelName 位，detail→errMsg 位） */
...
// 截断上限：title 200 / detail 500（前端冗余防御，Go 侧 maxFieldLen=1024 会再截断）；
// 同 (msg+status) 连续 5s 去重：网络抖动/批量导入失败风暴防写入放大（UI 错误挤占
// 500 条共享环形缓冲）；A-B-A 交错风暴按设计逐条记录
...
function logUiMsg(msg: string, status: DiaryStatus): void {
  if (!_sink) return;
  // 同 (msg+status) 连续 5s 去重（只记单条 key）
  const key = `${status}:${msg}`;
  const now = Date.now();
  if (key === _lastDedupKey && now - _lastDedupAt < DIARY_DEDUP_WINDOW) return;
  _lastDedupKey = key;
  _lastDedupAt = now;
  // 净化：剥 ❌/⚠️ 前缀（含 U+FE0F 变体选择器）+ 剥离内部路径段（stripPathSegments
  // 只算一次，title/detail 两持久化字段复用）+ 截断
