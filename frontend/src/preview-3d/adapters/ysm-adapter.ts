...
async function scanAnimFiles(sc: YsmSceneCtx): Promise<{
  ...
  for (const animFile of animFiles) {
    try {
      ...
      logWarn(
        "preview-3d",
        `动画文件解析失败跳过（模型将缺该组动画）: ${e instanceof Error ? e.message : e}`,
      );
    }
  }
  ...
  for (const ctrlFile of controllerFiles) {
    try {
      ...
      logWarn("preview-3d", `控制器文件解析失败跳过: ${e instanceof Error ? e.message : e}`);
    }
  }
  return { clips: allClips, controllers: allControllers };
}
...
async function buildBonePanelAndAnim(sc: YsmSceneCtx, core: YsmBuildCore): Promise<YsmPanelAnim> {
  ...
  if (!isGenericMode) {
    try {
      ...
      logWarn(
        "preview-3d",
        `动画扫描失败，静默降级（模型可渲染但无动画）: ${e instanceof Error ? e.message : e}`,
      );
    }
  }
  return {
