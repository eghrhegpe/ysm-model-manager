...
// 消费方（app-tree 键盘门禁、3D 菜单）统一经本函数查询，禁止各自裸 getElementById——
// 隐式契约收编为显式 API，模式可被后续组件安全复用而不扩散。
// ADR-220 归位：随挂载核心下沉 preview-3d/adapters/，与唯一生产者 mount-preview-core 同目录
// （原「勿因单消费者下沉」辩护随归位失效，已压缩）；新增消费方直接 import 本文件即可。
