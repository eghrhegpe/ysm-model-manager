// ===== cap-to-node.ts — ADR-195：MenuControlDef → MenuNode 同构映射 =====
...
//   - cap.getMenuControls(): MenuControlDef[]（平行数组自报）
//   - PreviewMenuNode[]（节点树）
...
// cap 自身（getMenuControls 退役），最后 MenuControlDef 类型名作废、字段并入
// PreviewControlSpec。本文件纯转换，零注册表依赖，可单测。
 ...
 * controls 通道是节点树内嵌 MenuControlDef 的受控委托，仍经 renderMenu 调度 +
 * renderCapControls 渲染，不触发 renderCustom 审计门。
 * 原生名单随 spec/节点能力补齐扩展（单一事实源）。
 */
export function canNodeRepresent(c: MenuControlDef): boolean {
  return (
    c.kind === "toggle" ||
    c.kind === "slider" ||
    c.kind === "select" ||
...
/**
 * 单个 MenuControlDef → 原生控件节点。
 * 仅对 canNodeRepresent=true 的 kind 调用（toggle/slider/select/divider/color）。
 * control 字段承载 MenuControlDef 全部能力（spec 与 MenuControlDef 同构——
 * 见 node-types PreviewControlSpec），nodeControlToCapControl 反向纯搬运零损失。
 */
export function capControlToNode(c: MenuControlDef): PreviewMenuNode {
  ...
  // control spec = MenuControlDef 同构（getValue/setValue 闭包直迁）
...
/**
 * MenuControlDef[]（含 group 语义）→ PreviewMenuNode[]。
 ...
 */
export function capControlsToNodes(controls: MenuControlDef[]): PreviewMenuNode[] {
