...
async function buildScene(): Promise<{
  ...
  const panel = items[0]!;
  const builder = getSchema(LITEMATIC_SLICE_SCHEMA_ID)!;
  return { ctx, built, panel, nodes: builder(previewSnapshot()) };
}
...
describe("litematic 分层切片（schema builder 声明式契约）", () => {
  it("panel 节点走 schemaId 注册通道（无 renderCustom 逃生舱）", async () => {
    ...
    expect(panel.schemaId).toBe(LITEMATIC_SLICE_SCHEMA_ID);
    expect(panel.renderCustom).toBeUndefined();
    expect(getSchema(LITEMATIC_SLICE_SCHEMA_ID)).toBeTypeOf("function");
  });
  ...
  it("轴 select：set 更新闭包轴 + 重置层值，重建节点后 slider max 随轴刷新", async () => {
    ...
    const rebuilt = getSchema(LITEMATIC_SLICE_SCHEMA_ID)!(previewSnapshot());
    ...
    expect(nodeById(getSchema(LITEMATIC_SLICE_SCHEMA_ID)!(previewSnapshot()), "slice-layer").control!.max).toBe(13);
  });
  ...
  it("dispose 注销 schema + 重置切片模式（防跨会话泄漏）", async () => {
    ...
    setStateValue("ui.litematicSliceMode", "single");
    built.dispose();
    expect(getSchema(LITEMATIC_SLICE_SCHEMA_ID)).toBeUndefined();
