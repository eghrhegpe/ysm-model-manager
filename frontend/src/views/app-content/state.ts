...
export class AppContentState {
  ...
  // 注：`currentSite` 已下沉到工坊页作用域（ADR-263，`site/workshop-page-state.ts`）——
   ...
   *  ⚠️ 跨切归属（ADR-263）：写入方 `features/community/download-queue-store.ts` 是模块级持久层
  ...
  /** 创意工坊默认站点延迟加载定时器
   *  ⚠️ 跨切归属（ADR-263）：清理点是 app 壳层的 `_render()` 开头（早于 `page.init`，
  ...
  // 不再在共享 state 上开专用字段；页面级可变状态由各页闭包自持（currentSite 已于 ADR-263 归位）。
