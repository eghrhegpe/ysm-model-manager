...
// code_review 4c485675e #1（P3）：路由签名收紧为 PreviewCtx & PreviewRouterCtx 后，
// mountPreview() 的真实 AppPreview 元素同时实现两者——cast 目标同步收紧，
// 最小面 mock（仅 PreviewRouterCtx）从此编译不过，契约由编译器重新校验
type RouterCtxArg = PreviewCtx & PreviewRouterCtx;
...
describe("_showModelDetail — 类型分流", () => {
  it("PACK → showResourcePack", async () => {
    ...
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/pack");
  ...
  it("YSM → showModelDetail；空检测 → toast + showSimplePreview（不假装 YSM）", async () => {
    ...
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/m.ysm");
    appObj.DetectResourceType.mockResolvedValue("");
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/unknown");
  ...
  it("LITEMATIC / BLUEPRINT → showLitematic", async () => {
    ...
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/a.litematic");
    appObj.DetectResourceType.mockResolvedValue(RESOURCE_TYPES.BLUEPRINT);
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/b.blueprint");
  ...
  it("shaderpack → showShaderpack（lang 提取显示名）", async () => {
    ...
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/s.zip");
  ...
  it("EntityPlayer → showMmdPreview（文件名 + FAB 进 3D）", async () => {
    ...
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/m.pmx");
  ...
  it("VRC .vrm → showVrmMeta（meta 卡）", async () => {
    ...
    appObj.DetectResourceType.mockResolvedValue(RESOURCE_TYPES.MMD);
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/avatar.vrm");
  ...
  it("VRC .vrca → 未识别类型（.vrca 不在注册表 extensions 里）", async () => {
    ...
    appObj.DetectResourceType.mockResolvedValue("");
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/avatar.vrca");
  ...
  it("未注册类型 → showSimplePreview 兜底（查表未命中）", async () => {
    ...
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/x.xyz");
    expect(detailSpies.showSimplePreview).toHaveBeenCalledWith(
      el,
      "/repo/x.xyz",
  ...
  it("DetectResourceType 抛错 → 空类型走 unrecognizedType 提示（不假装 YSM）", async () => {
    ...
    await routeModelPreview(el as unknown as RouterCtxArg,"/repo/e.ysm");
...
describe("_showPackInfo", () => {
  it("有信息 → 渲染包名与描述", async () => {
    ...
    await routePackInfo(el as unknown as RouterCtxArg,"/repo/pack");
  ...
  it("无信息 → 渲染文件夹名 + folderNoInfo", async () => {
    const el = mountPreview();
    await routePackInfo(el as unknown as RouterCtxArg,"/repo/pack-folder");
    const text = el.root.querySelector("#preview-content")?.textContent || "";
