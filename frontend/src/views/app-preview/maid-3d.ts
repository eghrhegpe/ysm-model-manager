 ...
 */
async function createMaid3D(path: string, texIdx = 0, opts: MaidOpenOptions): Promise<void> {
...
function toStatsCardModel(info: MaidModelInfo, componentCounts: ComponentCount[]): StatsCardModel {
  return {
...
<button class="preview-fab" id="btn-3d-preview" title="${t("preview.title3d")}" aria-label="${t("preview.title3d")}"><span class="preview-ic">&#x1F3A8;</span></button>`;
  ...
  try {
    ...
    if (spec) {
      ...
      const m0 = (spec as { models?: Array<{ textureWidth?: number; textureHeight?: number }> })
        .models?.[0];
      if (m0 && baseModelInfo) {
