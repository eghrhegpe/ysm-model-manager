...
export async function loadModel2D(
  ...
  // ===== 3D 切换机制（ADR-253 遗留缺陷修复 2026-09-16）=====
  // 本块**必须**在 2D 骨架加载之前同步执行：原实现把 btn3d.onclick 挂在 try 尾部
  // （所有 await + 两个早退之后），导致 loadModelData 抛错 / 无 bones / 摘要提取失败
  // 三条路径上 FAB 渲染出来却点了没反应（死点击）。
  // 依据：_toggle3D 只依赖 modelPath 参数与 ctx，与本函数加载出的 model 无关
  // （3D 侧自带 loader 重新加载），因此把绑定与 2D 骨架加载解耦是语义安全的。
  let _is3D = false,
