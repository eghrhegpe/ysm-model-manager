...
export function containerHTML(): string {
  return (
    ...
    // 选中态判据（2026-09 修订）：**同一条工具栏内选中态必须一致**，优先于「数按钮个数」。
    // 本栏 6 个筛选并排（all/synced/missing/disabled/optional/legacy），实心会是视觉噪音。
    // 统一范式 = accent 18% 淡化底 + accent 字 + accent 边框，与 .cr-tag-filter-btn.active、
    // .diag-log-fbtn.active、.diag-sub-tab.active 逐值同款。
