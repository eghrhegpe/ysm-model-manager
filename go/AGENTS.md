...
- **新注释必须携带「代码读不出的信息」**：ADR 编号、历史 bug 上下文、平台坑点、非显然的取舍。禁止复述代码（`// 设置状态` 之于 `setState()` 属冗余）
- **禁止全仓注释瘦身**：本仓注释是「查证优先」体系的检索载体（决策与数据见 ADR-236），存量注释不得批量删除或压缩
- **踩坑类注释收敛进本节「Go 专属坑点」**，别在每个文件各留一份副本 —— N 份副本必然漂移
- **包名与标准库同名时**（如 `go/sync`），包注释必须写明 alias 约定与「勿重命名」理由，见 ADR-236
