// ===== DetectContainerTypeFromBase64Tail 尾部探针测试（audit #1）=====
package importer
...
// DetectContainerType 按 local header 顺序解析（data descriptor 流式写入下只取到首条），
...
func TestDetectContainerTypeFromBase64Tail(t *testing.T) {
	// 正常 zip：尾部探针与全量 DetectContainerType 同答案。ysm.json 恒为首条——
	...
	want := ""
	if full := DetectContainerType(mustDecode(t, b64)); full != "" {
		want = full
	}
	if id, ok := DetectContainerTypeFromBase64Tail(b64); !ok || id != want {
		t.Errorf("尾部探针 = (%q, %v), want (%q, true)", id, ok, want)
	}
	// 与全量路径交叉验证：两者判定一致
	fullID := DetectContainerType(mustDecode(t, b64))
	tailID, _ := DetectContainerTypeFromBase64Tail(b64)
	if fullID != tailID {
		t.Errorf("全量=%q 尾部=%q 应一致", fullID, tailID)
	}

	// 非 zip 垃圾：ok=false，交全量兜底
	if _, ok := DetectContainerTypeFromBase64Tail(base64.StdEncoding.EncodeToString(bytes.Repeat([]byte{0xAB}, 128))); ok {
		t.Error("非 zip 输入应返回 ok=false")
	}

	// 非法 base64（长度非 4 倍数）：ok=false
	if _, ok := DetectContainerTypeFromBase64Tail("abc"); ok {
	...
	if id, ok := DetectContainerTypeFromBase64Tail(b64Many); !ok || id == "" {
		t.Errorf("多条目 zip 尾部探针应成功, got (%q, %v)", id, ok)
	}
}

func TestDetectContainerTypeFromBase64Tail_LargePaddingFile(t *testing.T) {
	...
	id, ok := DetectContainerTypeFromBase64Tail(b64)
	if !ok || id == "" {
