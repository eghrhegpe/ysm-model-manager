...
// ===== 侧栏实例状态测试已随 ADR-310 迁移 =====
// 旧链 GetInstanceStatus/GetInstanceStatusWith（compareHashMode/compareRelKeyMode）已删除，
// 对应断言迁至 go/instance/status_counts_test.go（同一份语义在面板链上重新钉死）：
//   - Missing/Extra/Synced 计数与清单 → TestBuildInstanceStatusCounts_DirLevelFoldAndLists
//   - 空入参守卫 / 禁用桶不蒸发 → TestBuildInstanceStatusCounts_EmptyInputs / _RepoBannedDisabled
// ===== 测试用例 =====
