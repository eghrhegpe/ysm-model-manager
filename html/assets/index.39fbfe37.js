...
`;function M(){return`<div class="hdr">
<div class="hdr-row">
...
</div>`}function C(){return`<div class="ftr">
...
</div>`}function y(e,t){return`<div class="empty"><div class="big">${e}</div>${t}</div>`}function h(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function $(e,t){const n=h(e);if(!t)return n;const a=t.toLowerCase(),s=e.toLowerCase().indexOf(a);if(s===-1)return n;const o=h(e.substring(0,s)),r=h(e.substring(s,s+t.length)),i=h(e.substring(s+t.length));return o+"<mark>"+r+"</mark>"+i}function H(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function B(e){if(!e)return"";const t=new Date(e),n=new Date;return t.toDateString()===n.toDateString()?t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):(n-t)/864e5<7?["\u65E5","\u4E00","\u4E8C","\u4E09","\u56DB","\u4E94","\u516D"][t.getDay()]+" "+t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):t.toLocaleDateString([],{month:"short",day:"numeric"})}function I(e){const t=(e.split(".").pop()||"").toLowerCase();return t==="ysm"?"\u{1F48E}":["zip","rar","7z","tar","gz"].includes(t)?"\u{1F4E6}":["png","jpg","jpeg","gif","webp","bmp"].includes(t)?"\u{1F5BC}\uFE0F":["txt","md","json","xml","yml","yaml","cfg","conf","ini"].includes(t)?"\u{1F4C4}":"\u{1F9CA}"}function A(e,t,n){const a=[...e].sort((r,i)=>t==="size"?(i.size||0)-(r.size||0):t==="date"?(i.modTime||0)-(r.modTime||0):r.name.localeCompare(i.name)),s=(n||"").trim().toLowerCase(),o={};return a.forEach(r=>{if(s&&!r.name.toLowerCase().includes(s))return;const i=r.path.replace(/\\/g,"/").split("/");let d=o;for(let c=0;c<i.length-1;c++)!i[c]||(d[i[c]]||(d[i[c]]={}),d=d[i[c]]);d[i[i.length-1]]={_e:r}}),o}function z(e,t){if(!t||!e)return!1;for(const n of Object.keys(e)){const a=e[n];if(a._e){if(a._e.name.toLowerCase().includes(t))return!0}else if(n.toLowerCase().includes(t)||z(a,t))return!0}return!1}function P(e){const t=e.length,n=e.filter(s=>!s.banned).length,a=e.reduce((s,o)=>s+(o.size||0),0);return{total:t,enabled:n,totalSize:a}}function D(e,t,n,a){const s=b(e.path),o=b(e.fullPath||e.path),r=e.banned?"":" on",i=e.banned?"":"\u2713";return`<div class="fl${e.banned?" ban":""}" data-path="${s}" data-fullpath="${o}">
<span class="ck${r}" data-path="${s}" data-fullpath="${o}">${i}</span>
<span class="ficon">${n}</span>
<span class="nm">${t}</span>
<span class="sz">${R(e.size)}</span>${a?`<span class="dt">${a}</span>`:""}</div>`}function O(e,t,n,a){const s=a?"\u{1F512}":"\u{1F4C1}",o=a?"#585b70":"#a6adc8",r=a?" locked":"",i=n?"\u25BC":"\u25B6",d=n?" open":"";return`<div class="fh${r}" data-dir="${b(t)}">
<span class="ar${d}">${i}</span>
<span class="nm" style="color:${o}">${s} ${b(e)}</span></div>
<div class="ch" style="display:${n?"block":"none"}">`}function b(e){return(e||"").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function R(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function F(e,t,n,a,s){if(!t.length){e.innerHTML=y("\u{1F4C1}","\u6682\u65E0\u6A21\u578B\u6587\u4EF6");return}const o=A(t,a,n),r=L(o,"",n,a,s);if(!r){e.innerHTML=y("\u{1F50D}","\u672A\u627E\u5230\u5339\u914D\u7684\u6587\u4EF6");return}e.innerHTML=r}function j(e,t){if(!e)return;const n=P(t);e.textContent=`\u5171 ${n.total} \u9879 (\u5DF2\u542F\u7528 ${n.enabled}) \xB7 ${H(n.totalSize)}`}function L(e,t,n,a,s){const o=!!(n||"").trim(),r=Object.keys(e).sort((d,c)=>{const u=!e[d]._e,p=!e[c]._e;if(u&&!p)return-1;if(!u&&p)return 1;const f=e[d]._e,l=e[c]._e;return a==="size"?((l==null?void 0:l.size)||0)-((f==null?void 0:f.size)||0):a==="date"?((l==null?void 0:l.modTime)||0)-((f==null?void 0:f.modTime)||0):d.localeCompare(c)});let i="";return r.forEach(d=>{const c=e[d],u=t?t+"/"+d:d;if(c._e){const p=c._e;if(o&&!p.name.toLowerCase().includes(n.toLowerCase()))return;const f=o?$(p.name,n):p.name,l=p.modTime?B(p.modTime):"",v=I(p.name);i+=D(p,f,v,l)}else{const p=d.startsWith("_"),f=o||!!s[u],l=o?z(c,n.toLowerCase()):!1;i+=O(d,u,f||o&&l,p),i+=L(c,u,n,a,s),i+="</div>"}}),i}function g(e){!e||(e.classList.add("flash"),setTimeout(()=>e.classList.remove("flash"),400))}function N(e,t){e.querySelectorAll(".fh").forEach(n=>{n.onclick=a=>{a.stopPropagation();const s=n.nextElementSibling,o=n.querySelector(".ar");if(!s)return;const r=s.style.display!=="none";s.style.display=r?"none":"block",o.classList.toggle("open",!r),t._dirOpen[n.dataset.dir]=!r,localStorage.setItem("at_dirs",JSON.stringify(t._dirOpen))}}),e.querySelectorAll(".ck").forEach(n=>{n.onclick=a=>{a.stopPropagation();const s=n.classList.contains("on");n.classList.toggle("on"),n.textContent=n.classList.contains("on")?"\u2713":"";const o=n.closest(".fl");o&&o.classList.add("flash"),setTimeout(()=>o==null?void 0:o.classList.remove("flash"),400);const r=n.dataset.path,i=n.dataset.fullpath||r;bus.emit("entry:toggle",{path:i,relPath:r,enabled:!s})}}),e.querySelectorAll(".fl").forEach(n=>{n.oncontextmenu=a=>{var i,d,c;a.preventDefault();const s=((d=(i=n.querySelector(".nm"))==null?void 0:i.textContent)==null?void 0:d.replace(/^\S+\s/,""))||"",o=!((c=n.querySelector(".ck"))!=null&&c.classList.contains("on")),r=n.dataset.fullpath||n.dataset.path;bus.emit("ctx:show",{x:a.clientX,y:a.clientY,path:r,relPath:n.dataset.path,name:s,banned:o})}})}function q(e,t){var s,o,r,i,d,c,u,p,f;const n=l=>e.getElementById(l),a=()=>t._renderTree();(s=n("srch"))==null||s.addEventListener("input",l=>{t._search=l.target.value,a()}),(o=n("sort"))==null||o.addEventListener("change",l=>{t._sort=l.target.value,a()}),(r=n("btn-repo"))==null||r.addEventListener("click",()=>bus.emit("dir:select-repo")),(i=n("btn-dedup"))==null||i.addEventListener("click",()=>bus.emit("entries:dedup")),(d=n("btn-trash"))==null||d.addEventListener("click",()=>bus.emit("recycle:open")),(c=n("btn-pv"))==null||c.addEventListener("click",()=>bus.emit("preview:toggle")),(u=n("btn-ea"))==null||u.addEventListener("click",()=>{g(n("btn-ea")),t._entries.forEach(l=>{l.banned=!1}),a()}),(p=n("btn-da"))==null||p.addEventListener("click",()=>{g(n("btn-da")),t._entries.forEach(l=>{l.banned=!0}),a()}),(f=n("btn-st"))==null||f.addEventListener("click",()=>g(n("btn-st")))}function U(e){return window.go.main.App.IsFileBanned(e)}function Y(){return window.go.main.App.LoadAppConfig()}function K(e,t,n){return window.go.main.App.SaveAppConfig(e,t,n)}function J(e){return window.go.main.App.ScanModelEntries(e)}function W(){return window.go.main.App.SelectDirectory()}function G(e){return window.go.main.App.ToggleModelEnable(e)}async function V(){const t=(await Y()).RepoRoot||"";if(!t)return{repoRoot:t,entries:[]};const n=await J(t);if(!n||!n.length)return{repoRoot:t,entries:[]};const a=[];for(const s of n){let o=!1;try{o=await U(s.Path)}catch{}let r=s.Path;t&&s.Path.startsWith(t)&&(r=s.Path.slice(t.length).replace(/^[/\\]+/,"")),a.push({name:s.Name,path:r,fullPath:s.Path,size:s.Size,modTime:s.ModTime,banned:o})}return{repoRoot:t,entries:a}}function w(){const e=Date.now();return[{name:"steve_skin.ysm",path:"steve_skin.ysm",size:1258291,modTime:e-864e5,banned:!1},{name:"alex_deluxe.ysm",path:"alex/alex_deluxe.ysm",size:2516582,modTime:e-1728e5,banned:!1},{name:"alex_head.ysm",path:"alex/alex_head.ysm",size:524288,modTime:e-1728e5,banned:!1},{name:"dragon_armor.zip",path:"dragon/dragon_armor.zip",size:3984588,modTime:e-36e5,banned:!1},{name:"dragon_wings.ysm",path:"dragon/dragon_wings.ysm",size:2202009,modTime:e-72e5,banned:!1},{name:"neon_sword.ysm",path:"weapons/neon_sword.ysm",size:1572864,modTime:e,banned:!1},{name:"magic_staff.zip",path:"weapons/magic_staff.zip",size:4404019,modTime:e-72e5,banned:!1},{name:"photon_body.ysm",path:"photon/photon_body.ysm",size:2202009,modTime:e-432e5,banned:!1},{name:"old_model.ysm",path:"_disabled/old_model.ysm",size:943718,modTime:e-6048e5,banned:!0},{name:"custom_hat.ysm",path:"custom/custom_hat.ysm",size:838860,modTime:e-2592e5,banned:!1},{name:"steve_2d.ysm",path:"custom/steve_2d.ysm",size:314572,modTime:e-5e5,banned:!1}]}class X extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(E),this._entries=[],this._search="",this._sort="name",this._dirOpen={},this._repoRoot=""}async connectedCallback(){try{Object.assign(this._dirOpen,JSON.parse(localStorage.getItem("at_dirs")||"{}"))}catch{}this._renderLayout(),q(this._root,this),this._unsubToggle=bus.on("entry:toggle",async({path:t})=>{try{await G(t)}catch{}await this._load(),this._renderTree()}),this._unsubRepo=bus.on("dir:select-repo",async()=>{try{const t=await W();if(!t)return;await K(t,"","copy"),this._repoRoot=t,await this._load(),this._renderTree()}catch{this._entries=w(),this._renderTree()}}),this._unsubDedup=bus.on("entries:dedup",async()=>{bus.emit("toast:show",{msg:"\u{1F517} \u53BB\u91CD\u529F\u80FD\u5F00\u53D1\u4E2D",duration:2e3,type:"info"})}),this._unsubRecycle=bus.on("recycle:open",async()=>{bus.emit("toast:show",{msg:"\u{1F5D1}\uFE0F \u56DE\u6536\u7AD9\u529F\u80FD\u5F00\u53D1\u4E2D",duration:2e3,type:"info"})}),await this._load(),this._renderTree()}disconnectedCallback(){[this._unsubToggle,this._unsubRepo,this._unsubDedup,this._unsubRecycle].forEach(t=>t==null?void 0:t())}async _load(){try{const{repoRoot:t,entries:n}=await V();this._repoRoot=t,this._entries=n}catch{this._entries=w()}}_renderLayout(){this._root.innerHTML=M()+'<div class="list" id="tree"><div class="empty"><div class="big">\u{1F4C1}</div>\u6682\u65E0\u6A21\u578B\u6587\u4EF6</div></div>'+C()}_renderTree(){F(this._root.getElementById("tree"),this._entries,this._search,this._sort,this._dirOpen),N(this._root.getElementById("tree"),this),j(this._root.getElementById("ftr-stat"),this._entries)}}customElements.define("app-tree",X);const Q=`
:host {
...
`;function Z(){return`<div class="header">
...
</div>`}function ee(){return`<div class="footer">
<button class="footer-btn" id="btn-mc">\u{1F3AE} \u6307\u5B9A\u6E38\u620F\u8DEF\u5F84</button>
</div>`}function te(){return'<div class="list" id="vg"></div>'}function ne(e,t,n){const a=[];return t>0&&a.push(`<span class="tag green">\u2705 ${t}</span>`),n>0&&a.push(`<span class="tag red">\u2B07\uFE0F ${n}</span>`),`<div class="vc-header">
<span class="arrow">\u25B6</span>
<span class="name">\u{1F4E6} ${T(e)}</span>
${a.join("")}
</div>`}function m(e,t){return`<div class="sec-title">${e} (${t})</div>`}function x(e,t,n){return`<div class="row"><span class="dot" style="background:${e}"></span><span class="rn">${T(t)}</span><span class="sz">${n}</span></div>`}function T(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const se=[{name:"\u6211\u7684\u6574\u5408\u5305",synced:3,missing:1,extra:2,items:{synced:[{name:"steve_skin.ysm",size:"1.2 MB"},{name:"alex_deluxe.ysm",size:"2.4 MB"},{name:"neon_sword.ysm",size:"1.5 MB"}],missing:[{name:"dragon_armor.zip",size:"3.8 MB"}],extra:[{name:"custom_hat.ysm",size:"0.8 MB"},{name:"old_hat.ysm",size:"0.3 MB"}]}},{name:"\u5149\u5F71\u6D4B\u8BD5\u5305",synced:1,missing:2,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"\u7A7A\u5C9B\u751F\u5B58",synced:5,missing:0,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"RPG \u5192\u9669",synced:2,missing:3,extra:0,items:{synced:[],missing:[],extra:[]}}];function ae(e,t){e.innerHTML="",t.forEach((n,a)=>{const s=document.createElement("div");s.className="vc",s.dataset.idx=a,s.innerHTML=ne(n.name,n.synced,n.missing),e.appendChild(s);const o=document.createElement("div");o.className="vc-body",o.style.display=a===0?"":"none",o.innerHTML=oe(n),e.appendChild(o)})}function oe(e){let t="";return e.items.synced.length&&(t+=m("\u2705 \u5DF2\u540C\u6B65",e.items.synced.length),e.items.synced.forEach(n=>{t+=x("#a6e3a1",n.name,n.size)})),e.items.missing.length&&(t+=m("\u2B07\uFE0F \u7F3A\u5931",e.items.missing.length),e.items.missing.forEach(n=>{t+=x("#f38ba8",n.name,n.size)})),e.items.extra.length&&(t+=m("\u{1F4E4} \u989D\u5916",e.items.extra.length),e.items.extra.forEach(n=>{t+=x("#f9a826",n.name,n.size)})),t}function re(e){e.querySelectorAll(".vc-header").forEach(t=>{t.onclick=()=>{const n=t.nextElementSibling,a=t.querySelector(".arrow");n&&n.classList.contains("vc-body")&&(n.style.display=n.style.display==="none"?"":"none",a.classList.toggle("open"))}})}function ie(e){const t=e.getElementById("ver-search");t&&(t.oninput=n=>{bus.emit("ver:search",{keyword:n.target.value})})}function de(e){const t=e.getElementById("btn-mc");t&&(t.onclick=()=>bus.emit("dir:select-mc"))}function ce(e,t){t.push(bus.on("versions:updated",({instances:n})=>{const a=e.getElementById("ver-stat");a&&(a.textContent=`${n.length}\u4E2A\u6574\u5408\u5305`)}))}class le extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(Q),this._instances=se,this._unsubs=[]}connectedCallback(){this._renderLayout(),ae(this._root.getElementById("vg"),this._instances),re(this._root),ie(this._root),de(this._root),ce(this._root,this._unsubs)}disconnectedCallback(){this._unsubs.forEach(t=>t())}_renderLayout(){this._root.innerHTML=Z()+te()+ee()}}customElements.define("app-sidebar",le);const pe=`
:host {
  display: flex; flex-direction: column;
  background: #11111b;
...
`;function fe(){return`<div class="content">
<h3>\u{1F4CA} \u7EDF\u8BA1</h3>
...
</div>`}const ue={repo:6,ver:4,ok:2,tot:4,pending:2};function _(e,t){const n=a=>e.getElementById(a);n("s-repo")&&(n("s-repo").textContent=t.repo),n("s-ver")&&(n("s-ver").textContent=t.ver),n("s-ok")&&(n("s-ok").textContent=t.ok),n("s-tot")&&(n("s-tot").textContent=t.tot),n("s-pending")&&(n("s-pending").textContent=t.pending)}function he(e){var t,n,a;(t=e.getElementById("btn-refresh"))==null||t.addEventListener("click",()=>bus.emit("stats:refresh")),(n=e.getElementById("btn-upload"))==null||n.addEventListener("click",()=>bus.emit("stats:upload")),(a=e.getElementById("btn-logs"))==null||a.addEventListener("click",()=>bus.emit("stats:logs"))}function be(e,t,n){n.push(bus.on("stats:updated",a=>{a&&Object.assign(t,a),bus.emit("_preview:needs-update")}))}class ge extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(pe),this._stats={...ue},this._unsubs=[]}connectedCallback(){this._renderLayout(),he(this._root),be(this._root,this._stats,this._unsubs),this._unsubs.push(bus.on("_preview:needs-update",()=>{_(this._root,this._stats)})),_(this._root,this._stats)}disconnectedCallback(){this._unsubs.forEach(t=>t())}_renderLayout(){this._root.innerHTML=fe()}}customElements.define("app-preview",ge);const me=`
...
`;function k(){return`<div style="flex:1;display:flex;flex-direction:column;overflow-y:auto">
...
</div>`}function xe(){return'<div class="repo-layout"><app-tree></app-tree></div>'}function ve(){return'<div class="repo-layout"><app-sidebar></app-sidebar><app-preview></app-preview></div>'}function ye(){return`<div style="flex:1;overflow-y:auto">
...
</div>`}function S(e,t){return`<div class="placeholder-box"><div class="big">${e}</div><div>${t}</div><span class="ptag">\u9884\u544A</span></div>`}class we extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(me),this._current="dashboard"}connectedCallback(){this._unsub=bus.on("nav:change",({page:t})=>{this._current=t,bus.emit("nav:changed",{page:t}),this._render()}),this._render()}disconnectedCallback(){this._unsub&&this._unsub()}_render(){let t="";switch(this._current){case"dashboard":t=k();break;case"repository":t=xe();break;case"instances":t=ve();break;case"downloads":t=S("\u2B07\uFE0F","\u4E0B\u8F7D\u4E0E\u66F4\u65B0");break;case"diagnostics":t=S("\u{1F6E0}\uFE0F","\u8BCA\u65AD\u4E0E\u51B2\u7A81\u68C0\u6D4B");break;case"settings":t=ye();break;default:t=k()}this._root.innerHTML=`<div class="page">${t}</div>`}}customElements.define("app-content",we);
