...
</div>`}function Qe(){return'<div class="list" id="vg"></div>'}function Ze(e,t,n,o=!1){const s=[];return t>0&&s.push(`<span class="tag green">\u2705 ${t}</span>`),n>0&&s.push(`<span class="tag red">\u2B07\uFE0F ${n}</span>`),`<div class="vc-header">
<span class="${o?"arrow open":"arrow"}">\u25B6</span>
<span class="name">\u{1F4E6} ${V(e)}</span>
${s.join("")}
</div>`}function M(e,t){return`<div class="sec-title">${e} (${t})</div>`}function z(e,t,n){return`<div class="row"><span class="dot" style="background:${e}"></span><span class="rn">${V(t)}</span><span class="sz">${n}</span></div>`}function V(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function et(e,t){e.innerHTML="",t.forEach((n,o)=>{const s=document.createElement("div");s.className="vc",s.dataset.idx=o;const a=o===0;s.innerHTML=Ze(n.name,n.synced,n.missing,a),e.appendChild(s);const i=document.createElement("div");i.className="vc-body",i.style.display=a?"":"none",i.innerHTML=tt(n),e.appendChild(i)})}function tt(e){let t="";return e.items.synced.length&&(t+=M("\u2705 \u5DF2\u540C\u6B65",e.items.synced.length),e.items.synced.forEach(n=>{t+=z("#a6e3a1",n.name,n.size)})),e.items.missing.length&&(t+=M("\u2B07\uFE0F \u7F3A\u5931",e.items.missing.length),e.items.missing.forEach(n=>{t+=z("#f38ba8",n.name,n.size)})),e.items.extra.length&&(t+=M("\u{1F4E4} \u989D\u5916",e.items.extra.length),e.items.extra.forEach(n=>{t+=z("#f9a826",n.name,n.size)})),t}function nt(e){e.querySelectorAll(".vc").forEach(t=>{const n=t.querySelector(".vc-header"),o=t.nextElementSibling;!n||!o||!o.classList.contains("vc-body")||(n.onclick=()=>{const s=n.querySelector(".arrow");o.style.display=o.style.display==="none"?"":"none",s&&s.classList.toggle("open")},n.ondblclick=()=>{const s=n.querySelector(".name"),a=s?s.textContent.replace(/^📦\s*/,""):"";bus.emit("instance:select",{name:a})})})}function st(e){const t=e.getElementById("ver-search");t&&(t.oninput=n=>{bus.emit("ver:search",{keyword:n.target.value})})}function ot(e){const t=e.getElementById("btn-mc");t&&(t.onclick=()=>bus.emit("dir:select-mc"))}async function at(){const e=await _(),t=e.mcRoot||e.McRoot||"",n=e.repoRoot||e.RepoRoot||"";if(!t||!n)return null;const o=await y(n),s=new Set;o.forEach(r=>s.add(r.Name.replace(/\.ban$/i,"")));const a=await W(t);if(!a||!a.length)return null;const i=await Y(t,n),c={};return(i||[]).forEach(r=>{c[r.Name]=r}),a.map(r=>{const f=c[r.Name]||{},l=f.Missing||[],m=f.Extra||[];f.Disabled;const p=new Set(l.map(u=>u.replace(/\.ban$/i,""))),g=new Set(m.map(u=>u.replace(/\.ban$/i,""))),S=[];s.forEach(u=>{!p.has(u)&&!g.has(u)&&S.push(u)});const $=[];if(r.Exists)try{const u=y(r.CustomDir),b={};(u||[]).forEach(x=>{b[x.Name.replace(/\.ban$/i,"")]=x}),$.push(...u||[])}catch{}const k=u=>{const b=$.find(x=>x.Name.replace(/\.ban$/i,"")===u.replace(/\.ban$/i,""));return b?it(b.Size):""};return{name:r.Name,exists:r.Exists,hasYSM:f.HasYSM,synced:S.length,missing:l.length,extra:m.length,items:{synced:S.slice(0,20).map(u=>({name:u,size:k(u)})),missing:l.slice(0,20).map(u=>({name:u,size:k(u)})),extra:m.slice(0,20).map(u=>({name:u,size:k(u)}))}}})}function it(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function H(){return[{name:"\u6211\u7684\u6574\u5408\u5305",synced:3,missing:1,extra:2,items:{synced:[{name:"steve_skin.ysm",size:"1.2 MB"},{name:"alex_deluxe.ysm",size:"2.4 MB"},{name:"neon_sword.ysm",size:"1.5 MB"}],missing:[{name:"dragon_armor.zip",size:"3.8 MB"}],extra:[{name:"custom_hat.ysm",size:"0.8 MB"},{name:"old_hat.ysm",size:"0.3 MB"}]}},{name:"\u5149\u5F71\u6D4B\u8BD5\u5305",synced:1,missing:2,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"\u7A7A\u5C9B\u751F\u5B58",synced:5,missing:0,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"RPG \u5192\u9669",synced:2,missing:3,extra:0,items:{synced:[],missing:[],extra:[]}}]}class rt extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(Ve),this._instances=[],this._unsubs=[]}async connectedCallback(){this._renderLayout(),this._unsubs.push(bus.on("dir:select-mc",async()=>{try{const t=await A();if(!t)return;const n=await _();await C(n.repoRoot||"",t,n.linkMode||"copy")}catch{}await this._reload()})),await this._reload()}async _reload(){try{const t=await at();t?this._instances=t:this._instances=H()}catch{this._instances=H()}et(this._root.getElementById("vg"),this._instances),nt(this._root),st(this._root),ot(this._root)}disconnectedCallback(){this._unsubs.forEach(t=>t())}_renderLayout(){this._root.innerHTML=Je()+Qe()+Xe()}}customElements.define("app-sidebar",rt);const ct="modulepreload",dt=function(e){return"/"+e},B={},lt=function(t,n,o){if(!n||n.length===0)return t();const s=document.getElementsByTagName("link");return Promise.all(n.map(a=>{if(a=dt(a),a in B)return;B[a]=!0;const i=a.endsWith(".css"),c=i?'[rel="stylesheet"]':"";if(!!o)for(let f=s.length-1;f>=0;f--){const l=s[f];if(l.href===a&&(!i||l.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${a}"]${c}`))return;const r=document.createElement("link");if(r.rel=i?"stylesheet":ct,i||(r.as="script",r.crossOrigin=""),r.href=a,document.head.appendChild(r),i)return new Promise((f,l)=>{r.addEventListener("load",f),r.addEventListener("error",()=>l(new Error(`Unable to preload CSS for ${a}`)))})})).then(()=>t())},pt=`
:host {
  display: flex; flex-direction: column;
  background: #11111b;
...
`;function D(){return`<div class="content" id="preview-content">
<h3>\u{1F4CA} \u7EDF\u8BA1</h3>
...
</div>`}function F(e){return!e||e.hasError?`<hr class="divider">
<h3>\u{1F4C4} \u6A21\u578B\u4FE1\u606F</h3>
<div class="err">\u26A0\uFE0F ${e?e.errorMsg:"\u672A\u77E5\u9519\u8BEF"}</div>`:`<hr class="divider">
<h3>\u{1F4C4} \u6A21\u578B\u4FE1\u606F</h3>
<div class="md-row"><span class="md-label">\u540D\u79F0</span><span class="md-value">${T(e.name||"-")}</span></div>
<div class="md-row"><span class="md-label">\u4F5C\u8005</span><span class="md-value">${T(e.author||"-")}</span></div>
<div class="md-row"><span class="md-label">\u7248\u672C</span><span class="md-value">${T(e.version||"-")}</span></div>
<hr class="divider">
...
<div class="md-row"><span class="md-label">\u25FB\uFE0F \u9762</span><span class="md-value">${(e.faces||0).toLocaleString()}</span></div>`}function T(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const ut={repo:6,ver:4,ok:2,tot:4,pending:2};function E(e,t){const n=o=>e.getElementById(o);n("s-repo")&&(n("s-repo").textContent=t.repo),n("s-ver")&&(n("s-ver").textContent=t.ver),n("s-ok")&&(n("s-ok").textContent=t.ok),n("s-tot")&&(n("s-tot").textContent=t.tot),n("s-pending")&&(n("s-pending").textContent=t.pending)}function P(e){var t,n,o;(t=e.getElementById("btn-refresh"))==null||t.addEventListener("click",()=>bus.emit("stats:refresh")),(n=e.getElementById("btn-upload"))==null||n.addEventListener("click",()=>bus.emit("stats:upload")),(o=e.getElementById("btn-logs"))==null||o.addEventListener("click",()=>bus.emit("stats:logs"))}function ft(e,t,n){n.push(bus.on("stats:updated",o=>{o&&Object.assign(t,o),bus.emit("_preview:needs-update")}))}class mt extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(pt),this._stats={...ut},this._unsubs=[]}connectedCallback(){this._renderLayout(),P(this._root),ft(this._root,this._stats,this._unsubs),this._unsubs.push(bus.on("_preview:needs-update",()=>{E(this._root,this._stats)})),this._unsubs.push(bus.on("model:select",async({path:t})=>{this._showModelDetail(t)})),this._unsubs.push(bus.on("model:deselect",()=>{this._showStats()})),E(this._root,this._stats)}disconnectedCallback(){this._unsubs.forEach(t=>t())}_renderLayout(){this._root.innerHTML=D()}async _showModelDetail(t){const n=this._root.getElementById("preview-content");if(!!n){n.innerHTML='<div class="content"><h3>\u{1F4C4} \u6A21\u578B\u4FE1\u606F</h3><div class="stat-row"><span class="label">\u52A0\u8F7D\u4E2D...</span></div></div>';try{const{AnalyzeYSMModel:o}=await lt(()=>Promise.resolve().then(()=>We),void 0),s=await o(t);n.innerHTML=F(s)}catch(o){n.innerHTML=F({hasError:!0,errorMsg:String(o)})}}}_showStats(){this._root.innerHTML=D(),E(this._root,this._stats),P(this._root)}}customElements.define("app-preview",mt);const ht=`
...
</div>`}function gt(){return'<div class="repo-layout"><app-tree></app-tree><app-preview></app-preview></div>'}function bt(){return'<div class="repo-layout"><app-sidebar></app-sidebar><app-preview></app-preview></div>'}function xt(){return`<div style="flex:1;overflow-y:auto">
