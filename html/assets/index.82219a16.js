...
</div>`}function Xt(){return'<div class="list" id="vg"></div>'}function Jt(t,e,s,o,n=!1){let a="";o==="complete"?a='<span class="tag green">\u2705</span>':o==="extra"?a='<span class="tag orange">\u{1F4E4}</span>':o==="missing"&&(a='<span class="tag red">\u2B07\uFE0F</span>');const i=[];return e>0&&i.push(`<span class="tag green">\u2705 ${e}</span>`),s>0&&i.push(`<span class="tag red">\u2B07\uFE0F ${s}</span>`),`<div class="vc-header">
<span class="${n?"arrow open":"arrow"}">\u25B6</span>
${a}
<span class="name">\u{1F4E6} ${W(t)}</span>
${i.join("")}
</div>`}function k(t,e){return`<div class="sec-title">${t} (${e})</div>`}function S(t,e,s,o){const n=o?`<span class="link-icon">${o}</span>`:"";return`<div class="row"><span class="dot" style="background:${t}"></span><span class="rn">${W(e)}</span>${n}<span class="sz">${s}</span></div>`}function W(t){return(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Qt(t,e){t.innerHTML="",e.forEach((s,o)=>{const n=document.createElement("div");n.className="vc",n.dataset.idx=o;const a=o===0;n.innerHTML=Jt(s.name,s.synced,s.missing,s.status,a),t.appendChild(n);const i=document.createElement("div");i.className="vc-body",i.style.display=a?"":"none",i.innerHTML=Zt(s),t.appendChild(i)})}function Zt(t){let e="";return t.items.synced.length&&(e+=k("\u2705 \u5DF2\u540C\u6B65",t.items.synced.length),t.items.synced.forEach(s=>{e+=S("#a6e3a1",s.name,s.size,s.linkType)})),t.items.missing.length&&(e+=k("\u2B07\uFE0F \u7F3A\u5931",t.items.missing.length),t.items.missing.forEach(s=>{e+=S("#f38ba8",s.name,s.size,"")})),t.items.extra.length&&(e+=k("\u{1F4E4} \u989D\u5916",t.items.extra.length),t.items.extra.forEach(s=>{e+=S("#f9a826",s.name,s.size,"")})),e}function te(t){t.querySelectorAll(".vc-context-menu").forEach(e=>e.remove()),t.querySelectorAll(".vc").forEach(e=>{const s=e.querySelector(".vc-header"),o=e.nextElementSibling;!s||!o||!o.classList.contains("vc-body")||(s.onclick=()=>{const n=s.querySelector(".arrow");o.style.display=o.style.display==="none"?"":"none",n&&n.classList.toggle("open")},s.oncontextmenu=n=>{n.preventDefault(),n.stopPropagation();const a=s.querySelector(".name"),i=a?a.textContent.replace(/^📦\s*/,""):"";bus.emit("ctx:show",{x:n.clientX,y:n.clientY,type:"instance",instanceName:i})})})}function ee(t,e){const s=t.getElementById("ver-search");s&&(s.oninput=o=>{const n=o.target.value.toLowerCase().trim();e._search=n,e._renderCards()})}function se(t){const e=t.getElementById("btn-mc");e&&(e.onclick=()=>bus.emit("dir:select-mc"))}async function ne(){bus.emit("loading:start");try{const t=await v(),e=t.mcRoot||t.McRoot||"",s=t.repoRoot||t.RepoRoot||"";if(!e||!s)return null;const o=await I(s),n=new Set;o.forEach(c=>n.add(c.Name.replace(/\.ban$/i,"")));const a=await R(e);if(!a||!a.length)return null;const i=await q(e,s),r={};return(i||[]).forEach(c=>{r[c.Name]=c}),a.map(c=>{const d=r[c.Name]||{},f=d.Missing||[],u=d.Extra||[],p=new Set(f.map(g=>g.replace(/\.ban$/i,""))),m=new Set(u.map(g=>g.replace(/\.ban$/i,""))),_=[];return n.forEach(g=>{!p.has(g)&&!m.has(g)&&_.push(g)}),{name:c.Name,exists:c.Exists,hasYSM:d.HasYSM,status:d.Status||"missing",synced:_.length,missing:f.length,extra:u.length,items:{synced:_.slice(0,20).map(g=>{const x=L(g,d.Files);return{name:g,size:"",linkType:x}}),missing:f.slice(0,20).map(g=>{const x=L(g,d.Files);return{name:g,size:"",linkType:x}}),extra:u.slice(0,20).map(g=>{const x=L(g,d.Files);return{name:g,size:"",linkType:x}})}}})}finally{bus.emit("loading:end")}}function L(t,e){if(!e||!e.length)return"";const s=e.find(o=>o.Name===t);return s?s.LinkType==="symlink"||s.LinkType==="hardlink"?"\u{1F517}":"\u{1F4CB}":""}function P(){return[{name:"\u6211\u7684\u6574\u5408\u5305",synced:3,missing:1,extra:2,items:{synced:[{name:"steve_skin.ysm",size:"1.2 MB"},{name:"alex_deluxe.ysm",size:"2.4 MB"},{name:"neon_sword.ysm",size:"1.5 MB"}],missing:[{name:"dragon_armor.zip",size:"3.8 MB"}],extra:[{name:"custom_hat.ysm",size:"0.8 MB"},{name:"old_hat.ysm",size:"0.3 MB"}]}},{name:"\u5149\u5F71\u6D4B\u8BD5\u5305",synced:1,missing:2,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"\u7A7A\u5C9B\u751F\u5B58",synced:5,missing:0,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"RPG \u5192\u9669",synced:2,missing:3,extra:0,items:{synced:[],missing:[],extra:[]}}]}class oe extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(Ut),this._instances=[],this._unsubs=[],this._search=""}async connectedCallback(){this._renderLayout(),this._unsubs.push(bus.on("dir:select-mc",async()=>{try{const e=await H();if(!e)return;const s=await v();await $(s.repoRoot||"",e,s.linkMode||"copy")}catch{}await this._reload()})),this._unsubs.push(bus.on("stats:refresh",async()=>{await this._reload()})),await this._reload()}_renderCards(){const e=this._root.getElementById("vg");if(!e)return;const s=this._search,o=s?this._instances.filter(n=>n.name.toLowerCase().includes(s)):this._instances;Qt(e,o),te(this._root)}async _reload(){try{const s=await ne();s?this._instances=s:this._instances=P()}catch{this._instances=P()}const e=this._root.getElementById("ver-stat");e&&(e.textContent=`${this._instances.length}\u4E2A\u6574\u5408\u5305`),this._renderCards(),ee(this._root,this),se(this._root)}disconnectedCallback(){this._unsubs.forEach(e=>e())}_renderLayout(){this._root.innerHTML=Wt()+Xt()+Kt()}}customElements.define("app-sidebar",oe);const ae="modulepreload",ie=function(t){return"/"+t},F={},b=function(e,s,o){if(!s||s.length===0)return e();const n=document.getElementsByTagName("link");return Promise.all(s.map(a=>{if(a=ie(a),a in F)return;F[a]=!0;const i=a.endsWith(".css"),r=i?'[rel="stylesheet"]':"";if(!!o)for(let d=n.length-1;d>=0;d--){const f=n[d];if(f.href===a&&(!i||f.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${a}"]${r}`))return;const c=document.createElement("link");if(c.rel=i?"stylesheet":ae,i||(c.as="script",c.crossOrigin=""),c.href=a,document.head.appendChild(c),i)return new Promise((d,f)=>{c.addEventListener("load",d),c.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${a}`)))})})).then(()=>e())},re=`
:host {
  display: flex; flex-direction: column;
  background: #11111b;
...
/* \u8BCA\u65AD\u9875\u9762 */
.diagnostics-layout { flex:1; display:flex; flex-direction:column; overflow:hidden; }
.diag-tabs { display:flex; border-bottom:1px solid rgba(255,255,255,.08); padding:0 16px; }
.diag-tab { padding:8px 16px; cursor:pointer; font-size:12px; color:#6c7086; border-bottom:2px solid transparent; transition:all .12s; }
.diag-tab.active { color:#7c83ff; border-bottom-color:#7c83ff; }
.diag-tab:hover { color:#cdd6f4; }
.diag-content { flex:1; overflow-y:auto; display:flex; flex-direction:column; }
.diag-toolbar { padding:8px 16px; display:flex; gap:6px; flex-shrink:0; }
...
</div>`}function be(t,e){return`<div class="placeholder-box"><div class="big">${t}</div><div>${e}</div><span class="ptag">\u9884\u544A</span></div>`}function xe(){return`<div class="diagnostics-layout">
<div class="section-title">\u{1F6E0}\uFE0F \u8BCA\u65AD\u4E0E\u51B2\u7A81</div>
<div class="diag-tabs">
<span class="diag-tab active" data-diag="log">\u{1F4CB} \u64CD\u4F5C\u65E5\u5FD7</span>
<span class="diag-tab" data-diag="conflict">\u26A1 \u51B2\u7A81\u68C0\u6D4B</span>
</div>
<div class="diag-content" id="diag-log">
<div class="diag-toolbar">
<button class="hdr-btn" id="diag-refresh">\u{1F504} \u5237\u65B0</button>
<button class="hdr-btn" id="diag-clear">\u{1F5D1}\uFE0F \u6E05\u7A7A\u65E5\u5FD7</button>
</div>
<div id="diag-log-list"><div class="stat-row" style="padding:12px;color:#6c7086;font-size:11px">\u6682\u65E0\u65E5\u5FD7</div></div>
</div>
<div class="diag-content" id="diag-conflict" style="display:none">
<div class="diag-toolbar">
