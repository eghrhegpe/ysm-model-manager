...
</div>`}function R(e,t){return`<div class="empty"><div class="big">${e}</div>${t}</div>`}function v(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Z(e,t){const n=v(e);if(!t)return n;const o=t.toLowerCase(),s=e.toLowerCase().indexOf(o);if(s===-1)return n;const a=v(e.substring(0,s)),i=v(e.substring(s,s+t.length)),c=v(e.substring(s+t.length));return a+"<mark>"+i+"</mark>"+c}function ee(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function te(e){if(!e)return"";const t=new Date(e),n=new Date;return t.toDateString()===n.toDateString()?t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):(n-t)/864e5<7?["\u65E5","\u4E00","\u4E8C","\u4E09","\u56DB","\u4E94","\u516D"][t.getDay()]+" "+t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):t.toLocaleDateString([],{month:"short",day:"numeric"})}function ne(e){const t=(e.split(".").pop()||"").toLowerCase();return t==="ysm"?"\u{1F48E}":["zip","rar","7z","tar","gz"].includes(t)?"\u{1F4E6}":["png","jpg","jpeg","gif","webp","bmp"].includes(t)?"\u{1F5BC}\uFE0F":["txt","md","json","xml","yml","yaml","cfg","conf","ini"].includes(t)?"\u{1F4C4}":"\u{1F9CA}"}function se(e,t,n){const o=[...e].sort((i,c)=>t==="size"?(c.size||0)-(i.size||0):t==="date"?(c.modTime||0)-(i.modTime||0):i.name.localeCompare(c.name)),s=(n||"").trim().toLowerCase(),a={};return o.forEach(i=>{if(s&&!i.name.toLowerCase().includes(s))return;const c=i.path.replace(/\\/g,"/").split("/");let d=a;for(let r=0;r<c.length-1;r++)!c[r]||(d[c[r]]||(d[c[r]]={}),d=d[c[r]]);d[c[c.length-1]]={_e:i}}),a}function G(e,t){if(!t||!e)return!1;for(const n of Object.keys(e)){const o=e[n];if(o._e){if(o._e.name.toLowerCase().includes(t))return!0}else if(n.toLowerCase().includes(t)||G(o,t))return!0}return!1}function oe(e){const t=e.length,n=e.filter(s=>!s.banned).length,o=e.reduce((s,a)=>s+(a.size||0),0);return{total:t,enabled:n,totalSize:o}}function ae(e,t,n,o){const s=w(e.path),a=w(e.fullPath||e.path),i=e.banned?"":" on",c=e.banned?"":"\u2713";return`<div class="fl${e.banned?" ban":""}" data-path="${s}" data-fullpath="${a}">
<span class="ck${i}" data-path="${s}" data-fullpath="${a}">${c}</span>
<span class="ficon">${n}</span>
<span class="nm">${t}</span>
<span class="sz">${re(e.size)}</span>${o?`<span class="dt">${o}</span>`:""}</div>`}function ie(e,t,n,o){const s=o?"\u{1F512}":"\u{1F4C1}",a=o?"#585b70":"#a6adc8",i=o?" locked":"",c=n?"\u25BC":"\u25B6",d=n?" open":"";return`<div class="fh${i}" data-dir="${w(t)}">
<span class="ar${d}">${c}</span>
<span class="nm" style="color:${a}">${s} ${w(e)}</span></div>
<div class="ch" style="display:${n?"block":"none"}">`}function w(e){return(e||"").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function re(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function ce(e,t,n,o,s){if(!t.length){e.innerHTML=R("\u{1F4C1}","\u6682\u65E0\u6A21\u578B\u6587\u4EF6");return}const a=se(t,o,n),i=j(a,"",n,o,s);if(!i){e.innerHTML=R("\u{1F50D}","\u672A\u627E\u5230\u5339\u914D\u7684\u6587\u4EF6");return}e.innerHTML=i}function de(e,t){if(!e)return;const n=oe(t);e.textContent=`\u5171 ${n.total} \u9879 (\u5DF2\u542F\u7528 ${n.enabled}) \xB7 ${ee(n.totalSize)}`}function j(e,t,n,o,s){const a=!!(n||"").trim(),i=Object.keys(e).sort((d,r)=>{const u=!e[d]._e,l=!e[r]._e;if(u&&!l)return-1;if(!u&&l)return 1;const m=e[d]._e,p=e[r]._e;return o==="size"?((p==null?void 0:p.size)||0)-((m==null?void 0:m.size)||0):o==="date"?((p==null?void 0:p.modTime)||0)-((m==null?void 0:m.modTime)||0):d.localeCompare(r)});let c="";return i.forEach(d=>{const r=e[d],u=t?t+"/"+d:d;if(r._e){const l=r._e;if(a&&!l.name.toLowerCase().includes(n.toLowerCase()))return;const m=a?Z(l.name,n):l.name,p=l.modTime?te(l.modTime):"",g=ne(l.name);c+=ae(l,m,g,p)}else{const l=d.startsWith("_"),m=a||!!s[u],p=a?G(r,n.toLowerCase()):!1;c+=ie(d,u,m||a&&p,l),c+=j(r,u,n,o,s),c+="</div>"}}),c}function L(e){!e||(e.classList.add("flash"),setTimeout(()=>e.classList.remove("flash"),400))}function le(e,t){e.querySelectorAll(".fh").forEach(n=>{n.onclick=o=>{o.stopPropagation();const s=n.nextElementSibling,a=n.querySelector(".ar");if(!s)return;const i=s.style.display!=="none";s.style.display=i?"none":"block",a.classList.toggle("open",!i),t._dirOpen[n.dataset.dir]=!i,localStorage.setItem("at_dirs",JSON.stringify(t._dirOpen))}}),e.querySelectorAll(".ck").forEach(n=>{n.onclick=o=>{o.stopPropagation();const s=n.classList.contains("on");n.classList.toggle("on"),n.textContent=n.classList.contains("on")?"\u2713":"";const a=n.closest(".fl");a&&a.classList.add("flash"),setTimeout(()=>a==null?void 0:a.classList.remove("flash"),400);const i=n.dataset.path,c=n.dataset.fullpath||i;bus.emit("entry:toggle",{path:c,relPath:i,enabled:!s})},n.ondblclick=()=>{const o=n.dataset.fullpath||n.dataset.path;o&&bus.emit("model:select",{path:o})}}),e.querySelectorAll(".fl").forEach(n=>{n.oncontextmenu=o=>{var c,d,r;o.preventDefault();const s=((d=(c=n.querySelector(".nm"))==null?void 0:c.textContent)==null?void 0:d.replace(/^\S+\s/,""))||"",a=!((r=n.querySelector(".ck"))!=null&&r.classList.contains("on")),i=n.dataset.fullpath||n.dataset.path;bus.emit("ctx:show",{x:o.clientX,y:o.clientY,path:i,relPath:n.dataset.path,name:s,banned:a})}})}function pe(e,t){var s,a,i,c,d,r,u,l,m;const n=p=>e.getElementById(p),o=()=>t._renderTree();(s=n("srch"))==null||s.addEventListener("input",p=>{t._search=p.target.value,o()}),(a=n("sort"))==null||a.addEventListener("change",p=>{t._sort=p.target.value,o()}),(i=n("btn-repo"))==null||i.addEventListener("click",()=>bus.emit("dir:select-repo")),(c=n("btn-dedup"))==null||c.addEventListener("click",()=>bus.emit("entries:dedup")),(d=n("btn-trash"))==null||d.addEventListener("click",()=>bus.emit("recycle:open")),(r=n("btn-pv"))==null||r.addEventListener("click",()=>bus.emit("preview:toggle")),(u=n("btn-ea"))==null||u.addEventListener("click",()=>{L(n("btn-ea")),t._entries.forEach(p=>{p.banned=!1}),o()}),(l=n("btn-da"))==null||l.addEventListener("click",()=>{L(n("btn-da")),t._entries.forEach(p=>{p.banned=!0}),o()}),(m=n("btn-st"))==null||m.addEventListener("click",()=>L(n("btn-st")))}function ue(e,t,n,o,s,a){return window.go.main.App.AddImportLog(e,t,n,o,s,a)}function fe(e){return window.go.main.App.AnalyzeYSMModel(e)}function me(e){return window.go.main.App.CheckFileExists(e)}function he(e){return window.go.main.App.ClearCustomDir(e)}function ge(){return window.go.main.App.ClearImportLogs()}function be(e){return window.go.main.App.CountLinkedModels(e)}function xe(e){return window.go.main.App.CreateDir(e)}function ve(e){return window.go.main.App.DeduplicateCustomDir(e)}function we(e){return window.go.main.App.DeleteFromRecycle(e)}function ye(e){return window.go.main.App.EmptyRecycleBin(e)}function _e(e){return window.go.main.App.GetGlobalCustomDir(e)}function Se(){return window.go.main.App.GetImportLogs()}function Y(e,t){return window.go.main.App.GetInstanceStatus(e,t)}function ke(){return window.go.main.App.GetLinkMode()}function Le(){return window.go.main.App.GetMinecraftPath()}function Me(){return window.go.main.App.GetWindowPosition()}function ze(e){return window.go.main.App.HasYSMMod(e)}function Ee(e,t){return window.go.main.App.ImportModelFile(e,t)}function Ce(e,t){return window.go.main.App.InstallModelFile(e,t)}function Te(e,t){return window.go.main.App.InstallModelTo(e,t)}function Ae(e,t){return window.go.main.App.InstallModelWithOverlay(e,t)}function q(e){return window.go.main.App.IsFileBanned(e)}function $e(e){return window.go.main.App.IsSymlink(e)}function Re(e){return window.go.main.App.ListFileNames(e)}function Ie(e){return window.go.main.App.ListRecycleBin(e)}function W(e){return window.go.main.App.ListVersionInstances(e)}function _(){return window.go.main.App.LoadAppConfig()}function He(e,t){return window.go.main.App.MoveModelFile(e,t)}function Be(e){return window.go.main.App.MoveToRecycle(e)}function Pe(e){return window.go.main.App.MoveToRecycleEx(e)}function De(e){return window.go.main.App.OpenFolder(e)}function Fe(e,t){return window.go.main.App.RestoreFromRecycle(e,t)}function T(e,t,n){return window.go.main.App.SaveAppConfig(e,t,n)}function Oe(e,t,n,o){return window.go.main.App.SaveWindowPosition(e,t,n,o)}function Ne(e){return window.go.main.App.ScanCustomModels(e)}function y(e){return window.go.main.App.ScanModelEntries(e)}function A(){return window.go.main.App.SelectDirectory()}function Ge(e){return window.go.main.App.SetLinkMode(e)}function je(e){return window.go.main.App.SetRepoRoot(e)}function Ye(e,t){return window.go.main.App.SyncCustomToRepo(e,t)}function qe(e,t){return window.go.main.App.SyncModelToggleStatus(e,t)}function U(e){return window.go.main.App.ToggleModelEnable(e)}const I=Object.freeze(Object.defineProperty({__proto__:null,AddImportLog:ue,AnalyzeYSMModel:fe,CheckFileExists:me,ClearCustomDir:he,ClearImportLogs:ge,CountLinkedModels:be,CreateDir:xe,DeduplicateCustomDir:ve,DeleteFromRecycle:we,EmptyRecycleBin:ye,GetGlobalCustomDir:_e,GetImportLogs:Se,GetInstanceStatus:Y,GetLinkMode:ke,GetMinecraftPath:Le,GetWindowPosition:Me,HasYSMMod:ze,ImportModelFile:Ee,InstallModelFile:Ce,InstallModelTo:Te,InstallModelWithOverlay:Ae,IsFileBanned:q,IsSymlink:$e,ListFileNames:Re,ListRecycleBin:Ie,ListVersionInstances:W,LoadAppConfig:_,MoveModelFile:He,MoveToRecycle:Be,MoveToRecycleEx:Pe,OpenFolder:De,RestoreFromRecycle:Fe,SaveAppConfig:T,SaveWindowPosition:Oe,ScanCustomModels:Ne,ScanModelEntries:y,SelectDirectory:A,SetLinkMode:Ge,SetRepoRoot:je,SyncCustomToRepo:Ye,SyncModelToggleStatus:qe,ToggleModelEnable:U},Symbol.toStringTag,{value:"Module"}));async function V(){const e=await _(),t=e.repoRoot||e.RepoRoot||"";if(!t)return null;const n=await y(t);if(!n||!n.length)return null;const o=[];for(const s of n){let a=!1;try{a=await q(s.Path)}catch{}let i=s.Path;t&&s.Path.startsWith(t)&&(i=s.Path.slice(t.length).replace(/^[/\\]+/,"")),o.push({name:s.Name,path:i,fullPath:s.Path,size:s.Size,modTime:s.ModTime,banned:a})}return{repoRoot:t,entries:o}}function h(){const e=Date.now();return[{name:"steve_skin.ysm",path:"steve_skin.ysm",size:1258291,modTime:e-864e5,banned:!1},{name:"alex_deluxe.ysm",path:"alex/alex_deluxe.ysm",size:2516582,modTime:e-1728e5,banned:!1},{name:"alex_head.ysm",path:"alex/alex_head.ysm",size:524288,modTime:e-1728e5,banned:!1},{name:"dragon_armor.zip",path:"dragon/dragon_armor.zip",size:3984588,modTime:e-36e5,banned:!1},{name:"dragon_wings.ysm",path:"dragon/dragon_wings.ysm",size:2202009,modTime:e-72e5,banned:!1},{name:"neon_sword.ysm",path:"weapons/neon_sword.ysm",size:1572864,modTime:e,banned:!1},{name:"magic_staff.zip",path:"weapons/magic_staff.zip",size:4404019,modTime:e-72e5,banned:!1},{name:"photon_body.ysm",path:"photon/photon_body.ysm",size:2202009,modTime:e-432e5,banned:!1},{name:"old_model.ysm",path:"_disabled/old_model.ysm",size:943718,modTime:e-6048e5,banned:!0},{name:"custom_hat.ysm",path:"custom/custom_hat.ysm",size:838860,modTime:e-2592e5,banned:!1},{name:"steve_2d.ysm",path:"custom/steve_2d.ysm",size:314572,modTime:e-5e5,banned:!1}]}function We(e){const t=[];return t.push(bus.on("entry:toggle",async({path:n})=>{try{await U(n)}catch{}await H(e)})),t.push(bus.on("dir:select-repo",async()=>{try{const n=await A();if(!n)return;await T(n,"","copy"),e._repoRoot=n,await H(e)}catch{e._entries=h(),e._renderTree()}})),t.push(bus.on("entries:dedup",()=>{bus.emit("toast:show",{msg:"\u{1F517} \u53BB\u91CD\u529F\u80FD\u5F00\u53D1\u4E2D",duration:2e3,type:"info"})})),t.push(bus.on("recycle:open",()=>{bus.emit("toast:show",{msg:"\u{1F5D1}\uFE0F \u56DE\u6536\u7AD9\u529F\u80FD\u5F00\u53D1\u4E2D",duration:2e3,type:"info"})})),t}async function H(e){try{const t=await V();t?(e._repoRoot=t.repoRoot,e._entries=t.entries):e._entries=h()}catch{e._entries=h()}e._renderTree()}class Ue extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(J),this._entries=[],this._search="",this._sort="name",this._dirOpen={},this._repoRoot=""}async connectedCallback(){try{Object.assign(this._dirOpen,JSON.parse(localStorage.getItem("at_dirs")||"{}"))}catch{}this._renderLayout(),pe(this._root,this),this._unsubs=We(this),await this._load(),this._renderTree()}disconnectedCallback(){var t;(t=this._unsubs)==null||t.forEach(n=>n==null?void 0:n())}async _load(){try{const t=await V();t?(this._repoRoot=t.repoRoot,this._entries=t.entries):this._entries=h()}catch{this._entries=h()}}_renderLayout(){this._root.innerHTML=X()+'<div class="list" id="tree"><div class="empty"><div class="big">\u{1F4C1}</div>\u6682\u65E0\u6A21\u578B\u6587\u4EF6</div></div>'+Q()}_renderTree(){const t=this._root.getElementById("tree");ce(t,this._entries,this._search,this._sort,this._dirOpen),le(t,this),de(this._root.getElementById("ftr-stat"),this._entries)}}customElements.define("app-tree",Ue);const Ve=`
:host {
...
</div>`}function Xe(){return'<div class="list" id="vg"></div>'}function Qe(e,t,n,o=!1){const s=[];return t>0&&s.push(`<span class="tag green">\u2705 ${t}</span>`),n>0&&s.push(`<span class="tag red">\u2B07\uFE0F ${n}</span>`),`<div class="vc-header">
<span class="${o?"arrow open":"arrow"}">\u25B6</span>
<span class="name">\u{1F4E6} ${K(e)}</span>
${s.join("")}
</div>`}function M(e,t){return`<div class="sec-title">${e} (${t})</div>`}function z(e,t,n){return`<div class="row"><span class="dot" style="background:${e}"></span><span class="rn">${K(t)}</span><span class="sz">${n}</span></div>`}function K(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Ze(e,t){e.innerHTML="",t.forEach((n,o)=>{const s=document.createElement("div");s.className="vc",s.dataset.idx=o;const a=o===0;s.innerHTML=Qe(n.name,n.synced,n.missing,a),e.appendChild(s);const i=document.createElement("div");i.className="vc-body",i.style.display=a?"":"none",i.innerHTML=et(n),e.appendChild(i)})}function et(e){let t="";return e.items.synced.length&&(t+=M("\u2705 \u5DF2\u540C\u6B65",e.items.synced.length),e.items.synced.forEach(n=>{t+=z("#a6e3a1",n.name,n.size)})),e.items.missing.length&&(t+=M("\u2B07\uFE0F \u7F3A\u5931",e.items.missing.length),e.items.missing.forEach(n=>{t+=z("#f38ba8",n.name,n.size)})),e.items.extra.length&&(t+=M("\u{1F4E4} \u989D\u5916",e.items.extra.length),e.items.extra.forEach(n=>{t+=z("#f9a826",n.name,n.size)})),t}function tt(e){e.querySelectorAll(".vc").forEach(t=>{const n=t.querySelector(".vc-header"),o=t.nextElementSibling;!n||!o||!o.classList.contains("vc-body")||(n.onclick=()=>{const s=n.querySelector(".arrow");o.style.display=o.style.display==="none"?"":"none",s&&s.classList.toggle("open")},n.ondblclick=()=>{const s=n.querySelector(".name"),a=s?s.textContent.replace(/^📦\s*/,""):"";bus.emit("instance:select",{name:a})})})}function nt(e,t){const n=e.getElementById("ver-search");n&&(n.oninput=o=>{const s=o.target.value.toLowerCase().trim();t._search=s,t._renderCards()})}function st(e){const t=e.getElementById("btn-mc");t&&(t.onclick=()=>bus.emit("dir:select-mc"))}async function ot(){const e=await _(),t=e.mcRoot||e.McRoot||"",n=e.repoRoot||e.RepoRoot||"";if(!t||!n)return null;const o=await y(n),s=new Set;o.forEach(r=>s.add(r.Name.replace(/\.ban$/i,"")));const a=await W(t);if(!a||!a.length)return null;const i=await Y(t,n),c={};return(i||[]).forEach(r=>{c[r.Name]=r}),a.map(r=>{const u=c[r.Name]||{},l=u.Missing||[],m=u.Extra||[];u.Disabled;const p=new Set(l.map(f=>f.replace(/\.ban$/i,""))),g=new Set(m.map(f=>f.replace(/\.ban$/i,""))),S=[];s.forEach(f=>{!p.has(f)&&!g.has(f)&&S.push(f)});const $=[];if(r.Exists)try{const f=y(r.CustomDir),b={};(f||[]).forEach(x=>{b[x.Name.replace(/\.ban$/i,"")]=x}),$.push(...f||[])}catch{}const k=f=>{const b=$.find(x=>x.Name.replace(/\.ban$/i,"")===f.replace(/\.ban$/i,""));return b?at(b.Size):""};return{name:r.Name,exists:r.Exists,hasYSM:u.HasYSM,synced:S.length,missing:l.length,extra:m.length,items:{synced:S.slice(0,20).map(f=>({name:f,size:k(f)})),missing:l.slice(0,20).map(f=>({name:f,size:k(f)})),extra:m.slice(0,20).map(f=>({name:f,size:k(f)}))}}})}function at(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function B(){return[{name:"\u6211\u7684\u6574\u5408\u5305",synced:3,missing:1,extra:2,items:{synced:[{name:"steve_skin.ysm",size:"1.2 MB"},{name:"alex_deluxe.ysm",size:"2.4 MB"},{name:"neon_sword.ysm",size:"1.5 MB"}],missing:[{name:"dragon_armor.zip",size:"3.8 MB"}],extra:[{name:"custom_hat.ysm",size:"0.8 MB"},{name:"old_hat.ysm",size:"0.3 MB"}]}},{name:"\u5149\u5F71\u6D4B\u8BD5\u5305",synced:1,missing:2,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"\u7A7A\u5C9B\u751F\u5B58",synced:5,missing:0,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"RPG \u5192\u9669",synced:2,missing:3,extra:0,items:{synced:[],missing:[],extra:[]}}]}class it extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(Ve),this._instances=[],this._unsubs=[],this._search=""}async connectedCallback(){this._renderLayout(),this._unsubs.push(bus.on("dir:select-mc",async()=>{try{const t=await A();if(!t)return;const n=await _();await T(n.repoRoot||"",t,n.linkMode||"copy")}catch{}await this._reload()})),this._unsubs.push(bus.on("stats:refresh",async()=>{await this._reload()})),await this._reload()}_renderCards(){const t=this._root.getElementById("vg");if(!t)return;const n=this._search,o=n?this._instances.filter(s=>s.name.toLowerCase().includes(n)):this._instances;Ze(t,o),tt(this._root)}async _reload(){try{const n=await ot();n?this._instances=n:this._instances=B()}catch{this._instances=B()}const t=this._root.getElementById("ver-stat");t&&(t.textContent=`${this._instances.length}\u4E2A\u6574\u5408\u5305`),this._renderCards(),nt(this._root,this),st(this._root)}disconnectedCallback(){this._unsubs.forEach(t=>t())}_renderLayout(){this._root.innerHTML=Ke()+Xe()+Je()}}customElements.define("app-sidebar",it);const rt="modulepreload",ct=function(e){return"/"+e},P={},D=function(t,n,o){if(!n||n.length===0)return t();const s=document.getElementsByTagName("link");return Promise.all(n.map(a=>{if(a=ct(a),a in P)return;P[a]=!0;const i=a.endsWith(".css"),c=i?'[rel="stylesheet"]':"";if(!!o)for(let u=s.length-1;u>=0;u--){const l=s[u];if(l.href===a&&(!i||l.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${a}"]${c}`))return;const r=document.createElement("link");if(r.rel=i?"stylesheet":rt,i||(r.as="script",r.crossOrigin=""),r.href=a,document.head.appendChild(r),i)return new Promise((u,l)=>{r.addEventListener("load",u),r.addEventListener("error",()=>l(new Error(`Unable to preload CSS for ${a}`)))})})).then(()=>t())},dt=`
:host {
  display: flex; flex-direction: column;
  background: #11111b;
...
</div>`}function E(e){return!e||e.hasError?`<div class="content" id="preview-content">
<h3>\u{1F4C4} \u6A21\u578B\u4FE1\u606F</h3>
...
</div>`}function C(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const pt={repo:6,ver:4,ok:2,tot:4,pending:2};function F(e,t){const n=o=>e.getElementById(o);n("s-repo")&&(n("s-repo").textContent=t.repo),n("s-ver")&&(n("s-ver").textContent=t.ver),n("s-ok")&&(n("s-ok").textContent=t.ok),n("s-tot")&&(n("s-tot").textContent=t.tot),n("s-pending")&&(n("s-pending").textContent=t.pending)}function ut(e){var t,n,o;(t=e.getElementById("btn-refresh"))==null||t.addEventListener("click",()=>bus.emit("stats:refresh")),(n=e.getElementById("btn-upload"))==null||n.addEventListener("click",()=>bus.emit("stats:upload")),(o=e.getElementById("btn-logs"))==null||o.addEventListener("click",()=>bus.emit("stats:logs"))}function ft(e,t,n){n.push(bus.on("stats:updated",o=>{o&&Object.assign(t,o),bus.emit("_preview:needs-update")}))}class mt extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(dt),this._stats={...pt},this._unsubs=[],this._mode="stat"}static get observedAttributes(){return["mode"]}attributeChangedCallback(t,n,o){t==="mode"&&(this._mode=o==="model"?"model":"stat",this._root.isConnected&&this._render())}connectedCallback(){this._mode=this.getAttribute("mode")==="model"?"model":"stat",this._render(),this._mode==="stat"&&(ft(this._root,this._stats,this._unsubs),this._unsubs.push(bus.on("_preview:needs-update",()=>{F(this._root,this._stats)})),this._unsubs.push(bus.on("stats:refresh",()=>{this._loadRealStats()})),this._loadRealStats()),this._mode==="model"&&this._unsubs.push(bus.on("model:select",async({path:t})=>{this._showModelDetail(t)}))}disconnectedCallback(){this._unsubs.forEach(t=>t())}_render(){this._mode==="stat"?(this._root.innerHTML=lt(),ut(this._root)):this._root.innerHTML=E(null)}async _loadRealStats(){try{const{LoadAppConfig:t,GetInstanceStatus:n,ScanModelEntries:o}=await D(()=>Promise.resolve().then(()=>I),void 0),s=await t(),a=s.mcRoot||s.McRoot||"",i=s.repoRoot||s.RepoRoot||"",c=await o(i),d=c?c.length:0;let r=0,u=0,l=0;if(a&&i){const m=await n(a,i);m&&(r=m.length,m.forEach(p=>{p.Status==="complete"&&u++,(p.Status==="extra"||p.Status==="missing")&&l++}))}this._stats={repo:d,ver:r,ok:u,tot:r,pending:l},F(this._root,this._stats)}catch{}}async _showModelDetail(t){this._root.innerHTML='<div class="content" id="preview-content"><h3>\u{1F4C4} \u6A21\u578B\u4FE1\u606F</h3><div class="stat-row"><span class="label">\u52A0\u8F7D\u4E2D...</span></div></div>';try{const{AnalyzeYSMModel:n}=await D(()=>Promise.resolve().then(()=>I),void 0),o=await n(t);this._root.innerHTML=E(o)}catch(n){this._root.innerHTML=E({hasError:!0,errorMsg:String(n)})}}}customElements.define("app-preview",mt);const ht=`
...
`;function O(){return`<div style="flex:1;display:flex;flex-direction:column;overflow-y:auto">
