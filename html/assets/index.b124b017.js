...
</div>`}function O(e,t){return`<div class="placeholder-box"><div class="big">${e}</div><div>${t}</div><span class="ptag">\u9884\u544A</span></div>`}let M,C,z;class vt extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(ht),this._current="dashboard"}connectedCallback(){this._unsub=bus.on("nav:change",({page:t})=>{this._current=t,bus.emit("nav:changed",{page:t}),this._render()}),bus.on("stats:upload",()=>this._handleUpload()),E(()=>Promise.resolve().then(()=>T),void 0).then(t=>{M=t.LoadAppConfig,C=t.GetInstanceStatus,z=t.SyncCustomToRepo}),this._render()}disconnectedCallback(){this._unsub&&this._unsub()}async _handleUpload(){if(!(!M||!C||!z))try{const t=await M(),n=t.mcRoot||t.McRoot||"",o=t.repoRoot||t.RepoRoot||"";if(!n||!o)return;const s=await C(n,o);if(!s||!s.length)return;let a=0;for(const i of s){if(!i.Extra||i.Extra.length===0)continue;const r=await z(i.CustomDir,o);a+=r}bus.emit("stats:refresh")}catch{}}_render(){let t="";switch(this._current){case"dashboard":t=F();break;case"repository":t=gt();break;case"instances":t=bt();break;case"downloads":t=O("\u2B07\uFE0F","\u4E0B\u8F7D\u4E0E\u66F4\u65B0");break;case"diagnostics":t=O("\u{1F6E0}\uFE0F","\u8BCA\u65AD\u4E0E\u51B2\u7A81\u68C0\u6D4B");break;case"settings":t=xt();break;default:t=F()}this._root.innerHTML=`<div class="page">${t}</div>`}}customElements.define("app-content",vt);bus.on("ctx:show",({x:e,y:t,type:n,instanceName:o,path:s,name:a,banned:i})=>{if(n==="instance"){bus.emit("menu:show",{x:e,y:t,items:[{label:`\u{1F4E6} ${o}`,icon:"\u{1F4CC}",onClick:()=>bus.emit("instance:select",{name:o})},{divider:!0},{label:"\u{1F4E5} \u5B89\u88C5\u6A21\u578B",icon:"\u{1F4E5}",onClick:()=>bus.emit("instance:install",{name:o})},{label:"\u{1F504} \u540C\u6B65\u72B6\u6001",icon:"\u{1F504}",onClick:()=>bus.emit("instance:sync",{name:o})},{label:"\u{1F5D1}\uFE0F \u6E05\u7A7A\u76EE\u5F55",icon:"\u{1F5D1}\uFE0F",danger:!0,onClick:()=>bus.emit("instance:clear",{name:o})}]});return}});
