...
</div>`}function y(e,t){return`<div class="empty"><div class="big">${e}</div>${t}</div>`}function b(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function M(e,t){const n=b(e);if(!t)return n;const s=t.toLowerCase(),a=e.toLowerCase().indexOf(s);if(a===-1)return n;const o=b(e.substring(0,a)),r=b(e.substring(a,a+t.length)),i=b(e.substring(a+t.length));return o+"<mark>"+r+"</mark>"+i}function $(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function H(e){if(!e)return"";const t=new Date(e),n=new Date;return t.toDateString()===n.toDateString()?t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):(n-t)/864e5<7?["\u65E5","\u4E00","\u4E8C","\u4E09","\u56DB","\u4E94","\u516D"][t.getDay()]+" "+t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):t.toLocaleDateString([],{month:"short",day:"numeric"})}function B(e){const t=(e.split(".").pop()||"").toLowerCase();return t==="ysm"?"\u{1F48E}":["zip","rar","7z","tar","gz"].includes(t)?"\u{1F4E6}":["png","jpg","jpeg","gif","webp","bmp"].includes(t)?"\u{1F5BC}\uFE0F":["txt","md","json","xml","yml","yaml","cfg","conf","ini"].includes(t)?"\u{1F4C4}":"\u{1F9CA}"}function D(e,t,n){const s=[...e].sort((r,i)=>t==="size"?(i.size||0)-(r.size||0):t==="date"?(i.modTime||0)-(r.modTime||0):r.name.localeCompare(i.name)),a=(n||"").trim().toLowerCase(),o={};return s.forEach(r=>{if(a&&!r.name.toLowerCase().includes(a))return;const i=r.path.replace(/\\/g,"/").split("/");let d=o;for(let l=0;l<i.length-1;l++)!i[l]||(d[i[l]]||(d[i[l]]={}),d=d[i[l]]);d[i[i.length-1]]={_e:r}}),o}function S(e,t){if(!t||!e)return!1;for(const n of Object.keys(e)){const s=e[n];if(s._e){if(s._e.name.toLowerCase().includes(t))return!0}else if(n.toLowerCase().includes(t)||S(s,t))return!0}return!1}function I(e){const t=e.length,n=e.filter(a=>!a.banned).length,s=e.reduce((a,o)=>a+(o.size||0),0);return{total:t,enabled:n,totalSize:s}}function O(e,t,n,s){const a=x(e.path),o=e.banned?"":" on",r=e.banned?"":"\u2713";return`<div class="fl${e.banned?" ban":""}" data-path="${a}">
<span class="ck${o}" data-path="${a}">${r}</span>
<span class="ficon">${n}</span>
<span class="nm">${t}</span>
<span class="sz">${F(e.size)}</span>${s?`<span class="dt">${s}</span>`:""}</div>`}function A(e,t,n,s){const a=s?"\u{1F512}":"\u{1F4C1}",o=s?"#585b70":"#a6adc8",r=s?" locked":"",i=n?"\u25BC":"\u25B6",d=n?" open":"";return`<div class="fh${r}" data-dir="${x(t)}">
<span class="ar${d}">${i}</span>
<span class="nm" style="color:${o}">${a} ${x(e)}</span></div>
<div class="ch" style="display:${n?"block":"none"}">`}function x(e){return(e||"").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function F(e){return!e&&e!==0?"":e<1024?e+" B":e<1048576?(e/1024).toFixed(1)+" KB":(e/1048576).toFixed(1)+" MB"}function j(e,t,n,s,a){if(!t.length){e.innerHTML=y("\u{1F4C1}","\u6682\u65E0\u6A21\u578B\u6587\u4EF6");return}const o=D(t,s,n),r=z(o,"",n,s,a);if(!r){e.innerHTML=y("\u{1F50D}","\u672A\u627E\u5230\u5339\u914D\u7684\u6587\u4EF6");return}e.innerHTML=r}function N(e,t){if(!e)return;const n=I(t);e.textContent=`\u5171 ${n.total} \u9879 (\u5DF2\u542F\u7528 ${n.enabled}) \xB7 ${$(n.totalSize)}`}function z(e,t,n,s,a){const o=!!(n||"").trim(),r=Object.keys(e).sort((d,l)=>{const u=!e[d]._e,p=!e[l]._e;if(u&&!p)return-1;if(!u&&p)return 1;const f=e[d]._e,c=e[l]._e;return s==="size"?((c==null?void 0:c.size)||0)-((f==null?void 0:f.size)||0):s==="date"?((c==null?void 0:c.modTime)||0)-((f==null?void 0:f.modTime)||0):d.localeCompare(l)});let i="";return r.forEach(d=>{const l=e[d],u=t?t+"/"+d:d;if(l._e){const p=l._e;if(o&&!p.name.toLowerCase().includes(n.toLowerCase()))return;const f=o?M(p.name,n):p.name,c=p.modTime?H(p.modTime):"",v=B(p.name);i+=O(p,f,v,c)}else{const p=d.startsWith("_"),f=o||!!a[u],c=o?S(l,n.toLowerCase()):!1;i+=A(d,u,f||o&&c,p),i+=z(l,u,n,s,a),i+="</div>"}}),i}function h(e){!e||(e.classList.add("flash"),setTimeout(()=>e.classList.remove("flash"),400))}function P(e,t){e.querySelectorAll(".fh").forEach(n=>{n.onclick=s=>{s.stopPropagation();const a=n.nextElementSibling,o=n.querySelector(".ar");if(!a)return;const r=a.style.display!=="none";a.style.display=r?"none":"block",o.classList.toggle("open",!r),t._dirOpen[n.dataset.dir]=!r,localStorage.setItem("at_dirs",JSON.stringify(t._dirOpen))}}),e.querySelectorAll(".ck").forEach(n=>{n.onclick=s=>{s.stopPropagation();const a=n.classList.contains("on");n.classList.toggle("on"),n.textContent=n.classList.contains("on")?"\u2713":"";const o=n.closest(".fl");o&&o.classList.add("flash"),setTimeout(()=>o==null?void 0:o.classList.remove("flash"),400),bus.emit("entry:toggle",{path:n.dataset.path,enabled:!a})}}),e.querySelectorAll(".fl").forEach(n=>{n.oncontextmenu=s=>{var r,i,d;s.preventDefault();const a=((i=(r=n.querySelector(".nm"))==null?void 0:r.textContent)==null?void 0:i.replace(/^\S+\s/,""))||"",o=!((d=n.querySelector(".ck"))!=null&&d.classList.contains("on"));bus.emit("ctx:show",{x:s.clientX,y:s.clientY,path:n.dataset.path,name:a,banned:o})}})}function q(e,t){var a,o,r,i,d,l,u,p,f;const n=c=>e.getElementById(c),s=()=>t._renderTree();(a=n("srch"))==null||a.addEventListener("input",c=>{t._search=c.target.value,s()}),(o=n("sort"))==null||o.addEventListener("change",c=>{t._sort=c.target.value,s()}),(r=n("btn-repo"))==null||r.addEventListener("click",()=>bus.emit("dir:select-repo")),(i=n("btn-dedup"))==null||i.addEventListener("click",()=>bus.emit("entries:dedup")),(d=n("btn-trash"))==null||d.addEventListener("click",()=>bus.emit("recycle:open")),(l=n("btn-pv"))==null||l.addEventListener("click",()=>bus.emit("preview:toggle")),(u=n("btn-ea"))==null||u.addEventListener("click",()=>{h(n("btn-ea")),t._entries.forEach(c=>{c.banned=!1}),s()}),(p=n("btn-da"))==null||p.addEventListener("click",()=>{h(n("btn-da")),t._entries.forEach(c=>{c.banned=!0}),s()}),(f=n("btn-st"))==null||f.addEventListener("click",()=>h(n("btn-st")))}class R extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(T),this._entries=[],this._search="",this._sort="name",this._dirOpen={}}connectedCallback(){this._entries=[{name:"steve_skin.ysm",path:"steve_skin.ysm",size:1258291,modTime:Date.now()-864e5,banned:!1},{name:"alex_deluxe.ysm",path:"alex/alex_deluxe.ysm",size:2516582,modTime:Date.now()-1728e5,banned:!1},{name:"alex_head.ysm",path:"alex/alex_head.ysm",size:524288,modTime:Date.now()-1728e5,banned:!1},{name:"dragon_armor.zip",path:"dragon/dragon_armor.zip",size:3984588,modTime:Date.now()-36e5,banned:!1},{name:"dragon_wings.ysm",path:"dragon/dragon_wings.ysm",size:2202009,modTime:Date.now()-72e5,banned:!1},{name:"neon_sword.ysm",path:"weapons/neon_sword.ysm",size:1572864,modTime:Date.now(),banned:!1},{name:"magic_staff.zip",path:"weapons/magic_staff.zip",size:4404019,modTime:Date.now()-72e5,banned:!1},{name:"photon_body.ysm",path:"photon/photon_body.ysm",size:2202009,modTime:Date.now()-432e5,banned:!1},{name:"old_model.ysm",path:"_disabled/old_model.ysm",size:943718,modTime:Date.now()-6048e5,banned:!0},{name:"custom_hat.ysm",path:"custom/custom_hat.ysm",size:838860,modTime:Date.now()-2592e5,banned:!1},{name:"steve_2d.ysm",path:"custom/steve_2d.ysm",size:314572,modTime:Date.now()-5e5,banned:!1}];try{Object.assign(this._dirOpen,JSON.parse(localStorage.getItem("at_dirs")||"{}"))}catch{}this._renderLayout(),q(this._root,this),this._renderTree()}disconnectedCallback(){}_renderLayout(){this._root.innerHTML=E()+'<div class="list" id="tree"><div class="empty"><div class="big">\u{1F4C1}</div>\u6682\u65E0\u6A21\u578B\u6587\u4EF6</div></div>'+C()}_renderTree(){const t=this._root.getElementById("tree");j(t,this._entries,this._search,this._sort,this._dirOpen),P(t,this),N(this._root.getElementById("ftr-stat"),this._entries)}}customElements.define("app-tree",R);const U=`
:host {
...
`;function Y(){return`<div class="header">
...
</div>`}function K(){return`<div class="footer">
<button class="footer-btn" id="btn-mc">\u{1F3AE} \u6307\u5B9A\u6E38\u620F\u8DEF\u5F84</button>
</div>`}function J(){return'<div class="list" id="vg"></div>'}function G(e,t,n){const s=[];return t>0&&s.push(`<span class="tag green">\u2705 ${t}</span>`),n>0&&s.push(`<span class="tag red">\u2B07\uFE0F ${n}</span>`),`<div class="vc-header">
<span class="arrow">\u25B6</span>
<span class="name">\u{1F4E6} ${L(e)}</span>
${s.join("")}
</div>`}function g(e,t){return`<div class="sec-title">${e} (${t})</div>`}function m(e,t,n){return`<div class="row"><span class="dot" style="background:${e}"></span><span class="rn">${L(t)}</span><span class="sz">${n}</span></div>`}function L(e){return(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const V=[{name:"\u6211\u7684\u6574\u5408\u5305",synced:3,missing:1,extra:2,items:{synced:[{name:"steve_skin.ysm",size:"1.2 MB"},{name:"alex_deluxe.ysm",size:"2.4 MB"},{name:"neon_sword.ysm",size:"1.5 MB"}],missing:[{name:"dragon_armor.zip",size:"3.8 MB"}],extra:[{name:"custom_hat.ysm",size:"0.8 MB"},{name:"old_hat.ysm",size:"0.3 MB"}]}},{name:"\u5149\u5F71\u6D4B\u8BD5\u5305",synced:1,missing:2,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"\u7A7A\u5C9B\u751F\u5B58",synced:5,missing:0,extra:0,items:{synced:[],missing:[],extra:[]}},{name:"RPG \u5192\u9669",synced:2,missing:3,extra:0,items:{synced:[],missing:[],extra:[]}}];function W(e,t){e.innerHTML="",t.forEach((n,s)=>{const a=document.createElement("div");a.className="vc",a.dataset.idx=s,a.innerHTML=G(n.name,n.synced,n.missing),e.appendChild(a);const o=document.createElement("div");o.className="vc-body",o.style.display=s===0?"":"none",o.innerHTML=X(n),e.appendChild(o)})}function X(e){let t="";return e.items.synced.length&&(t+=g("\u2705 \u5DF2\u540C\u6B65",e.items.synced.length),e.items.synced.forEach(n=>{t+=m("#a6e3a1",n.name,n.size)})),e.items.missing.length&&(t+=g("\u2B07\uFE0F \u7F3A\u5931",e.items.missing.length),e.items.missing.forEach(n=>{t+=m("#f38ba8",n.name,n.size)})),e.items.extra.length&&(t+=g("\u{1F4E4} \u989D\u5916",e.items.extra.length),e.items.extra.forEach(n=>{t+=m("#f9a826",n.name,n.size)})),t}function Q(e){e.querySelectorAll(".vc-header").forEach(t=>{t.onclick=()=>{const n=t.nextElementSibling,s=t.querySelector(".arrow");n&&n.classList.contains("vc-body")&&(n.style.display=n.style.display==="none"?"":"none",s.classList.toggle("open"))}})}function Z(e){const t=e.getElementById("ver-search");t&&(t.oninput=n=>{bus.emit("ver:search",{keyword:n.target.value})})}function ee(e){const t=e.getElementById("btn-mc");t&&(t.onclick=()=>bus.emit("dir:select-mc"))}function te(e,t){t.push(bus.on("versions:updated",({instances:n})=>{const s=e.getElementById("ver-stat");s&&(s.textContent=`${n.length}\u4E2A\u6574\u5408\u5305`)}))}class ne extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(U),this._instances=V,this._unsubs=[]}connectedCallback(){this._renderLayout(),W(this._root.getElementById("vg"),this._instances),Q(this._root),Z(this._root),ee(this._root),te(this._root,this._unsubs)}disconnectedCallback(){this._unsubs.forEach(t=>t())}_renderLayout(){this._root.innerHTML=Y()+J()+K()}}customElements.define("app-sidebar",ne);const se=`
:host {
  display: flex; flex-direction: column;
  background: #11111b;
...
`;function ae(){return`<div class="content">
<h3>\u{1F4CA} \u7EDF\u8BA1</h3>
...
</div>`}const oe={repo:6,ver:4,ok:2,tot:4,pending:2};function w(e,t){const n=s=>e.getElementById(s);n("s-repo")&&(n("s-repo").textContent=t.repo),n("s-ver")&&(n("s-ver").textContent=t.ver),n("s-ok")&&(n("s-ok").textContent=t.ok),n("s-tot")&&(n("s-tot").textContent=t.tot),n("s-pending")&&(n("s-pending").textContent=t.pending)}function re(e){var t,n,s;(t=e.getElementById("btn-refresh"))==null||t.addEventListener("click",()=>bus.emit("stats:refresh")),(n=e.getElementById("btn-upload"))==null||n.addEventListener("click",()=>bus.emit("stats:upload")),(s=e.getElementById("btn-logs"))==null||s.addEventListener("click",()=>bus.emit("stats:logs"))}function ie(e,t,n){n.push(bus.on("stats:updated",s=>{s&&Object.assign(t,s),bus.emit("_preview:needs-update")}))}class de extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(se),this._stats={...oe},this._unsubs=[]}connectedCallback(){this._renderLayout(),re(this._root),ie(this._root,this._stats,this._unsubs),this._unsubs.push(bus.on("_preview:needs-update",()=>{w(this._root,this._stats)})),w(this._root,this._stats)}disconnectedCallback(){this._unsubs.forEach(t=>t())}_renderLayout(){this._root.innerHTML=ae()}}customElements.define("app-preview",de);const ce=`
...
</div>`}function le(){return'<div class="repo-layout"><app-tree></app-tree></div>'}function pe(){return'<div class="repo-layout"><app-sidebar></app-sidebar><app-preview></app-preview></div>'}function fe(){return`<div style="flex:1;overflow-y:auto">
...
</div>`}function k(e,t){return`<div class="placeholder-box"><div class="big">${e}</div><div>${t}</div><span class="ptag">\u9884\u544A</span></div>`}class ue extends HTMLElement{constructor(){super(),this._root=this.attachShadow({mode:"open"}),this._root.adoptedStyleSheets=[new CSSStyleSheet],this._root.adoptedStyleSheets[0].replaceSync(ce),this._current="dashboard"}connectedCallback(){this._unsub=bus.on("nav:change",({page:t})=>{this._current=t,bus.emit("nav:changed",{page:t}),this._render()}),this._render()}disconnectedCallback(){this._unsub&&this._unsub()}_render(){let t="";switch(this._current){case"dashboard":t=_();break;case"repository":t=le();break;case"instances":t=pe();break;case"downloads":t=k("\u2B07\uFE0F","\u4E0B\u8F7D\u4E0E\u66F4\u65B0");break;case"diagnostics":t=k("\u{1F6E0}\uFE0F","\u8BCA\u65AD\u4E0E\u51B2\u7A81\u68C0\u6D4B");break;case"settings":t=fe();break;default:t=_()}this._root.innerHTML=`<div class="page">${t}</div>`}}customElements.define("app-content",ue);
