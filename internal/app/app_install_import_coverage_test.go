// ===== DetectZipType / importModelFile 守卫 / SaveCachedTexture 补测 =====
...
// TestDetectZipType_TailProbeAndFallback 覆盖 DetectZipType 尾部探针命中 /
// 整包兜底无特征 / 解码失败 → unknown 三态（9e0b72c2 audit #1 口径）。
func TestDetectZipType_TailProbeAndFallback(t *testing.T) {
	a, _ := guardedApp(t)

	// 合法 zip（含 ysm.json）→ 尾部探针命中，返回非空类型
	okZip := buildZipB64T(t, map[string]string{"ysm.json": "{}", "model/body.ysm": "x"})
	if got := a.DetectZipType(okZip); got == "" {
	...
	if got := a.DetectZipType(garbage); got != "" {
		t.Errorf("非 zip 应返回空串, got %q", got)
	}

	// 非法 base64 → DecodeBase64Limited 失败 → unknown
	if got := a.DetectZipType("not-base64!!"); got != "unknown" {
		t.Errorf("非法 base64 应返回 unknown, got %q", got)
	}
}

// TestImportModelFile_TraversalRejected 覆盖 importModelFileWithOptions 透传
// ImportFromBase64 的路径穿越守卫（HasTraversal 收敛口径）：拒绝且不落盘。
func TestImportModelFile_TraversalRejected(t *testing.T) {
	a, _ := guardedApp(t)
	if err := a.ImportModelFileTo("../evil.ysm", "", base64.StdEncoding.EncodeToString([]byte("x"))); err == nil {
		t.Error("路径穿越文件名应被拒绝")
	}
}
