...
| `check-android-unavailable.ts` | `node scripts/check-android-unavailable.ts` / `--json`                                                            | **Android 平台黑名单完整性检测**：读 `platform-web.ts` 的 `ANDROID_UNAVAILABLE` 黑名单 vs `bindings/app.ts` 全部 binding，发现未覆盖的 desktop-only binding 即报（新增 Go desktop binding 时提醒前端同步）。零依赖，bindings 未生成自动跳过。已挂 pre-commit |
...
| `check-biome-lines.ts`     | `node scripts/check-biome-lines.ts`                                                                               | **Biome 行级增量闸**（2026-09-04，A4）：检 staged frontend TS/TSX，「staged diff 新增行 ∩ biome 违规行」交集非空即 exit 1——只拦本次提交真正引入的违规，存量债永不误伤。补 check-biome.ts 结构性盲区（`--changed` 基准 main，main 直提工作流恒空；换 origin/main 会炸 885 条存量债）。已挂 pre-commit 为唯一 exit-1 阻断段，逃生阀 `YSM_SKIP_BIOME_LINES=1`                                                                                |
