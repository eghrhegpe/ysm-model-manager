...
import { getList, getScalar, parseFrontmatter, parseSourceFiles } from "../_lib/frontmatter.ts";
import { CATEGORY_LABELS, KNOW_DIR, KNOWLEDGE_NON_CARDS } from "../_lib/knowledge-cards.ts";
...
function resolveSource(p: string, _kind: string) {
  const full = path.join(ROOT, p);
  if (fs.existsSync(full)) return { p, kind: "file", depth: pathDepth(p) };
  if (fs.existsSync(full) && fs.statSync(full).isDirectory())
    return { p, kind: "dir", depth: pathDepth(p) };
...
interface AnalyzeData {
  ...
  byCategory: Record<
    string,
    { count: number; sourceFiles: number; maxDepth: number; kinds: string[]; label: string }
  >;
  reverseRefs: Array<{
    path: string;
    cardCount: number;
...
function analyze(cards: Card[]): AnalyzeData {
  ...
  const perCard = cards.map((c) => {
    ...
    return {
      ...
      avgDepth: okRefs.length
        ? +(okRefs.reduce((s, r) => s + r.depth, 0) / okRefs.length).toFixed(1)
        : 0,
  ...
  for (const c of cards) {
    ...
    byCategory[cat].maxDepth = Math.max(
      byCategory[cat].maxDepth,
      perCard.find((p) => p.kind === c.kind)?.maxDepth,
    );
  ...
  const noIn = cards
    .filter(
      (c) => (crossLinks[c.kind]?.inCount || 0) === 0 && (crossLinks[c.kind]?.outCount || 0) === 0,
    )
    .map((c) => c.kind);
...
function render(_cards: Card[], data: AnalyzeData) {
  ...
  L.push("");
  L.push(
    `- **零源码引用卡**（${data.islands.noSource.length} 张）：${data.islands.noSource.length ? data.islands.noSource.map((k) => `\`${k}\``).join(", ") : "无"}`,
  );
  L.push(
    `- **零互链卡**（${data.islands.noIn.length} 张）：${data.islands.noIn.length ? data.islands.noIn.map((k) => `\`${k}\``).join(", ") : "无"}`,
  );
  L.push("");
  ...
  for (const p of data.perCard) {
    L.push(
      `### \`${p.kind}\`（${p.category}，引用 ${p.sourceCount}/${p.okCount}，最深 ${p.maxDepth} 层）`,
    );
    L.push("");
  ...
  if (p3dDeep > 0) {
    L.push(
      `1. **\`preview-3d/\` 尚有 ${p3dDeep} 个 ≥5 层引用路径**（ADR-138 已整体上提为 \`src/preview-3d\`）。`,
    );
  ...
  L.push(
    "5. 移动任何源码前，先跑 `node scripts/check-knowledge-drift.ts --affected <新路径>` 验证卡面",
  );
...
function main() {
  ...
  if (!NO_WRITE) {
    ...
    console.log(
      `✅ 分析完成：${data.summary.cardCount} 张卡 / ${data.summary.totalOkRefs} 命中引用 / ${data.summary.totalMissingRefs} 缺失`,
    );
