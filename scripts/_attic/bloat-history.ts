 ...
 *   node scripts/bloat-history.ts <path> --limit 60           # 增加历史深度
 *   node scripts/bloat-history.ts <path> --limit 30 --first N # 只看最近 N 条记录
 *   node scripts/bloat-history.ts <path> --json               # JSON（供子代理/CI 消费）
 *
 * 退出码：0（无论有无跳点）。情报型工具，不阻断任何流程。
 */
import { logPathDetail, showAt } from "../_lib/git-ref.ts";
...
function human(report: any, firstN: number | null) {
  ...
  L.push("① 首末快照");
  L.push(
    `   首次出现  ${first.commit.date}  ${first.commit.short}  ${first.thisSig.lines || "-"}行 ${first.thisSig.exports}导出/${first.thisSig.tops}顶层`,
  );
  L.push(
    `   当前     ${last.commit.date}  ${last.commit.short}  ${last.thisSig.lines || "-"}行 ${last.thisSig.exports}导出/${last.thisSig.tops}顶层`,
  );
  ...
  if (!jumps.length) {
    ...
    for (const j of jumps) {
      ...
      L.push(
        `       ${r.prevSig.lines}行→${r.thisSig.lines}行  导出 ${r.prevSig.exports}→${r.thisSig.exports}  顶层 ${r.prevSig.tops}→${r.thisSig.tops}`,
      );
    }
  }
  ...
  for (const r of records) {
    ...
    L.push(
      `   ${r.commit.date}  ${r.commit.short}  ${deltaTag.padStart(6)}行  ${r.thisSig.lines || "-"}  ${r.commit.author}`,
    );
  }
  return L.join("\n");
}
...
if (args.help) {
  console.log("用法: node scripts/bloat-history.ts <path> [--json|--limit N|--first N]");
  process.exit(0);
}
...
if (!pathArg) {
  console.error("用法: node scripts/bloat-history.ts <path> [--json|--limit N|--first N]");
  process.exit(2);
}
