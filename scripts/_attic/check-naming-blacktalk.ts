...
const BUILT_NOUN_RE =
  /\b(?:setBuilt|workerBuilt|builtScene|builtContent|allBuilt|getBuilt|makeBuilt|unloadBuilt|built\d+)\b/g;

/** 1b. 裸 built 使用（非注释）：built 作场景对象变量名（声明/属性访问/比较）。含 built 的
 *     注释历史与 i18n 英文（built-in/not built yet）不在此列——本正则要求词边界后的
 *     赋值/点访问/空值合并/参数语境。 */
const BUILT_NAKED_RE =
  /\bbuilt\s*(?:=|\?\.|\.(?:dispose|menuItems|update|roots|boneMaps)|\)\s*=>)/g;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (code && !/^(\/\/|\*|\/\*)/.test(code)) {
      ...
      for (const m of code.matchAll(BUILT_NAKED_RE)) {
        findings.push({
          name: `built${m[0].replace(/\s+/g, " ").replace(/^built/, "")}`,
          loc,
          kind: "builtNoun",
          line: code.slice(0, 120),
        });
      }
    }
...
function main() {
  ...
  for (const f of findings) {
    const key =
      f.kind === "whlTriple" ? `${f.kind}@${f.loc}` : `${f.kind}@${f.name}@${f.loc.split(":")[0]}`;
      ...
      JSON.stringify(
        {
          _summary: {
            ...
            builtNoun: builtResults.length,
            whlTriple: whlResults.length,
          },
          findings: results,
          scanned: scannedCount,
          strict: STRICT,
  ...
  for (const r of results) {
    if (r.kind === "builtNoun") {
      ...
      console.log(
        `  ⚠️ ${r.loc}  w/h/l 三轴单字母挤一行——展开为 width/height/length（可辩护例外：循环下标）`,
      );
    }
  }
  ...
  console.log(
    builtResults.length || whlResults.length
      ? "\n（默认不阻断；--strict 将 built 名词升级为 ERROR）"
      : "✅ 无 built 名词黑话 + 无三轴单字母挤一行。",
  );
}
