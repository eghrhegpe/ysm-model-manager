#!/usr/bin/env node
/**
 * gen-guide-gap.ts — 用户指南覆盖缺口扫描（适配自 MikuMikuAR）。
 *
 ...
 *
 * 用法：
 *   node scripts/gen-guide-gap.ts            # 扫描并输出缺口清单
 *   node scripts/gen-guide-gap.ts --strict   # 有缺口时 exit 1（CI 可选卡点）
 *
...
function main() {
  ...
  const guidePages = fs
    ...
    .filter(
      (f) =>
        f.endsWith(".md") && !["README.md", "index.md", "用户指南.md", "项目意义.md"].includes(f),
    )
    .map((f) => f.replace(/\.md$/, ""));
