#!/usr/bin/env node
/**
 * line-counter.ts — 代码行数统计与文件健康度分析（文件级 + 函数级双粒度）。
 *
 ...
 *
 * 用法：
 *   node scripts/line-counter.ts                                    # 默认：文件级总览（不变）
 *   node scripts/line-counter.ts --funcs                            # 新增：函数级三档统计（frontend/src + go/）
 *   node scripts/line-counter.ts --funcs --scope frontend/src/utils # 新增：限定扫描目录
 *   node scripts/line-counter.ts --funcs --threshold 80             # 新增：自定义黄档阈值（橙=2x / 红=3x）
 *   node scripts/line-counter.ts --funcs --json                     # 新增：JSON（子代理/CI 消费）
 *
...
const TS_FUNCTION_DECL_RE =
  /^(?<indent>[ \t]*)(?:export\s+)?(?:declare\s+)?(?:async\s+)?function\s+(?<name>[A-Za-z0-9_$]+)\s*(?:<[^>]*>)?\s*\(/;
const TS_CLASS_DECL_RE =
  /^(?<indent>[ \t]*)(?:export\s+)?(?:declare\s+)?(?:abstract\s+)?class\s+(?<name>[A-Za-z0-9_$]+)\s*(?:<[^>]*>)?\s*(?:extends\s+[^{]+?|implements\s+[^{]+?)?\s*\{/;
const TS_ARROW_DECL_RE =
  /^(?<indent>[ \t]*)(?:export\s+)?(?:declare\s+)?(?:const|let|var)\s+(?<name>[A-Za-z0-9_$]+)\s*(?::\s*[^=]+?)?\s*=\s*(?:async\s+)?(?:\([^)]*\)|[A-Za-z0-9_$]+)\s*=>\s*\{/;
const TS_CLASS_METHOD_RE =
  /^(?<indent>[ \t]*)(?:(?:public|private|protected|static|readonly|async)\s+)*(?<name>[A-Za-z0-9_$]+)\s*(?:<[^>]*>)?\s*\([^)]*\)\s*(?::\s*[^{]+?)?\s*\{/;
const TS_INTERFACE_TYPE_RE =
  /^(?<indent>[ \t]*)(?:export\s+)?(?:interface|type|enum)\s+(?<name>[A-Za-z0-9_$]+)\s*(?:<[^>]*>)?\s*(?:extends\s+[^{]+?|implements\s+[^{]+?)?\s*\{/;
...
//   func (r *Receiver) foo(
const GO_FUNC_RE =
  /^(?<indent>[ \t]*)func\s+(?:\((?<recv>[^)]*)\)\s+)?(?<name>[A-Za-z0-9_]+)\s*(?:\[[^\]]*\])?\s*\(/;
...
function findFunctionEnd(lines: string[], startLine: number, isGo: boolean) {
  ...
  let inSingle = false,
    ...
    inBacktick = false,
    inLineComment = false;
  outer: for (let li = startLine; li < n; li++) {
    const line = lines[li]!;
    inLineComment = false;
  ...
  for (let li = braceLine; li < n; li++) {
    ...
    if (li + 1 < n) {
      ...
      const hitTs =
        !isGo &&
        (TS_FUNCTION_DECL_RE.test(nextLine) ||
          TS_CLASS_DECL_RE.test(nextLine) ||
          TS_ARROW_DECL_RE.test(nextLine) ||
          TS_INTERFACE_TYPE_RE.test(nextLine));
...
function extractFunctions(file: string) {
  ...
  try {
    ...
    console.warn(`[line-counter] 跳过 ${relPosix(file)}: ${(e as Error).message}`);
    return [];
  }
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    if (isGo) {
      ...
      if (mFunc || mClass || mArrow || mIface) {
        ...
        if (!name) continue; // 理论不可达：四个正则均强制 name 命名组
        let kind = "func";
        if (mFunc) kind = "func";
...
function runFuncsMode(args: any) {
  ...
  const items: {
    ...
    tier: string;
    truncated: boolean;
  }[] = []; // { file, rel, name, kind?, startLine, lines, tier, truncated }
  const stats = {
    totalFiles: 0,
    totalFuncs: 0,
    red: 0,
  ...
  for (const root of roots) {
    if (!fs.existsSync(root)) {
      console.warn(`[line-counter] --funcs 目录不存在：${relPosix(root) || root}`);
      continue;
    }
    ...
    const files = walk(root, { exts, skipTest: false }).filter((f) => {
      if (isTestFile(f as string)) {
        stats.skippedTest++;
        return false;
      }
      if (isGeneratedFile(f as string)) {
  ...
  items.sort(
    (a, b) =>
      (TIER_ORDER as Record<string, number>)[a.tier]! -
        (TIER_ORDER as Record<string, number>)[b.tier]! || b.lines - a.lines,
  );
  ...
  if (args.json) {
    const summary = {
      ...
      counts: {
        ...
        orange: stats.orange,
        yellow: stats.yellow,
        total: stats.red + stats.orange + stats.yellow,
      },
    ...
    console.log(
      `扫描文件 ${stats.totalFiles} 个，函数 ${stats.totalFuncs} 个；豁免：生成=${stats.skippedGenerated}，测试=${stats.skippedTest}`,
    );
    console.log(
      `命中：🟥 ${stats.red} · 🟧 ${stats.orange} · 🟨 ${stats.yellow} · 合计 ${stats.red + stats.orange + stats.yellow}`,
    );
    if (items.length === 0) {
      console.log("（干净，无命中）");
      return;
    ...
    for (const it of items) {
      ...
      console.log(
        `  ${it.tier === "red" ? "🟥" : it.tier === "orange" ? "🟧" : "🟨"} ${it.file}:${it.startLine}  ${it.name}${kindTag}  ${it.lines} 行${tag}`,
      );
    }
  }
}
  ...
  for (const p of paths) {
    for (const f of p) {
      ...
      try {
        ...
        console.warn(`[line-counter] 跳过 ${relPosix(f)}: ${(e as Error).message}`);
      }
    }
  }
  return total;
...
function main() {
    ...
    try {
      ...
      if (args.json) {
        process.stdout.write(
          `${JSON.stringify({ ok: false, mode: "funcs", error: e && (e as Error).message, stack: e && (e as Error).stack }, null, 2)}\n`,
        );
      } else {
        console.error(`[line-counter] --funcs 执行失败: ${e && (e as Error).message}`);
        if (e && (e as Error).stack) console.error((e as Error).stack);
      }
      process.exit(1);
    }
    return;
  ...
  // === 前端组件分布 ===
  console.log("\n=== 前端组件行数 ===");
  for (const [name, lines] of packageLines(path.join(ROOT, "frontend", "src", "views"), [
    "*.js",
    "*.ts",
  ])) {
    console.log(`  ${name}: ${lines} 行`);
  ...
  // === 功能模块分布 ===
  console.log("\n=== 功能模块行数 ===");
  for (const [name, lines] of packageLines(path.join(ROOT, "frontend", "src", "features"), [
    "*.js",
    "*.ts",
  ])) {
    console.log(`  ${name}: ${lines} 行`);
  ...
  // === 大文件预警 ===
  console.log("\n=== 大文件预警 (>700行) ===");
  const oversized = oversizedFiles(goDirs.map((d) => walkFiles(d, "*.go"))).concat(
    oversizedFiles([walkFiles(jsDir, ["*.js", "*.ts"])]),
  );
