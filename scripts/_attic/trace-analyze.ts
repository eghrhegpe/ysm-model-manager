...
for (let i = 0; i < ARGS.length; i++) {
  ...
  if (a === "--json") {
    ...
    topN = parseInt(ARGS[++i] ?? "", 10) || 30;
  } else if (a === "--pid") {
    filterPid = parseInt(ARGS[++i] ?? "", 10);
  } else if (a === "--tid") {
    filterTid = parseInt(ARGS[++i] ?? "", 10);
  } else if (a === "--help" || a === "-h") {
  ...
  -h, --help      帮助`);
    ...
    console.warn(`[trace-analyze] 忽略未知选项 "${a}"`);
  }
}
...
function analyze(label: string, trace: any) {
  ...
  for (const e of durEvents) {
    ...
    if (!byName.has(k))
      byName.set(k, { name: e.name, cat: e.cat, count: 0, totalDur: 0, maxDur: 0 });
...
function printReport(r: any) {
  ...
  lines.push(
    `  events=${r.eventCount}  durEvents=${r.durEventCount}  span=${r.spanSec.toFixed(3)}s  totalDur=${r.totalDurSec.toFixed(3)}s`,
  );
  lines.push("=".repeat(70));
  ...
  lines.push(
    `${"dur".padEnd(12)} ${"name".padEnd(40)} ${"cat".padEnd(25)} ${"pid".padEnd(6)} thread`,
  );
  for (const e of r.topEvents) {
    const tname = r.byThread.find((t: any) => t.pid === e.pid && t.tid === e.tid)?.name || "?";
    lines.push(
      `${fmtTime(e.dur).padEnd(12)} ${e.name.slice(0, 40).padEnd(40)} ${(e.cat || "").slice(0, 25).padEnd(25)} ${String(e.pid).padEnd(6)} ${tname}`,
    );
  }
  ...
  lines.push(
    `${"总dur".padEnd(12)} ${"次数".padEnd(6)} ${"单次max".padEnd(12)} ${"name".padEnd(40)} cat`,
  );
  for (const a of r.byName) {
    lines.push(
      `${fmtTime(a.totalDur).padEnd(12)} ${String(a.count).padEnd(6)} ${fmtTime(a.maxDur).padEnd(12)} ${a.name.slice(0, 40).padEnd(40)} ${(a.cat || "").slice(0, 30)}`,
    );
  }
  ...
  for (const a of r.byThread) {
    ...
    lines.push(
      `${(`${(a.pct * 100).toFixed(1)}%`).padEnd(7)} ${fmtTime(a.totalDur).padEnd(12)} ${String(a.count).padEnd(6)} ${a.pid}:${a.tid}  ${a.name}  ${bar}`,
    );
  }
  ...
  if (r.threadDetail) {
    lines.push("");
    lines.push(
      `── 线程 ${r.threadDetail[0]?.pid}:${r.threadDetail[0]?.tid} 最长事件 Top ${r.threadDetail.length} ──`,
    );
    lines.push(`${"dur".padEnd(12)} name  cat`);
    for (const e of r.threadDetail) {
      lines.push(
        `${fmtTime(e.dur).padEnd(12)} ${e.name.slice(0, 60)}  ${(e.cat || "").slice(0, 30)}`,
      );
    }
  }
  ...
  if (r.workerAgg.length) {
    ...
    for (const a of r.workerAgg) {
      lines.push(
        `${fmtTime(a.totalDur).padEnd(12)} ${String(a.count).padEnd(6)} ${a.name.slice(0, 60)}  ${(a.cat || "").slice(0, 25)}`,
      );
    }
  }
...
if (files.length === 2) {
  ...
  if (jsonOut) {
    ...
    console.log(
      `  A totalDur=${a.totalDurSec.toFixed(3)}s  B totalDur=${b.totalDurSec.toFixed(3)}s`,
    );
    // 线程占比差异
    ...
    if (aMain && bMain) {
      console.log(
        `  主线程占比: A=${(aMain.pct * 100).toFixed(1)}%  B=${(bMain.pct * 100).toFixed(1)}%`,
      );
    }
  }
} else {
