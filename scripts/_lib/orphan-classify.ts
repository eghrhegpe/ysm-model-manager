...
import { collectScripts, SCRIPTS_DIR } from "./collect-scripts.ts";
...
export function classifyScript(
  script: string,
  ctx: OrphanCtx,
): { status: "mounted" | "called" | "documented" | "orphan"; callers?: string[]; reason?: string } {
  ...
  const callers = ctx.siblings
    .filter((s) => s.name !== script && s.text.includes(script))
    .map((s) => s.name);
  if (callers.length) {
