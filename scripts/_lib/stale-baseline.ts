/**
 * stale-baseline.ts — baseline 文件超期告警共享层。
 *
 ...
 *
 * 用法：
 *   import { checkStale } from './stale-baseline.ts';
...
/**
 * 检查基线是否超期。
 * @param generatedISO  - ISO 8601 时间戳（JSON 中的 generated 字段）
 * @param label         - 基线名称（用于文案，如 'deadcode' / 'jscpd-go' / 'redlines'）
 * @returns 超期时返回警告字符串，否则返回 null
 */
