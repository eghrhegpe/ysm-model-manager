...
export const RE_SUPERSEDED_BY =
  /被\s*\*{0,2}\s*\[?ADR-(\d+)\]?(?:\s*\([^)]{0,80}\))?\s*(?:\*{0,2}\s*[（(][^）)]{0,40}[）)]\s*\*{0,2})?\s*(?:取代|替代|推翻|退役)/;
...
export const RE_CLAIM_B =
  /ADR-(\d+)\s*[）)]?\s*(?:已\s*(?:废弃|过时|放弃|搁置|退役)|被\s*(?:取代|推翻|替代))/;
...
export const RE_NEGATED_CLAIM =
  /(?:非|不|未|无|没有)\s*\*{0,2}\s*(?:取代|替代|推翻|废弃|废除|退役)\s*了?\s*\[?ADR-(\d+)\]?/;
