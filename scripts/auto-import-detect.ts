...
import { readText, SRC_DIR, toPosix, walk } from "./_lib/scan-files.ts";
import { GLOBALS, KEYWORDS, tokenize } from "./auto-import-lexer.ts";
import { extractDefined, extractExports, extractImported } from "./auto-import-symbols.ts";
...
export function checkFile(
  file: string,
  symbolMap: Map<string, Array<{ file: string; isType: boolean }>>,
  cache?: Map<
    string,
    { stripped: string; tokens: Array<{ name: string; start: number; line: number }>; text: string }
  >,
) {
  const entry =
    cache?.get(file) ??
...
export function run({
  ...
  const typeOnly = suggestions.reduce((n, s) => n + s.missing.filter((m) => m.typeOnly).length, 0);
