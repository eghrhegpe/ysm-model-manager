...
export function applyFixes(suggestions: Array<{ file: string; missing: any[] }>) {
  ...
  for (const s of suggestions) {
    ...
    for (const [p, g] of groups) {
      ...
      if (g.types.size)
        newLines.push(`import type { ${[...g.types].sort().join(", ")} } from "${p}"`);
    }
...
export function fmtText(
  {
    ...
    files: string[];
    suggestions: Array<{ file: string; missing: any[] }>;
    totals: { totalMissing: number; typeOnly: number; ambiguous: number };
  },
  srcDir: string,
) {
  const lines = [
