#!/usr/bin/env node
/**
 * auto-import-symbols.mjs — auto-import 符号收集层（导出 / 本文件定义 / 已导入）。
 *
 * 设计意图：从 auto-import.mjs 拆出的符号分析模块（2026-08-31 大脚本拆分基线 ADR）——
 * 在剥离后源码（tokenize 输出 stripped）上做正则级符号收集：导出符号表 / 本文件定义 /
 * 已导入符号，供 auto-import-detect.mjs 消费。领域专属（非通用共享层，故不入 _lib/）。
 *
 * 依赖：零依赖（node:fs / node:path / node:url）
 *
 * 用法：被 auto-import-detect.mjs 引用，非独立 CLI 入口。
