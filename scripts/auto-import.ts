#!/usr/bin/env node
/**
 * auto-import.ts — TS/JS 缺失 import 检测（只读提示版，ADR-014 试水）。
 *
 ...
 *   - 绑定符号（DetectContainerType 等）刻意不补：项目规范走 getApp()（ADR-012）。
 *
 * 结构（2026-08-31 大脚本拆分基线 ADR 拆出）：
 *   本文件仅保留 CLI 入口；词法/符号/检测/修复各层拆至
 *   auto-import-lexer.ts / auto-import-symbols.ts /
 *   auto-import-detect.ts / auto-import-fix.ts。
 *
 * 用法：
 *   node scripts/auto-import.ts                      # 检测全部 .ts
 *   node scripts/auto-import.ts frontend/src/core/handler-other.ts   # 单文件
 *   node scripts/auto-import.ts --include-js         # 连存量 .js 一起扫
 *   node scripts/auto-import.ts --fix                # 自动写入缺失 import（歧义跳过）
 *   node scripts/auto-import.ts --watch              # 监听变化自动重扫
 *   node scripts/auto-import.ts --json               # JSON 输出（CI 用）
 *   node scripts/auto-import.ts --strict             # 有缺失 → 退出码 1
 *
