...
const goModText = fs.existsSync(path.join(ROOT, "go.mod"))
  ? fs.readFileSync(path.join(ROOT, "go.mod"), "utf-8")
  : "";
...
function extractBindingsExports() {
  ...
  for (const m of text.matchAll(/export function (\w+)\(/g)) {
    ...
    if (!(name in result.funcs)) {
      ...
      const body = paramBody(text, open, close);
      result.funcs[name] = {
        file: path.basename(BINDINGS_FILE),
        arity: paramArity(text, open, close),
        params: tsParamTypes(body),
      };
    }
  }
  return result;
...
const out = {
  _summary: {
    go_functions: Object.keys(goExports).length,
    js_functions: Object.keys(jsExports).length,
