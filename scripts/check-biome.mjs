#!/usr/bin/env node
/**
 * check-biome.mjs — Biome 委托检查器（2026-08-27 新增，P0 落地）
 *
 * 为何自写包装而非直接调 biome：
 ...
 *
 * 用法：
 *   node scripts/check-biome.mjs            # 增量阻塞（push 门禁默认）
...
if (noFiles) {
  ...
  console.error('[check-biome] 变更文件存在 Biome 违规（已阻断）— 本地跑 `node scripts/check-biome.mjs --write` 修复后重推');
}
process.exit(ok ? 0 : 1);
