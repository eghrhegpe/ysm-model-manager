#!/usr/bin/env node
/**
 * check-circular-go.ts — Go 包级循环依赖检测器。
 *
 ...
 *   - DFS 三色标记找 import 级循环（与前端 check-circular.ts 对称）。
 *
 ...
 *
 * 用法：
 *   node scripts/check-circular-go.ts          # 文本报告
 *   node scripts/check-circular-go.ts --json   # JSON（CI 用）
 *
 * 退出码：发现环 → 1；否则 0。
...
function main() {
  ...
  for (const f of goFiles) {
    const k = dirKey(f);
    if (!dirImports.has(k)) dirImports.set(k, new Set<string>());
  ...
  const graph = new Map<string, string[]>([...nodes].map((n) => [n, []]));
