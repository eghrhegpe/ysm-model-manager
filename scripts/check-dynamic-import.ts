...
const TYPE_REF_RE =
  /(?:type\s+[A-Za-z_$][\w$]*\s*=\s*import\(|typeof\s+import\(|Array<import\(|import\([^)]*\)\s*\.\s*[A-Z][\w$]*\b(?!\s*\())/;
...
function isEmptyCatch(text: string, start: number) {
  ...
  return (
    /\.catch\s*\(\s*\(\)\s*=>\s*\{\s*\}\)/.test(tail) ||
    /\.catch\s*\(\s*\(\)\s*=>\s*\{\}\)/.test(tail)
  );
}
    ...
    for (const m of text.matchAll(DYNAMIC_IMPORT_RE)) {
      ...
      if (isLightweightUtil(spec)) {
        warns.push({
          ...
          line,
          spec,
          kind: "lightweight_util",
          msg: "轻量工具模块被动态导入（建议静态导入，无按需价值）",
        });
      }
      ...
      if (
        !isLoadViewWrapped(text, m.index) &&
        !isAwaitImport(text, m.index) &&
        isBareImport(text, m.index)
      ) {
        warns.push({
          file: rel,
          line,
          ...
          msg: "裸动态导入无 .catch（fire-and-forget，失败静默）",
        });
      }
