#!/usr/bin/env node
/**
 * check-go-diff-coverage.ts — Go 变更文件覆盖率门禁（diff-coverage gate，Go 版）。
 *
 * 设计意图：前端已有 check-diff-coverage.ts（只认 frontend/src/*.ts），Go 重构
 ...
 *   node scripts/check-go-diff-coverage.ts --threshold 70           # 提高阈值
 ...
 *   node scripts/check-go-diff-coverage.ts --staged                 # 仅本次暂存区（commit-with-check / prepare-commit-msg 场景）
 *   node scripts/check-go-diff-coverage.ts --uncommitted            # 纳入工作区+暂存区（本地预检）
...
function main() {
  ...
  const failOrWarn: (msg: any) => never = (msg) => {
    if (suggest) {
