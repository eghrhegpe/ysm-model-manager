#!/usr/bin/env node
/**
 * check-lib-adoption.mjs — _lib 共享层采用率检查（治理杠杆的「采用率闸门」）。
 *
 * 设计意图：scripts/_lib/ 现有 25 个共享模块，但只有 proc.mjs 配了专属采用率
 * 闸门（check-proc-adoption.mjs），结果是非直调占比 100% 全收敛；其余 24 个模块
 * 零闸门、纯靠自觉（parse-args 仅 37/95 采用，多脚本仍各自内联 walk/符号提取）。
 * 本脚本把「proc 的成功经验」推广为规则驱动的通用闸门：
 *   规则表（模块 → 手搓特征 / 采用特征）驱动，新增模块只需加一行 RULES。
 * 与相邻脚本的分工（避免重复告警）：
 *   - check-proc-adoption.mjs：专管子进程（proc.mjs），本脚本显式跳过该模块；
 *   - check-script-hygiene.mjs：管文件头 / 退出码 / --json 契约 / argv 契约，
 ...
 *   node scripts/check-lib-adoption.mjs --json    # JSON（doctor/CI 消费）
 *   node scripts/check-lib-adoption.mjs --strict  # 有违规 → 退出码 1
 *
 ...
 * proc.mjs 不在此表：由 check-proc-adoption.mjs 专管，避免重复告警。
 */
const RULES = [
  {
    lib: 'scan-files.ts',
