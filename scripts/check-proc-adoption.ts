#!/usr/bin/env node
/**
 * check-proc-adoption.ts — 子进程直调收敛检查（ADR-043 落地率守护）
 *
 ...
 *
 * 用法：
 *   node scripts/check-proc-adoption.ts           # 文本报告（WARN，不阻断）
 *   node scripts/check-proc-adoption.ts --json    # JSON（doctor/CI 消费）
 *   node scripts/check-proc-adoption.ts --strict  # 有 WARN → 退出码 1
 *
