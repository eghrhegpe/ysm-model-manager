 ...
 * 用法：
 *   node scripts/check-redlines.ts                 # 默认行为
 *   node scripts/check-redlines.ts --json # JSON 输出（CI/子代理消费）
 * 退出码：0（成功）
 * 依赖：本地模块
 */
...
function runChecks() {
  ...
  // 字符集与原 /\u0000-\u001F\u007F-\u009F/g 完全一致（U+0000-U+001F + U+007F-U+009F），
  // 字面正则写法触发 biome noControlCharactersInRegex，改用等价的 RegExp 构造器（行为不变）。
  // biome-ignore lint/complexity/useRegexLiterals: 字面正则会触发 noControlCharactersInRegex，二者只能以构造器形式共存（字符集等价，已逐码点核对）
  ...
  add(
    ...
    rgTracked("#[0-9a-f]{3}(?:[0-9a-f]{3})?\\b|rgba?\\(|hsla?\\(", "frontend", [
      "*.js",
      "*.ts",
      "*.css",
    ])
      ...
      .filter((l) => {
        const [f] = parseRgLine(l);
        return (
          !/voxel-colors-data\.ts$/.test(f) &&
          !/voxel-parse\.ts$/.test(f) &&
          !/ui-advanced-rows\.ts$/.test(f)
        );
      })
      .filter((l) => {
        const [f] = parseRgLine(l);
        return (
          !/litematic-(meta|3d)\.ts$/.test(f) &&
          !/mc-format\.ts$/.test(f) &&
          !/summarize\.ts$/.test(f) &&
          !/zoom\.ts$/.test(f) &&
          !/skeleton\.ts$/.test(f)
  ...
  add(
    ...
    rgTracked('"ysm"|"mmd-skin"|"vrchat-avatar"', "frontend/src", ["*.js", "*.ts"])
      ...
      .filter((l) => {
        const [f] = parseRgLine(l);
        return !f.includes("utils/resource/types");
      })
      .filter((l) => !/:\d+:\s*(?:\/\/|\/\*|\*)/.test(l))
  ...
  const r8Inner = rgTracked("innerHTML\\s*=\\s+[^'\"`\\n]", "frontend/src", [
    ...
    if (/t\("/.test(l)) {
      ...
      if (blocks?.every((b) => /t\(/.test(b))) return false;
    }
  ...
  add(
    ...
    rgTracked("window\\.go\\.main\\.App", "frontend/src", ["*.js", "*.ts"])
      .filter(
  ...
  add(
    "W5",
    "async DOM race (callback sets innerHTML without stale guard)",
    // 2026-09 性能：3 次 rg（箭头/then-finally/setTimeout）合并为单正则一次扫，行为不变
    rgTracked(
      "=>\\s*\\{[^}]*innerHTML\\s*=|\\.(then|finally)\\s*\\(.*innerHTML\\s*=|setTimeout\\s*\\(.*innerHTML\\s*=",
      "frontend/src",
      ["*.js", "*.ts"],
    ),
  ...
  add(
    "W7",
    "binding-layer write ops (need cache invalidation)",
    // 2026-09 性能：3 次 rg（os./fileops./recycle.）合并为单正则一次扫，行为不变
    rgTracked(
      "os\\.(Remove|RemoveAll|Rename)\\s*\\(|fileops\\.(RenameDir|RenameFile|RemoveDir|DeleteModelFile|WriteModelFolder)\\s*\\(|recycle\\.(MoveEx|Restore|Delete|Empty)\\s*\\(",
      "internal/app",
      ["*.go", "!*_test.go"],
    )
  ...
  add(
    "W8",
    "rtype literal branches (ADR-065 registry-driven)",
    rgTracked('(rtype|rt\\.ID)\\s*==\\s*"[a-z][a-z0-9-]*"', "go", ["*.go", "!*_test.go"]).filter(
      (l) => !/:\d+:\s*\/\//.test(l),
    ),
 ...
 */
export function redlineFilterKeysByChangedFiles(
  keys: string[],
  changedSet: Set<string> | null | undefined,
) {
...
function runBaseline(results: any[]) {
  ...
  if (update) {
    ...
    fs.writeFileSync(
      BASELINE_FILE,
      `${JSON.stringify(
        { generated: new Date().toISOString(), count: allKeys.length, violations: allKeys },
        null,
        2,
      )}\n`,
    );
    return {
      ok: true,
      note: `--update-baseline: 已写入 ${allKeys.length} 条红线基线`,
      current: allKeys,
    };
  }
  if (!fs.existsSync(BASELINE_FILE)) {
    return {
      ok: false,
      note: `[缺失基线] redlines-baseline.json 不存在——无法比对新增违规，请先运行 node scripts/check-redlines.ts --json --update-baseline 建立基线`,
      current: allKeys,
      newViolations: allKeys,
      advisoryViolations: [],
    };
  ...
  const staleWarn = checkStale((base as { generated?: string }).generated, "redlines");
  if (staleWarn) console.warn(staleWarn);
  const baseObj = (base ?? {}) as { violations?: string[] };
  const baseSet = new Set<string>(baseObj.violations || []);
  ...
  return {
    ...
    note: blocking
      ...
      : advisory
        ...
        : gone.length
          ? `${gone.length} 条历史违规已清理`
          : "红线零新增",
    current: allKeys,
    newViolations: newBlocking,
    advisoryViolations: newAdvisory,
    errors,
    warns,
    infos,
...
if (isMain) {
  ...
  if (baselineMode) {
    ...
    if (jsonMode) {
      ...
      if (!r.ok)
        console.log(
          "→ 修复: 检查新增红线违规项并修复，或 node scripts/check-redlines.ts --json --update-baseline 接受现状",
        );
    }
