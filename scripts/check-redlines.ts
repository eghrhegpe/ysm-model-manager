...
function runChecks() {
  ...
  const results: Array<{ rule_id: string; name: string; fix: string; count: number; violations: Array<{ file: string; line: number; snippet: string }> }> = [];
...
function runBaseline(results: any[]) {
  ...
  const baseSet = new Set<string>(base.violations || []);
...
if (isMain) {
  ...
  if (baselineMode) {
    ...
    if (jsonMode) {
      ...
      if (!r.ok) console.log('→ 修复: 检查新增红线违规项并修复，或 node scripts/check-redlines.ts --json --update-baseline 接受现状');
    }
