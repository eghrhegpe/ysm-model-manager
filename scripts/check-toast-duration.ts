#!/usr/bin/env node
/**
 * check-toast-duration.ts — toast 时长单一事实源守护（防回流闸）
 *
 ...
 * 依赖：node:child_process / node:fs / node:path / scripts/_lib/scan-files.ts / scripts/_lib/proc.ts（零外部依赖）
 *
 * 用法：node scripts/check-toast-duration.ts
 *
