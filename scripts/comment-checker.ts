...
function scanAiFluff() {
  ...
  for (const src of ['go', 'frontend/src']) {
    for (const line of rg(/^\s*\/\/.*\u7528\u4e8e|^\s*\/\/.*\u8fd9\u662f|^\s*\/\/.*\u68c0\u67e5.*\u662f\u5426/.source, src, ['*.go', '*.js', '*.ts'] as any)) {
...
function scanEmptyJsdoc() {
  ...
  const results: any[] = [];
  for (const line of rg(/@param\s+\{[^}]*\}\s+\w+\s*-?\s*$|@returns\s*\{[^}]*\}\s*$/.source, 'frontend/src', ['*.js', '*.ts'] as any)) {
    const [f, ln, txt] = parseRgLine(line);
...
function scanCommentedCode() {
  ...
  for (const line of rg(/^\s*\/\/\s+(var |let |const |function |if |for |return |import |export )/.source, 'frontend/src', ['*.js', '*.ts'] as any)) {
    const [f, ln, txt] = parseRgLine(line);
