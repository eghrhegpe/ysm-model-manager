#!/usr/bin/env node
/**
 * comment-checker.ts — 注释质量检查（AI 废话 / 空 JSDoc / TODO 无编号 / 调试残留）。
 *
 ...
 *
 * 用法：
 *   node scripts/comment-checker.ts              # 文本报告
 *   node scripts/comment-checker.ts --json       # JSON（CI / 子代理消费，每类截断 50 条）
 *   node scripts/comment-checker.ts --full       # 全量 JSON（不截断）
 *
 * 退出码：默认 0（提示工具）。
...
import { rgSafe as rg } from "./_lib/ripgrep.ts";
...
function scanEmptyJsdoc() {
  ...
  const results: any[] = [];
  for (const line of rg(
    /@param\s+\{[^}]*\}\s+\w+\s*-?\s*$|@returns\s*\{[^}]*\}\s*$/.source,
    "frontend/src",
    ["*.js", "*.ts"] as any,
  )) {
    const [f, ln, txt] = parseRgLine(line);
...
function scanCommentedCode() {
  ...
  const readLine = (f: string, ln: number) => {
    if (!fileCache.has(f)) {
      try {
        fileCache.set(f, fs.readFileSync(f, "utf8").split("\n"));
      } catch {
        fileCache.set(f, []);
      }
    }
  ...
  for (const line of rg(
    /^\s*\/\/\s+(var |let |const |function |if |for |return |import |export )/.source,
    "frontend/src",
    ["*.js", "*.ts"] as any,
  )) {
    const [f, ln, txt] = parseRgLine(line);
    ...
    const kw =
      (txt.match(/^(var|let|const|function|if|for|return|import|export)\b/) || [])[0] || "";
...
function scanTodoNoTicket() {
  ...
  const results: any[] = [];
  for (const src of ["go", "frontend/src"]) {
    for (const line of rg("TODO|FIXME|HACK|XXX|TEMP", src, ["*.go", "*.js", "*.ts"] as any)) {
      // 过滤有编号的
...
function scanDebugLog() {
  ...
  const results: any[] = [];
  for (const line of rg("console\\.log|console\\.debug", "frontend/src", ["*.js", "*.ts"] as any)) {
    const [f, ln, txt] = parseRgLine(line);
...
const results: Record<string, any> = runAll();
...
if (jsonMode) {
  ...
  const names = {
    AI_fluff: "AI 废话注释",
    empty_jsdoc: "空 JSDoc 模板",
    commented_code: "注释掉的代码",
    todo_no_ticket: "TODO 无编号",
    debug_log: "调试日志",
  };
  for (const [cat, items] of Object.entries(results)) {
