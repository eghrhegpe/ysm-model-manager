 ...
 *   2. 检查全部委托给 pre-push-gate.ts（--all --dry-run / --docs --dry-run）
 ...
 *   node scripts/commit-with-check.ts --check --docs          # 仅文档域验证
 *
 * 退出码：
...
for (let i = 0; i < args.length; i++) {
  ...
  if (a === '-m' || a === '--message') {
    ...
    console.log(`用法: node scripts/commit-with-check.ts -m "<msg>" [--docs|--check]
...
if (!checkOnly && !message) {
  console.error('用法: node scripts/commit-with-check.ts -m "<msg>" [--docs|--check]');
  process.exit(2);
}
