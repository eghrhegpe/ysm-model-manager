...
if (args.unknown.length)
  console.warn(`[compile-android-rust] 忽略未知参数: ${args.unknown.join(", ")}`);
...
for (const arch of arches) {
  ...
  if (!targets.out.includes(target)) {
    ...
    if (!r.ok)
      fail(`rustup target add ${target} 失败：
${r.out.slice(-400)}`);
  }
}
