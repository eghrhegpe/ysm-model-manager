...
/**
 * 项目健康诊断 CLI 派发器。
 * doctor.ts — 全量治理检查编排（2026-08-14 起为薄派发器）
 * 设计意图：三模式全部委托 pre-push-gate.ts（单一实现源头，消除双端漂移）
 * 依赖：node:child_process / node:path / node:url
 * 用法：
 *   node scripts/doctor.ts                 # 默认行为（委托 pre-push-gate --all --dry-run：全量体检）
 *   node scripts/doctor.ts --docs          # 文档模式（委托 pre-push-gate --docs --dry-run：轻量）
 *   node scripts/doctor.ts --gate          # 门禁模式（委托 pre-push-gate.ts --dry-run，不触发 push）
 *   node scripts/doctor.ts --gate <ref>    # 指定 ref（默认 HEAD，用于预检未提交的改动）
...
if (GATE_MODE) {
  // --gate 模式：委托 pre-push-gate.ts --dry-run（单一实现，避免双端漂移）。
  // 用法：node scripts/doctor.ts --gate [ref]
