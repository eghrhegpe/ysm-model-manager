...
/* ---------------- 真实数据代入 ---------------- */
const samples = [
  { kind: "存量债", tool: "check-knowledge-drift.ts", rc: 1, hasJson: true, json: {
  ...
  { kind: "本次引入", tool: "check-diff-coverage.ts", rc: 1, hasJson: false, raw: "frontend/src/views/app-content/index.ts: 低于 60% 变更行覆盖率，建议补测\nTS2345: Argument of type 'boolean | null' ...\nscripts/pre-push-gate.ts(687,75): error TS2345" },
  { kind: "失守", tool: "check-redlines.ts", rc: 1, hasJson: false, raw: "rg 缺失：fail-closed 阻断，红线门禁未执行" },
];
...
function styleC(s) {
  ...
  const label = `node scripts/${s.tool} --json`;
  let lines = [];
  if (s.hasJson && s.json.errors?.length) lines = s.json.errors.map((e, i) => (i === 0 ? e : `      ${i + 1}. ${e}`));
...
const fn = fns[STYLE];
if (!fn) { console.error("用法: node scripts/experiments/fail-output-styles.ts [A|B|C|D]"); process.exit(1); }
...
for (const s of samples) { console.log(fn(s)); console.log(""); }
