#!/usr/bin/env node
/**
 * gen-adr-supersede.ts — 扫描 docs/adr/ 全部 ADR，输出「取代关系」判定结果。
 *
 ...
 *
 * 用法：
 *   node scripts/gen-adr-supersede.ts         # 打印取代关系清单（0 = 正常）
 *   node scripts/gen-adr-supersede.ts --check # 仅 ②（漏标）失败时退出码 1（供 check:docs 用）
 *
