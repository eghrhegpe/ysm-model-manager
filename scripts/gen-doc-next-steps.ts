...
/**
 * gen-doc-next-steps.ts — 文档体系「待补地图」生成器（诊断聚合，不修改任何源文件）
 *
 ...
 *   - scripts/check-knowledge-drift.ts  → 知识卡 frontmatter / source_files / 覆盖盲区
 *   - scripts/link-checker.ts           → markdown 断链（含 position 字符偏移）
 *   - scripts/adr-check.ts              → ADR 撞号 / 漏登 / 幽灵 / 编号空缺
 *
 * 用法：
 *   node scripts/gen-doc-next-steps.ts
 *
...
function main() {
  const kd = runChecker("check-knowledge-drift.ts");
  const lc = runChecker("link-checker.ts");
  const ac = runChecker("adr-check.ts");
  ...
  L.push("");
  L.push(
    "> 由 `scripts/gen-doc-next-steps.ts` 自动生成（聚合 check-knowledge-drift / link-checker / adr-check 的 `--json`）。",
  );
  ...
  L.push("");
  L.push(
    `- 🔴 阻断级（须修）：知识卡失效 **${kdErr.length}** / 断链 **${broken.length}** / ADR 问题 **${adrErr.length}**${lcFail.length ? ` / link-checker **未运行**` : ""}`,
  );
  L.push(
    `- 🟡 提醒级（建议补）：知识卡待补 **${kdWarn.length}** / ADR 编号空缺 **${adrGaps.length}**`,
  );
  L.push("");
  ...
  if (adrGaps.length) {
    ...
    L.push("");
    L.push(
      `- ${adrGaps.map((n: any) => `ADR-${String(n).padStart(3, "0")}`).join("、")} 尚未占用，新建 ADR 时优先占用。`,
    );
    L.push("");
  }
