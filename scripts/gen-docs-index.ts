...
if (args.includes('--help') || args.includes('-h')) { console.log('用法: node scripts/gen-docs-index.ts [--check] [--adr|--guide|--releases|--knowledge]'); process.exit(0); }
...
function buildAdrIndex(list) {
  ...
  out += 'permalink: /adr/\n';
  out += '---\n\n';
  out += '<!-- 本文件由 scripts/gen-docs-index.ts 自动生成，禁止手改。重跑：node scripts/gen-docs-index.ts -->\n\n';
  out += '# 决策记录（ADR）\n\n';
  ...
  out += '---\n\n';
  out += '*登记表由 `gen-docs-index.ts` 自动重写；一致性校验已接入：`node scripts/check-adr-health.ts`（状态值域 + 登记同步 + 技术债）+ `node scripts/check-doc-drift.ts`（编号连续性/漏登/幽灵）+ `node scripts/gen-adr-supersede.ts`（取代关系扫描）。*\n';
  return out;
}
