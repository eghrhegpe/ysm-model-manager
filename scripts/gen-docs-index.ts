...
/**
 * gen-docs-index.ts — 分区索引自动生成（adr / knowledge / releases）。
 *
 ...
 *   - knowledge→ 委托 gen-knowledge-index.ts --check（不重写，避免双生成器打架）
 * 单一事实来源 = ADR 文件首部 / guide 各篇 frontmatter；状态映射以 ADR 首部状态行为准。
 *
 * 用法：
 *   node scripts/gen-docs-index.ts                # 全分区写入
 *   node scripts/gen-docs-index.ts --adr          # 只跑 adr
 *   node scripts/gen-docs-index.ts --guide        # 只跑 guide
 *   node scripts/gen-docs-index.ts --releases     # 只跑 releases
 *   node scripts/gen-docs-index.ts --knowledge    # 只校验 knowledge 漂移
 *   node scripts/gen-docs-index.ts --check        # 全分区只校验不写入
...
if (args.includes("--help") || args.includes("-h")) {
  console.log(
    "用法: node scripts/gen-docs-index.ts [--check] [--adr|--guide|--releases|--knowledge]",
  );
  process.exit(0);
}
...
const ONLY = args.some(
  (a) => a.startsWith("--") && ["--adr", "--releases", "--knowledge", "--guide"].includes(a),
);
...
function applyRegion(file: string, name: string, content: string) {
  ...
  if (next === null) {
    console.error(
      `[FAIL] ${path.relative(ROOT, file)} 缺少 <!-- GEN: ${name} --> 标记，需一次性插入`,
    );
    return { ok: false, changed: false };
  }
...
// ── ADR 解析与状态映射（状态值域以 adr-check.ts 为准）──
 ...
 * 改规则 = 改本常量后重跑 gen-docs-index.ts（index 保持「全生成、禁手改」）。
 */
...
function buildAdrIndex(list: any[]) {
  ...
  const _STATUS_EXAMPLES = DISPLAY_GROUPS.map((g) => g.label).join(" / ");
...
function buildGuideIndex() {
  ...
  let out = `> 按功能讲解入口路径与操作步骤，共 **${pages.length}** 篇。新功能持续建档；索引由 gen-docs-index.ts 自动生成，断链由 link-checker 兜底。\n\n`;
  out += "| 指南页 | 说明 |\n";
  out += "|--------|------|\n";
...
function checkKnowledge() {
  const gen = path.join("scripts", "gen-knowledge-index.ts");
...
function main() {
  ...
  if (RUN_ADR) {
    if (!fs.existsSync(ADR_DIR)) {
      ...
      if (r3.changed && r3.ok)
        console.log(`[OK] 已更新 docs/adr/index.md（规范索引 + 登记表 + 使用规则）`);
  ...
  if (RUN_RELEASES) {
    ...
    if (r.changed && r.ok)
      console.log(`[OK] 已更新 ${path.relative(ROOT, RELEASE_FILE)} 的 releases-index 区`);
    if (!r.ok) failed = true;
  }
  ...
  if (RUN_KNOWLEDGE) {
    if (!checkKnowledge()) {
      console.error("[FAIL] knowledge/index.md 过期，请运行 node scripts/gen-knowledge-index.ts");
      failed = true;
    }
  }
