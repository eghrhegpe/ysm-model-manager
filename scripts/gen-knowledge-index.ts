#!/usr/bin/env node
/**
 * gen-knowledge-index.ts — 按分类生成知识卡索引 (docs/knowledge/index.md)。
 *
 * 零依赖（仅 node:fs / node:path / node:url）。
 * 产出 docs/knowledge/index.md（自动生成物，禁止手改）；本文件是生成器本身。
 *
 * 用法：
 *   node scripts/gen-knowledge-index.ts           # 写入
 *   node scripts/gen-knowledge-index.ts --check    # 校验是否已同步（不写入）
 * 设计意图：知识索引生成器
 * 退出码：main() 失败 → 1；否则 0。
 */
...
import {
  CATEGORY_LABELS,
  KNOW_DIR as KC_DIR,
  KNOWLEDGE_NON_CARDS as NON_CARDS,
  PERF_TAGS,
} from "./_lib/knowledge-cards.ts";
import { parseArgs } from "./_lib/parse-args.ts";
import { readText, writeText } from "./_lib/scan-files.ts";
 ...
 * 改说明 = 改本常量后重跑 gen-knowledge-index.ts（index 保持「全生成、禁手改」）。
 */
