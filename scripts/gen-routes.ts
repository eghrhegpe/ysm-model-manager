...
function renderRoutes(
  cards: Array<{ file: string; name: string; useWhen: string[]; adrs: number[]; summary: string }>,
  kwToCards: Map<string, any[]>,
) {
  const out: string[] = [];
  out.push(BANNER);
  out.push("");
  ...
  for (const c of cards) {
    ...
    if (ambiguous.length) {
      const cands = [
        ...new Set(
          ambiguous.flatMap((kw) =>
            (kwToCards.get(kw) || []).map((x) => x.file).filter((f) => f !== c.file),
          ),
        ),
      ].slice(0, 3);
      const more =
        ambiguous
          .flatMap((kw) => (kwToCards.get(kw) || []).map((x) => x.file))
          .filter((f) => f !== c.file).length > cands.length
          ? "等"
          : "";
...
function main() {
  ...
  if (conflicts.length) {
    console.warn(
      `⚠️  ${conflicts.length} 个 use_when 关键词被多张卡共用（路由有歧义，建议人工消歧）:`,
    );
  ...
  console.error(
    `📄 ${routable.length} 张 architecture 卡可路由（${cards.length - routable.length} 张无 use_when 关键词）`,
  );
