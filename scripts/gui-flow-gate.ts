 ...
 *
 * 用法：
 *   node scripts/gui-flow-gate.ts                      # 默认 files-root=tests/fixtures/ysm/01_taisho_maid
 *   node scripts/gui-flow-gate.ts --files-root <dir>   # 指定模型仓库
 *   node scripts/gui-flow-gate.ts --threshold-ms 30000 # 收紧总耗时阈值
 *   node scripts/gui-flow-gate.ts --model <path>       # 指定要分析的模型（默认自动选第一个）
 *   node scripts/gui-flow-gate.ts --verbose            # 打印原始输出与解析明细
 * 退出码：0=通过，1=失败。
...
const opts = {
  ...
  requireModel: parsed["require-model"] as boolean,
};
...
// totalMatch 已通过 totalRe.test() ⇒ match 必命中，?? "" 分支不可达；NaN 与原
// `?.[1]!`（undefined → parseFloat → NaN）的运行时行为完全一致
...
let hasModel = false;
if (scanStats) {
  // scanStats 命中 `YAML:\s*\d+...YSM:\s*\d+` ⇒ 两处 match 必命中；?? "" 分支不可达。
  // NaN 与原 `?.[1]!`（undefined → parseInt → NaN）的运行时行为完全一致
...
if (hasModel || opts.requireModel) {
  ...
  const note = modelStage
    ...
    : "③ 未生成";
  console.log(
    `[提示] 未检测到可分析模型输入（YAML/YSM 计数为 0）——跳过 ③ 分析/后续阶段与总耗时的强验证，${note}`,
  );
  console.log(
    "        如需完整验证加载链，请传入含真实模型清单的 --files-root 并加 --require-model",
  );
}
