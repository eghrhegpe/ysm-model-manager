...
export function buildBlock(files, start = BLOCK_START, end = BLOCK_END) {
  ...
  if (files.length > MAX_SUGGEST_FILES) {
    lines.push(
      `- …其余 ${files.length - MAX_SUGGEST_FILES} 个见 node scripts/test-coverage-report.ts --suggest`,
    );
  }
  lines.push(end);
  return lines.join("\n");
...
function getDiffCoverageBlock(ROOT) {
  const r = run(
    process.execPath,
    [path.join(ROOT, "scripts", "check-diff-coverage.ts"), "--suggest", "--staged"],
    { cwd: ROOT },
  );
  if (!r.ok) return null;
...
function main() {
  ...
  if (files.length > 0) {
    parts.push(
      `🔬 ${files.length} 个源文件低于覆盖率阈值${files.length > 3 ? `（前 3：${preview}…）` : `：${preview}`}`,
    );
  }
  ...
  console.error(
    `[prepare-commit-msg] ${parts.join("；")}${buildStaleHint(covMtime)}（仅终端提醒）`,
  );
}
