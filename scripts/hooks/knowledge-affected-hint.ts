...
export function buildBlock(cards, _start = BLOCK_START, end = BLOCK_END) {
...
export function findStaleSnippets(cardText, diffText, pairs = STALE_KEYWORD_PAIRS, lineOffset = 0) {
  ...
  const CONTEXTUAL =
    /(替代|替换|取代|迁移|不再|禁止|不得|避免|仍用|旧写法|废弃|\bdeprecated\b|\bDON'T\b|\bMUST NOT\b)/i;
  const out = [];
...
function getAffectedCards(ROOT, changed) {
  ...
  const r = run(
    process.execPath,
    [path.join(ROOT, "scripts", "check-knowledge-drift.ts"), "--affected", "--quiet", ...changed],
    {},
  );
  if (!r.ok) return [];
  return r.out
  ...
  for (const card of cards) {
    ...
    if (stale?.length) {
      ...
      for (const s of stale) {
        lines.push(
          `      L${s.line} ${s.text.slice(0, 60)}（仍用 ${s.pair.oldWord}，建议 ${s.pair.new}）`,
        );
      }
    } else {
