#!/usr/bin/env node
/**
 * i18n-check.ts — i18n 国际化检查（key parity/占位符/漏译/语言清单漂移）
 *
 * 移植自联邦项目 MikuMikuAR scripts/i18n-check.ts（ADR-045 落地配套）。
 ...
 *
 * 用法：
 *   node scripts/i18n-check.ts                 # warning 模式（默认，非阻断）
 *   node scripts/i18n-check.ts --strict        # 缺口存在时非零退出（CI/doctor 门禁用）
 *   node scripts/i18n-check.ts --json          # JSON 输出（供脚本消费）
 *
...
function extractPlaceholders(file: string) {
  const text = readFileSync(file, "utf8");
  const map = new Map<string, Set<string>>(); // key -> Set<string> of placeholder names
  // 捕获值开引号（组2）并用负向前瞻排除对应引号，兼容单/双引号两种源码风格。
  // 移植适配：本项目 locale 用双引号，隔壁 `[^'\\]` 会吞掉 `"` 导致整段误匹配。
  for (const m of text.matchAll(/^\s*['"]([^'"]+)['"]\s*:\s*(['"])((?:\\.|(?!\2).)*)\2/gm)) {
    const key = m[1]!;
    const val = m[2]!;
    const ph = new Set<string>();
    for (const p of val.matchAll(/\{(\w+)\}/g)) ph.add(p[1]!);
...
const availableLangs = langsMatch?.[1]
...
if (inAvailNotFile.length || inFileNotAvail.length) {
  ...
  if (inAvailNotFile.length)
    log(`  仅声明于 SUPPORTED_LANGS 但无 bundle 文件: ${inAvailNotFile.join(", ")}`);
  if (inFileNotAvail.length)
    log(`  存在 bundle 文件但未列入 SUPPORTED_LANGS: ${inFileNotAvail.join(", ")}`);
