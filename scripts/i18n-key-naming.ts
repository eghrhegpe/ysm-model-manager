 ...
 *
 * 用法：
 *   node scripts/i18n-key-naming.ts                       # CI 模式：只检查新增/修改的键
 *   node scripts/i18n-key-naming.ts --list-violations     # 列出所有违规旧键（按"高频歧义实体"排序）
 *   node scripts/i18n-key-naming.ts --check key1 key2     # 检查指定键
 *   node scripts/i18n-key-naming.ts --entity bone          # 列出某实体的所有键（如所有含"骨骼"的键）
 *   node scripts/i18n-key-naming.ts --help                # 帮助
 *
...
function listViolations() {
  ...
  const violations: Array<{ key: string; reason?: string; suggestion?: string }> = [];
