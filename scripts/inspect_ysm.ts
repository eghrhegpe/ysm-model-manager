 ...
 * inspect_ysm.ts — inspect_ysm 工具脚本
 * 设计意图：inspect_ysm 工具脚本
 * 依赖：node:fs / node:path
 * 用法：
 *   node scripts/inspect_ysm.ts                 # 默认行为
 *   node scripts/inspect_ysm.ts --json # JSON 输出（CI/子代理消费）
 * 退出码：1（失败）
 */
...
function inspect(filepath: string) {
  ...
  const result = { file: name, size, format: 'unknown', has_bom: false,
    ...
    text_sections: [] as string[], binary_offset: 0, embedded_files: [] as { name: string; offset: number }[],
...
if (!fileArg) {
  console.error('Usage: node scripts/inspect_ysm.ts <path> [--json]');
  process.exit(1);
}
