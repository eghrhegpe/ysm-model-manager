...
function main() {
  if (!fs.existsSync(JSCPD!)) {
    console.error(
      `[jscpd-go] 未找到 jscpd 二进制: ${path.relative(ROOT, JSCPD!)}\n请先安装前端依赖 (frontend/node_modules/jscpd)。`,
    );
    process.exit(3);
  }
  ...
  try {
    ...
    if (!Array.isArray(report.duplicates)) {
      if (JSON_MODE)
        console.log(
          JSON.stringify({ _summary: { ok: false, error: "report-schema", language: FORMAT } }),
        );
    ...
    if (sources === 0) {
      if (JSON_MODE)
        console.log(
          JSON.stringify({ _summary: { ok: false, error: "empty-scan", language: FORMAT } }),
        );
      else
        console.error(
          "[jscpd-go] 扫描到 0 个 .go 文件（pattern 失配或 go/ 不存在），重复检测未实际运行，拒绝放行",
        );
      process.exitCode = 3;
      return; // finally 清理 tmp
    }
    ...
    if (!JSON_MODE)
      console.log(
        `[jscpd-go] 扫描 ${files} 文件 (重复率 ${pct}), ${dupCount} 处重复块 → ${current.length} 个唯一文件对`,
      );
    ...
    if (mode === "update") {
      ...
      if (JSON_MODE) {
        console.log(
          JSON.stringify({
            _summary: { ok: true, updated: current.length, language: FORMAT, duplicates: dupCount },
          }),
        );
      } else {
        console.log(
          `[jscpd-go] baseline 已写入 ${path.relative(ROOT, BASELINE)} (${current.length} 对)`,
        );
      }
      process.exitCode = 0;
      return; // finally 清理 tmp
    ...
    if (!base) {
      if (JSON_MODE)
        console.log(
          JSON.stringify({ _summary: { ok: false, error: "no-baseline", language: FORMAT } }),
        );
      else
        console.error(
          `[jscpd-go] 未找到 baseline (${path.relative(ROOT, BASELINE)})。\n首次接入请运行: node scripts/jscpd-go.ts --update`,
        );
      process.exitCode = 2;
      return; // finally 清理 tmp
    }
    ...
    if (!JSON_MODE && currentProd.length !== current.length) {
      console.log(
        `[jscpd-go] 过滤 ${current.length - currentProd.length} 个测试文件重复对(不算技术债)，剩 ${currentProd.length} 个生产重复对`,
      );
    }
    warnStale(base);
    const baseSet = new Set(filterTestDupes(base.clones || []));
    const added = currentProd.filter((p) => !baseSet.has(p));
    const fixed = filterTestDupes(base.clones || []).filter(
      (p: string) => !currentProd.includes(p),
    );
    ...
    if (added.length === 0) {
      if (JSON_MODE) console.log(JSON.stringify({ _summary: summary }));
        ...
        console.log(
          `[jscpd-go] ✅ 通过:无新增重复对 (baseline ${base.clones.length} 对, 已修复 ${fixed.length} 对可 --update 收紧)`,
        );
      process.exitCode = 0;
      return; // finally 清理 tmp
    }
    if (JSON_MODE) {
      ...
      console.error(
        `[jscpd-go] ❌ 门禁失败:新增 ${added.length} 个重复对 (其中 ${drifts.length} 个疑似搬迁漂移)`,
      );
      for (const p of added.slice(0, 40)) {
        ...
        if (d) {
          const kind =
            d.type === "exact" ? "搬迁(basename 集相同)" : `拆/并文件(共享 ${d.shared.join(", ")})`;
      ...
      if (orphan.length > 0) {
        console.error(
          `[jscpd-go] 消失旧对 ${orphan.length} 个（未被上方漂移解释，若非治理成果请研判）:`,
        );
      ...
      if (verbose) {
        ...
        for (const p of added.slice(0, 10)) {
          for (const d of (dupIndex.get(p) || []).slice(0, 2)) {
            const head = String(d.fragment || "")
              .split("\n")
              .slice(0, 3)
              .join(" | ")
              .slice(0, 200);
            console.error(
              `     ${p}: L${d.firstFile.start}-L${d.firstFile.end} ↔ L${d.secondFile.start}-L${d.secondFile.end} :: ${head}`,
            );
          }
        }
      }
