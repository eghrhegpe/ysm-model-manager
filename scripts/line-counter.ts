...
function extractFunctions(file) {
  ...
  const out: { name: string; kind?: string; startLine: number; lines: number; truncated: boolean }[] = [];
...
function runFuncsMode(args) {
  ...
  const items: { file: string; name: string; kind: string; startLine: number; lines: number; tier: string; truncated: boolean }[] = []; // { file, rel, name, kind?, startLine, lines, tier, truncated }
...
function main() {
  ...
  if (args.funcs) {
    try {
      ...
      if (args.json) {
        process.stdout.write(JSON.stringify({ ok: false, mode: 'funcs', error: e && (e as Error).message, stack: e && (e as Error).stack }, null, 2) + '\n');
      } else {
