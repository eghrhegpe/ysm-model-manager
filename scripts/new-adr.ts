 ...
 *
 * 用法：
 *   node scripts/new-adr.ts "标题" [--slug kebab-name] [--related 关联内容] [--supersedes ADR-0XX,...]
...
function usage() {
  console.error(
    '用法: node scripts/new-adr.ts "标题" [--slug kebab-name] [--related 关联内容] [--supersedes ADR-0XX,...] [--dry-run]\n' +
...
function maxFromFiles() {
  ...
  return fs
    ...
    .filter(Boolean)
    .reduce((m, s) => Math.max(m, parseInt(s as string, 10)), 0);
}
...
function main() {
  ...
  if (args.dryRun) {
    ...
    console.log(
      `[占号] 最大编号 ${pad(maxNumPre)} → 新编号 ADR-${nPre}（文件 ADR-${nPre}-${slug}.md）`,
    );
  ...
  if (!acquireLock()) {
    ...
    console.error(
      `[协调] 当前实际最大编号为 ADR-${pad(curMax)}；请稍后重试，或先跑 --dry-run 确认最新编号`,
    );
    return 1;
  }
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      ...
      console.error(
        `[协调] 当前实际最大编号为 ADR-${pad(curMax)}；请从 ADR-${pad(curMax + 1)} 起重新占号，或先跑 --dry-run 确认最新编号`,
      );
      return 1;
    }
    ...
    fs.writeFileSync(
      filePath,
      buildTemplate(num, args.title, slug, args.related as string),
      "utf8",
    );
