 ...
 * release-notes-gen.ts — 发布说明生成器
 * 设计意图：发布说明生成器
 * 依赖：node:path / node:url / scripts/_lib/{scan-files,proc}.mjs
 * 用法：
 *   node scripts/release-notes-gen.ts                 # 默认行为（输出 JSON 数据）
 *   node scripts/release-notes-gen.ts --check         # 漂移校验：git tag 必须有对应发版说明 md
...
const UNKNOWN_FLAGS = argv.filter(
  (a) => a.startsWith("--") && !["--check", "--json", "--help", "-h"].includes(a),
);
...
function collect() {
  ...
  // 3. commit 归类
  const categories: Record<string, { hash: string; message: string }[]> = {
    feat: [],
    fix: [],
    docs: [],
    refactor: [],
  ...
  const output: Record<string, any> = {
    ...
    categories: Object.fromEntries(
      Object.entries(categories).map(([k, v]) => [k, v.map((c) => c.message)]),
    ),
    stats: {
  ...
  if (uncommitted.length) {
    ...
    const modifiedFiles = uncommitted
      .filter((l) => !l.startsWith("??") && l.slice(0, 2).trim())
      .map((l) => l.slice(3).trim());
    const deletedFiles = uncommitted
...
function checkReleaseNotes() {
  ...
  const r = runProc("git", ["tag", "--list", "v*", "--sort=version:refname"], {
    timeout: 30000,
    cwd: ROOT,
  });
  if (!r.ok) {
  ...
  const missing = tags.filter((t) => {
    ...
    return (
      !fs.existsSync(path.join(RELEASES_DIR, `${t}.md`)) &&
  ...
  if (missing.length) {
    console.error(
      `❌ ${missing.length} 个版本缺发版说明（docs/releases/<tag>.md）——发版契约（docs/releases/index.md）要求每个正式 tag 有说明。`,
    );
    console.error("");
    console.error(
      "  如何修（AI 可执行）：对每个缺失 tag，用下方命令收集数据后参照 docs/releases/v1.8.8.md 模板补写 vX.md，再重跑 --check 验证：",
    );
    ...
    console.error("");
    console.error("  补写后提交；重跑 node scripts/release-notes-gen.ts --check 验证转绿。");
    process.exit(1);
  }
