...
async function variantA() {
  ...
  console.log(
    `① 临时 index 已构建，staged: ${git(["diff", "--cached", "--name-only"], dir, env).split("\n").filter(Boolean).join(", ")}`,
  );
  console.log(`② spawn commit（钩子 sleep 2s）… 1s 后 kill`);
  const child = spawn("git", ["commit", "-m", "intr-A"], {
    cwd: dir,
    env,
  ...
  console.log(
    `⑤ 主 index staged: ${git(["diff", "--cached", "--name-only"], dir).split("\n").filter(Boolean).join(", ") || "(空)"}`,
  );
...
async function variantB() {
  ...
  const child = spawn("git", ["commit", "-m", "intr-B"], {
    cwd: dir,
    env,
    stdio: ["ignore", "ignore", "ignore"],
  });
  ...
  console.log(
    `⑥ 主 index staged（含陈旧 other.ts + 应被 reset 的 a.ts）: ${git(["diff", "--cached", "--name-only"], dir).split("\n").filter(Boolean).join(", ") || "(空)"}`,
  );
  // 这正是实战现场：commit 落地、临时 index 在、主 index 有陈旧暂存
  const ok = listResidue(gitDir).length > 0 && headB.includes("intr-B");
  console.log(
    `   → 判定: ${ok ? "✅ 复现成功（HEAD 推进 + 临时 index 残留 + 主 index 陈旧暂存）" : "⚠️ 不符合预期"}`,
  );
  fs.rmSync(dir, { recursive: true, force: true });
  return ok;
}
...
try {
  ...
  console.log(
    `  变体 A  ${a ? "✅" : "❌"}  commit 未完成被中断 → 临时 index 残留（finally 没机会跑）`,
  );
  console.log(
    `  变体 B  ${b ? "✅" : "❌"}  commit 已完成但清理未跑 → 临时 index + 主 index 陈旧暂存（实战场景）`,
  );
