...
import { gitMaybe, showAt } from "./_lib/git-ref.ts";
import { parseArgs } from "./_lib/parse-args.ts";
import { ROOT, toPosix, walk } from "./_lib/scan-files.ts";
import { getExportedSymbolsAny, searchName, topDeclsAny } from "./_lib/source-graph.ts";

function analyze(commit: string) {
  const parent = `${commit}^`;
  // 注意：`git diff --name-only parent commit` 不带 `--` 在 ref 前，
  // 否则 parent/commit 会被当成路径 spec 处理，返回空。
  const modified = (gitMaybe(["diff", "--name-only", parent, commit]) || "")
    .trim()
    .split("\n")
    .filter(Boolean);
  const parents: Record<string, any> = {},
    news: Record<string, any> = {},
    parentExps: Record<string, any> = {};
  for (const p of modified) {
    const pt = showAt(parent, p);
    const nt = showAt(commit, p);
    if (pt !== null) {
      parents[p] = new Set(topDeclsAny(p, pt));
      parentExps[p] = new Set(getExportedSymbolsAny(p, pt));
    }
  ...
  for (const p of modified) {
    ...
    for (const sym of old) {
      if (!nw?.has(sym)) removed.push({ sym, file: p, wasExport: parentExps[p]?.has(sym) });
    }
  }
...
function scanCallers(searchTerms: string[], scope: string) {
  ...
  if (scope) {
    ...
    scanDirs.push(`${ROOT}/go`);
    scanDirs.push(`${ROOT}/frontend/src`);
  }
...
const result = analyze(commitArg) as {
  commit: string;
  removed: { sym: any; file: string; wasExport: any }[];
  callers: Map<string, string[]>;
};
if (result.removed.length)
  result.callers = scanCallers(
    result.removed.map((r) => r.sym),
    SCOPE as string,
  );
