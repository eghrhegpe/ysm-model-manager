...
import { getRoot, relPosix, toPosix } from "./_lib/scan-files.ts";
...
function repoRel(p: string) {
  ...
  if (rel.startsWith("frontend/") || rel.startsWith("go/") || rel.startsWith("internal/"))
    return rel;
...
if (jsonMode) {
  ...
  process.stdout.write(
    `来源: ${toPosix(relPosix(inputPath))} · 共 ${totalFiles} 个源文件 · 平均语句覆盖率 ${overall}% · 待补测 ${visibleRows.length} 个\n\n`,
  );
