...
function assertExtensionsDerived() {
  ...
  const viaSchema = /import\s+\{[^}]*allResourceTypes[^}]*\}\s+from\s+["']\.\/schema\.ts["']/.test(
    text,
  );
  if (!derives || (!viaDirectImport && !viaSchema)) {
    throw new Error(
      "extensions.ts 未从 resource_types.json 派生 RESOURCE_EXTS（ADR-014/T2 单一事实来源被破坏，勿手写副本）",
    );
  }
}
...
function emit(issues: number) {
  if (jsonMode) {
    process.stdout.write(
      `${JSON.stringify({ _summary: { issues }, issues: issues ? [{ type: "fatal", id: "", detail: "派生守卫失败" }] : [] }, null, 2)}\n`,
    );
  } else if (issues) {
...
try {
  ...
  if (jsonMode) {
    process.stdout.write(
      `${JSON.stringify({ _summary: { issues: 9999 }, issues: [{ type: "fatal", id: "", detail: `派生守卫失败: ${e?.message || e}` }] }, null, 2)}\n`,
    );
  } else {
