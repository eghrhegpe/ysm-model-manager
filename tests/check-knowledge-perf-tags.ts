...
let r: ReturnType<typeof runScript> | undefined;
...
try {
  ...
  ok(
    ...
    `不应出现 perf 相关 ERROR: ${legalOut.errors
      .filter((e) => e.includes("zzz-perf-contract-tmp"))
      .join("; ")
      .slice(0, 200)}`,
  );
  ...
  ok(
    "非法标签报 ERROR 含词表提示",
    badOut.errors.some(
      (e) => e.includes("zzz-perf-contract-tmp") && e.includes("perf") && e.includes("cpu-bound"),
    ),
...
ok(
  "索引含性能列",
  /\|\s*标识\s*\|\s*名称\s*\|\s*tier\s*\|\s*性能\s*\|/.test(indexText),
  "表头缺「性能」列",
);
ok(
