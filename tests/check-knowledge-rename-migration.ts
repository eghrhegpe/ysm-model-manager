...
try {
  ...
  ok(
    "rename 路径 → ERROR 且含迁移建议",
    e1.some((e) => e.includes(RENAME_HINT)),
    `期望含迁移建议: ${e1.join("; ").slice(0, 300)}`,
  );
  ok(
    "迁移建议给出新路径",
  ...
  ok(
    "无 rename 记录的 404 → ERROR 仍报「引用不存在」",
    e2.some((e) => e.includes("引用不存在")),
    `期望含引用不存在: ${e2.join("; ").slice(0, 300)}`,
  );
  ok(
  ...
  ok(
    "存在且未 rename 的文件 → 无 ERROR",
    e3.length === 0,
    `不应有针对临时卡的 ERROR: ${e3.join("; ").slice(0, 300)}`,
  );
} finally {
