...
import {
  ...
  formatCovTime,
  MAX_SUGGEST_FILES,
} from "../scripts/hooks/coverage-suggest-hint.ts";
...
check("buildBlock 含 uncoveredRanges", () => {
  const block = buildBlock([
    { file: "frontend/src/a.ts", stmts: 30.5, uncoveredRanges: "34, 56-58" },
  ]);
...
check("--suggest 解析真实 coverage（低覆盖率文件 > 0）", () => {
  if (!fs.existsSync(COV)) {
    console.log(
      "  ↳ skip: 无 frontend/coverage/coverage-final.json（CI 未跑 vitest --coverage），跳过真实数据断言",
    );
    return;
  }
  const out = execFileSync(
    process.execPath,
    [path.join(ROOT, "scripts/test-coverage-report.ts"), "--suggest", "--json"],
    { encoding: "utf8" },
  );
  ...
  assert.ok(
    j._summary.thresholdStmts === cfgStmts,
    `阈值应读 vitest.config.ts（配置 ${cfgStmts}，实际 ${j._summary.thresholdStmts}）`,
  );
