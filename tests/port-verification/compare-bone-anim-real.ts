...
const MODEL_DIR =
  process.env.YSM_MODEL_DIR ||
  join(REPO, "upstream", "[YSM模型]官方开源wine_fox_json", "01_taisho_maid");
...
function eulerZYXQuat(rxD, ryD, rzD) {
  const hx = (rxD * Math.PI) / 180 / 2,
    hy = (ryD * Math.PI) / 180 / 2,
    hz = (rzD * Math.PI) / 180 / 2;
...
const geo =
  rawGeo["minecraft:geometry"]?.[0] ??
  Object.values(rawGeo).find((v) => v && Array.isArray(v.bones));
...
function gameWorldMats(params) {
  ...
  const calc = (idx) => {
    ...
    const rx = params[off],
      ry = params[off + 1],
      rz = params[off + 2];
    const tx = params[off + 3],
      ty = params[off + 4],
      tz = params[off + 5];
    const sx = params[off + 6],
      sy = params[off + 7],
      sz = params[off + 8];
...
function ourWorldMats(params) {
  ...
  for (let i = 0; i < bones.length; i++) {
    ...
    m = matMul(
      m,
      quatToMat(eulerZYXQuat(rx / (Math.PI / 180), ry / (Math.PI / 180), rz / (Math.PI / 180))),
    );
    if (sx !== 1 || sy !== 1 || sz !== 1) m = matScale(m, sx, sy, sz);
    if (sx === 0 && sy === 0 && sz === 0) visible[i] = false;
    world[i] =
      bone.parentIdx !== -1 && world[bone.parentIdx] ? matMul(world[bone.parentIdx], m) : m;
  }
...
try {
  ...
  /* 无 ysm.json → 兜底 main.animation.json */
}
if (!animPath) {
...
if (animPath) {
  try {
    ...
    [clipName, clip] = (clipArg ? entries.find(([n]) => n === clipArg) : null) ||
      entries[0] || [null, null];
  } catch {
    clipName = "静态";
    clip = null;
...
for (let i = 0; i < bones.length; i++) {
  ...
  rows.push({
    ...
    rotDelta,
    fail,
    gHidden: !game.visible[i],
    oHidden: !ours.visible[i],
  });
}

console.log(
  `════ 真实模型逐骨骼对拍（${basename(MODEL_DIR)} · main.json ${bones.length} 骨骼 · ${clipName === "静态" ? "静态姿态（无动画）" : `clip「${clipName}」t=${t.toFixed(2)}s`}）════\n`,
);
console.log(
  `分歧骨骼: ${failCount}/${bones.length}  最大 posΔ=${maxPos.toFixed(4)}（${maxPosBone}） 最大 rotΔ=${maxRot.toFixed(4)}（${maxRotBone}）\n`,
);
if (failCount > 0) {
  ...
  for (const r of rows.filter((r) => r.fail).slice(0, 15)) {
    console.log(
      `${r.name.padEnd(28)}| ${r.posDelta.toFixed(4).padStart(8)} | ${r.rotDelta.toFixed(4).padStart(8)} | ${r.gHidden !== r.oHidden ? "⚠ 游戏内隐藏≠本项目" : r.gHidden ? "双方隐藏" : "可见"}`,
    );
  }
  console.log("─".repeat(70));
}
