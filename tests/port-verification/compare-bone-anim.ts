...
function ourBoneTransform(pivot, animR, animT, animS) {
  ...
  const pos = [(pivot[0] - animT[0]) / 16, (pivot[1] + animT[1]) / 16, (pivot[2] + animT[2]) / 16];
  const s = [...animS];
  return { q, pos, s };
}
...
function gamePivotPos(gm, pivot, animT) {
  void gm;
  return [(pivot[0] - animT[0]) / 16, (pivot[1] + animT[1]) / 16, (pivot[2] + animT[2]) / 16];
}
...
console.log(
  `${"case".padEnd(34)}| ${"游戏内位置".padEnd(22)}| ${"本项目位置".padEnd(22)}| posΔ | rotΔ |`,
);
...
console.log("\n说明：");
console.log(
  "  - 游戏内：M = T(-pos/16)·T(pivot/16)·Rz·Ry·Rx·T(-pivot/16)，posX 取负，scale=0 隐藏",
);
console.log(
  "  - 本项目：rotation=Euler(rz,ry,rx,'ZYX')；position/scale 直接覆盖（ysm-animation-player L113-128）",
);
