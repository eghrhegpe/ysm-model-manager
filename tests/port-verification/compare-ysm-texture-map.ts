...
function compareModel(modelDir) {
  ...
  const { playerTextures, projectiles } = parseYsmFiles(ysmPath);
  ...
  console.log(
    `ysm.json projectiles:     ${projectiles.map((p) => `${p.model}->${p.texture}`).join(" ") || "(无)"}`,
  );
  ...
  console.log(
    `咱们 pngOrder (前5):      ${pngOrder.slice(0, 5).join(", ")}${pngOrder.length > 5 ? "..." : ""}`,
  );
