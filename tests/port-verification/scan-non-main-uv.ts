...
function scanModel(modelDir) {
  ...
  const mainProfile = modelProfiles.find(
    (p) => !p.isNonMain && (p.file.includes("main") || p.file.includes("Main")),
  );
...
for (const r of results) {
  ...
  for (const p of r.modelProfiles) {
    ...
    console.log(
      `   ${filePadded} texW=${texWPadded} texH=${texHPadded} cubes=${cubePadded}${tag} ${tex}`,
    );
  }
