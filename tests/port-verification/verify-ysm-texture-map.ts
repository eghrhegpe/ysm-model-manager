...
// ===== 模拟"直读 files 段"的纹理分配 =====
function simulateDirectFilesAllocation(files, _modelDir) {
...
for (const r of results) {
  ...
  console.log(
    `   主体模型: [${r.alloc.playerModels.map((m) => `${m.key}=${m.modelFile}`).join(", ")}]`,
  );
  console.log(
    `   投射物: [${r.alloc.projectiles.map((p) => `${p.modelFile}->${p.texture}`).join(", ")}] || (无)'}`,
  );
