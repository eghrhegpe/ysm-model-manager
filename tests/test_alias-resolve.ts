...
import {
  classifyImport,
  loadAliases,
  resolveAliasToSrcRel,
  SRC_ROOT,
  tryResolveAlias,
} from "../scripts/_lib/alias-resolve.ts";
...
check("loadAliases 解析 ≥13 个别名（12 个 @/dir + #root）", () => {
  ...
  assert.ok(
    a.some((e) => e.prefix === "@/preview-3d"),
    "缺 @/preview-3d",
  );
  assert.ok(
    a.some((e) => e.prefix === "#root"),
    "缺 #root",
  );
...
check("tryResolveAlias #root/resource_types.json → REPO_ROOT/resource_types.json", () => {
  assert.strictEqual(
    tryResolveAlias("#root/resource_types.json"),
    path.join(ROOT, "resource_types.json"),
  );
});
...
check(
  "classifyImport 未登记别名 @/nope/x → resolved:false（catch-all 已禁，双写一致性兜底）",
  () => {
    const fromFile = path.join(SRC_ROOT, "views", "v.ts");
...
check(
  "classifyImport e2e/mock-data.ts 真实引用形状（3 层 ../ → frontend/e2e）→ isMockData:true 且越界",
  () => {
    const fromFile = path.join(SRC_ROOT, "core", "handlers", "sync.test.ts");
    const c = classifyImport("../../../e2e/mock-data.ts", fromFile);
    assert.ok(
      c.resolved && c.isMockData,
      "应标记 mock-data（真实位置 frontend/e2e），R4 内定入基线",
    );
...
check("gate 集成：check-path-hygiene --json 的 r4Count === 冻结基线（防漂移静默）", () => {
  ...
  const baseline = JSON.parse(
    readFileSync(path.join(ROOT, "docs", ".path-hygiene-baseline.json"), "utf-8"),
  ).crossBoundaryNonBindings;
  assert.strictEqual(out._summary.r4_cross_boundary.count, baseline, "实时计数必须等于冻结基线");
  assert.strictEqual(
    out._summary.r4_cross_boundary.baseline,
    baseline,
    "脚本侧 baseline 读取应与文件一致",
  );
});
