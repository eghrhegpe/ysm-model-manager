...
const FRONTEND_BRIDGE_FILES = [path.join("frontend", "src", "backend", "platform.ts")];
...
if (main !== null) {
  ...
  if (injectIdx === -1 || loadIdx === -1) {
    errors.push(
      `${MAIN_ACTIVITY}: 未找到 addJavascriptInterface 或 loadUrl（注入机制重构？需同步本测试）`,
    );
...
if (bridgeSrc !== null) {
  ...
  for (const m of bridgeSrc.matchAll(re)) annotated.add(m[1]);
  for (const name of [...PROBED_METHODS, ...CORE_METHODS]) {
    if (!annotated.has(name)) {
      errors.push(
        `${JS_BRIDGE}: 方法 ${name} 缺 @JavascriptInterface（API 17+ 未注解方法不暴露给 JS，前端探测静默假阴性）`,
      );
    }
  }
}
...
if (proguard !== null) {
  for (const cls of ["com.wails.app.WailsJSBridge", "com.wails.app.WailsBridge"]) {
    if (!proguard.includes(`-keep class ${cls}`)) {
      errors.push(
        `${PROGUARD}: 缺 "-keep class ${cls} { *; }"，release 构建混淆剥离后前端探测假阴性`,
      );
    }
  }
}
...
if (feParts.length > 0) {
  if (!fe.includes(".wails")) {
    errors.push(
      `${FRONTEND_BRIDGE_FILES.join(" / ")}: 未探测 window.wails（Java 侧注册名或本文件漂移，两侧需同步）`,
    );
  }
  for (const name of PROBED_METHODS) {
    if (!fe.includes(name)) {
      errors.push(
        `${FRONTEND_BRIDGE_FILES.join(" / ")}: 探口不再引用 ${name}，而 ${JS_BRIDGE} 仍暴露——删除 Java 侧方法前先同步本测试`,
      );
    }
  }
}
...
console.log(
  "OK: android bridge injection-order + @JavascriptInterface + proguard + naming parity checks passed",
);
