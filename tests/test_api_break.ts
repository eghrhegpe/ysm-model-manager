...
// ── 1. 缺参数 → 退出码 2 ──────────────────────────
{
  ...
  assert(
    r.status === 2,
    `缺参数应退出 2，实际 ${r.status}（stderr=${(r.stderr || "").slice(0, 100)}）`,
  );
...
// ── 2. 相同 ref → 无破坏性变更 ─────────────────────
{
  ...
  assert(
    r.status === 0,
    `相同 ref 应退出 0，实际 ${r.status}（stderr=${(r.stderr || "").slice(0, 200)}）`,
  );
  let j = null;
  try {
  ...
  if (j) {
    ...
    assert(
      typeof j.fileSummary === "object" && "modified" in j.fileSummary,
      "fileSummary 字段应齐全",
    );
  }
}
...
// ── 3. 有破坏性变更的 ref 对（CLI 移除提交）─────────
{
  ...
  assert(
    r.status === 0,
    `破坏性变更 ref 对应退出 0，实际 ${r.status}（stderr=${(r.stderr || "").slice(0, 200)}）`,
  );
  assert(
    !(r.stderr || "").includes("fatal:"),
    `stderr 不应有 fatal 噪声：${(r.stderr || "").slice(0, 200)}`,
  );
  let j = null;
  try {
    j = JSON.parse(r.stdout);
  ...
  if (j) {
    ...
    assert(
      typeof j.callers === "object" && j.callers !== null,
      "callers 应存在（导出消失时的调用方扫描）",
    );
    const deletedSyms = Object.keys(j.callers);
    assert(
      deletedSyms.some((s) => s === "CLIMain"),
      `应检出 CLIMain（实际缺失，共 ${deletedSyms.length} 个：${deletedSyms.slice(0, 10).join(",")}）`,
    );
...
// ── 4. 中文路径 ref 对（novel 章节提交，diff 含 03-UI器官/06-幽灵的渲染.md）──
{
  ...
  assert(
    r.status === 0,
    `中文路径 ref 对应退出 0，实际 ${r.status}（stderr=${(r.stderr || "").slice(0, 200)}）`,
  );
  assert(
    !(r.stderr || "").includes("fatal:"),
    `中文路径不应触发 fatal（quotepath 修复）：${(r.stderr || "").slice(0, 200)}`,
  );
...
// ── 5. --redline 模式：新增文件超红线 → 退出码 1 ───
{
  ...
  assert(
    r.status === 1,
    `新增超红线文件时 --redline 应退出 1，实际 ${r.status}（stderr=${(r.stderr || "").slice(0, 200)}）`,
  );
  let j = null;
  try {
    j = JSON.parse(r.stdout);
  ...
  if (j) {
    assert(j.redline && j.redline.over > 0, `redline.over 应 > 0，实际 ${j.redline?.over}`);
    assert(
      j.redline.files.some((f) => f.path.includes("web-fs.ts")),
      "redline.files 应含新增的 web-fs.ts",
    );
  }
}
