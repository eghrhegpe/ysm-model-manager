...
import {
  ...
  extractImported,
  splitBlockEntries,
} from "../scripts/auto-import-symbols.ts";
...
check("tokenize 剥离注释/字符串并收集标识符", () => {
  ...
  assert.ok(
    names.includes("const") && names.includes("a") && names.includes("b"),
    `标识符应收集: ${names}`,
  );
});

check("tokenize 剥离模板字面量（含插值不分析）", () => {
  // biome-ignore lint/suspicious/noTemplateCurlyInString: 模板字面量插值正是被测 fixture 本体（tokenize 须跳过它）
...
check("checkFile 检出缺失 import 的符号", () => {
  ...
  const _file = path.join(dir, "views/app-preview/detail.ts"); // 真实存在的 .ts，仅借用路径
  ...
  fs.unlinkSync(tmp);
  assert.ok(
    found.some((f) => f.symbol === "SOME_MISSING_SYMBOL"),
    `应检出 SOME_MISSING_SYMBOL，实际 ${JSON.stringify(found)}`,
  );
...
check("checkFile 不误报：已定义/已导入/全局/属性访问", () => {
  ...
  assert.deepEqual(
    found,
    [],
    `不应误报（已导入/定义/全局/属性访问），实际 ${JSON.stringify(found)}`,
  );
});
...
check("run 全量扫描当前仓库 0 缺失（与拆分前基线一致）", () => {
  const out = execFileSync(
    process.execPath,
    [path.join(ROOT, "scripts", "auto-import.ts"), "--json"],
    {
      cwd: ROOT,
      encoding: "utf8",
    },
