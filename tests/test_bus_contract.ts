...
function runOnFixture(files) {
  ...
  const r = spawnSync(process.execPath, [GUARD, "--root", tmp, "--strict", "--json"], {
    encoding: "utf-8",
  });
  let json = null;
  try {
...
console.log("[1] fixture 违例检测");
{
  ...
  ok(
    status === 1 && json !== null,
    `--strict 下违例阻断且输出 JSON（exit=${status}）${json ? "" : ` stderr=${err.slice(0, 200)}`}`,
  );
  ...
  ok(
    (s.arityIssues ?? []).some(
      (a) => a.type === "missing_payload" && a.file.includes("index.html"),
    ),
    "html 内联可选链缺参也被抓",
  );
  ok(
    !(s.undeclared ?? []).includes("ghost") &&
      !(s.arityIssues ?? []).some((a) => a.event === "ghost"),
    "注释内调用不误报",
  );
}
...
console.log("[1b] 跨行 on / 跨行 emit");
{
  ...
  const s = json?._summary ?? {};
  ok(
    (json?.events?.["a:void-event"]?.on?.length ?? 0) === 1,
    "跨行 bus.on( 订阅被记录（不再误报孤儿发射）",
  );
  ok(
    status === 1 &&
      (s.arityIssues ?? []).some((a) => a.type === "missing_payload" && a.event === "b:typed"),
    "跨行 emit 缺参 → missing_payload",
  );
  ok(
    (s.arityIssues ?? []).some((a) => a.type === "void_with_payload" && a.event === "a:void-event"),
...
console.log("[1c] 正则字面量不干扰实参提取");
{
  ...
  ok((json?.events?.["b:typed"]?.on?.length ?? 0) === 1, "回调体含正则字面量的订阅仍被记录");
}
...
console.log("[2] VOID_EVENTS 漂移检测");
{
  ...
  ok(
    status === 1 && (json?._summary?.voidDrift ?? []).some((v) => v.event === "a:void-event"),
...
console.log("[2b] fn 所属函数提取");
{
  ...
  ok(
    fnOf(ev["b:typed"]?.emit) === "topLevel",
    `function 声明内 emit → fn=topLevel（实为 ${fnOf(ev["b:typed"]?.emit)}）`,
  );
  ok(
    fnOf(ev["a:void-event"]?.on) === "method",
    `方法简写内 on → fn=method（实为 ${fnOf(ev["a:void-event"]?.on)}）`,
  );
  ok(
    (ev["a:void-event"]?.emit ?? []).some((r) => r.fn === "inner"),
    "箭头赋值内 emit → fn=inner",
  );
  ok(
    (ev["b:typed"]?.emit ?? []).some((r) => r.fn === "outer"),
    "嵌套具名函数不误报（应归 outer 而非 nested）",
  );
  ok(
    (ev["a:void-event"]?.emit ?? []).filter((r) => r.fn === "ctrl").length === 2,
    "if/catch 控制流块内调用归外层 ctrl 而非 if/catch 关键字",
  );
  ok(
    (ev["a:void-event"]?.emit ?? []).some((r) => r.fn === "hdl"),
    "解构参数跨行函数头内调用归 hdl（解构 } 不截断回卷）",
  );
  ok(
    (ev["b:typed"]?.on ?? []).some((r) => r.fn === "(顶层)"),
    "模块顶层调用 → fn=(顶层)",
  );
}
...
console.log("[3] 真实仓库");
{
  const r = spawnSync(process.execPath, [GUARD, "--strict", "--json"], {
    encoding: "utf-8",
    cwd: ROOT,
  });
  let json = null;
