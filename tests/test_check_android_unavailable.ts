...
import { extractAppMethods, extractGuardedAppMethods, parseNameSet } from '../scripts/check-android-unavailable.ts';
...
function runGuard(args: string[]) {
  const res = spawnSync(process.execPath, [path.join(ROOT, 'scripts', 'check-android-unavailable.ts'), ...args], {
    cwd: ROOT,
    encoding: 'utf8',
  });
...
check('extractAppMethods 提取 *App 导出方法', () => {
  const src = 'func (a *App) Foo(x string) error {\n\treturn nil\n}\nfunc helper() {}\nfunc (a *App) Bar() {}\n';
...
check('extractGuardedAppMethods 不认 != "android" 的桌面正向分支', () => {
  const src = ['func (a *App) Startup(ctx context.Context) {', '\tif runtime.GOOS != "android" {', '\t\tsetupTray()', '\t}', '}'].join(
    '\n',
  );
  assert.deepStrictEqual(extractGuardedAppMethods(src), []);
});
...
check('--json 输出合法 JSON 且 _summary 契约键齐全', () => {
  ...
  for (const key of ['ok', 'degraded', 'scanned', 'blacklist', 'uncovered', 'suspects', 'stale', 'testDrift']) {
