...
console.log(`   状态词审查: ${inWhitelist}/${STATUS_WORDS_THAT_SHOULD_PASS.length} 首词已被白名单接受`);

// 状态词首词都应被真实脚本白名单接受（绿阶段；改造后不再缺失）
assert.equal(
  inWhitelist, STATUS_WORDS_THAT_SHOULD_PASS.length,
...
for (const [name, prop] of Object.entries(CHECK_PROPERTIES)) {
  if (name === 'check-boolean-naming') {
    assert.equal(prop.category, 'debt',
      `boolean-naming 应归类为 debt 级（命名风格，不影响运行），实际=${prop.category}`);
  }
}

console.log('  ✓ 阻断策略：boolean-naming / orphan-exports / deadcode-baseline / jscpd-go 应降级为 debt');
...
console.log(`   期望分层: ${hardItems.length} hard + ${debtItems.length} debt + ${fcItems.length} failClosed`);
console.log(`   debt 级: ${debtItems.map(([k]) => k).join(', ')}`);

assert.ok(debtItems.length >= 7, '至少 7 个检查应降级为 debt（boolean/orphan/deadcode/jscpd/circular/proc/lib-adoption）');
...
try {
  ...
  const matched = fixtureBooleans.filter((n) => names.has(n) || [...firstWords].some((w) => n.toLowerCase().includes(w)));
  console.log(`   抽查 ${fixtureBooleans.length} 个真实 boolean 变量 → ${matched.length} 个在 findings 里`);
...
console.log('   2. VALID_PREFIXES 应扩展状态词：busy/disposed/changed/orbit/cast/depth/wireframe...');
