...
const UI_FILE = path.join(SRC_ROOT, 'utils', 'format', 'bytes.ts');    // 异目录目标

function runHygiene(args) {
  try {
    const out = execFileSync(process.execPath, [path.join(ROOT, 'scripts', 'check-path-hygiene.ts'), ...args], {
      cwd: ROOT,
      encoding: 'utf8',
    });
    return { rc: 0, out };
...
check('R6 极端-A2：测试 @/dir/index 桶 → testBarrelEntry=true', () => {
  assert.equal(classifyBarrelHygiene('@/views/app-tree/index', VIEW_TEST, true).testBarrelEntry, true);
});
...
check('R5 极端-B1：backend 内 @/backend/runtime.ts 同目录别名 → sameDirAlias=true', () => {
  assert.equal(classifyBarrelHygiene('@/backend/runtime.ts', BACKEND_FILE, false).sameDirAlias, true);
});
check('R5 极端-B2：异目录 @/utils/format/bytes.ts 合法别名 → sameDirAlias=false', () => {
  assert.equal(classifyBarrelHygiene('@/utils/format/bytes.ts', BACKEND_FILE, false).sameDirAlias, false);
});
