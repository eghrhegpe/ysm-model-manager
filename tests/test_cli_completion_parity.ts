...
for (const f of FILES) {
  must(
    fs.existsSync(path.join(COMP_DIR, f)),
    `MISSING: completions/${f}（运行 node scripts/gen-cli-completion.ts）`,
  );
}
...
if (errors.length) {
  ...
  console.error(
    "  提示：命令/子命令变更后运行 `node scripts/gen-cli-completion.ts` 刷新 completions/。",
  );
  process.exit(1);
}
