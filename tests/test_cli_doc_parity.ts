...
const _DOC = path.join(ROOT, "docs", "cli-commands.md");
const _AGENTS = path.join(ROOT, "AGENTS.md");
...
const goFiles = fs.readdirSync(CLI_DIR).filter((f) => f.endsWith(".go") && !f.endsWith("_test.go"));
const registered = new Set();
for (const f of goFiles) {
  const text = readOrDie(`go/cli/${f}`);
  const re = /RegisterCommandC\(\s*"([a-z0-9-]+)"/g;
  for (const m of text.matchAll(re)) registered.add(m[1]);
}
...
{
  const re = /^### `([a-z0-9-]+)`$/gm;
  for (const m of docText.matchAll(re)) documented.add(m[1]);
}
...
must(
  registered.size >= 38,
  `顶层命令数异常（期望 ≥38，实际 ${registered.size}）——注册表被大量删除时需人工确认`,
);
...
if (errors.length) {
  ...
  console.error(
    "  提示：命令/子命令/选项变更后运行 `node scripts/gen-cli-doc.ts` 刷新 docs/cli-commands.md。",
  );
  process.exit(1);
}
console.log(
  `✅ 契约测试通过：CLI 注册表 ↔ 文档 parity 一致（${registered.size} 个顶层命令全部在册，AGENTS.md 入口指向生成文档）`,
);
process.exit(0);
