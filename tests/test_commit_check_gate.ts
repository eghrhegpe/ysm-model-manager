...
function fakeRun(canned: Record<string, { out: string; ok?: boolean; rc?: number }>) {
  return (_bin: string, args: string[]): ProcResult => {
    ...
    const c: ProcResult = (hit ? canned[hit] : undefined) ?? { out: "{}" };
...
const redline = (summary: object) =>
  JSON.stringify({
    _summary: { ok: true, scanHealthy: true, newViolations: 0, baselineViolations: 5, ...summary },
  });
const driftJson = (summary: object) =>
  JSON.stringify({ _summary: { errors: 0, warns: 0, ...summary } });
...
const main = async () => {
  ...
  await check("红线健康零违规 → allow", async () => {
    const r = await runCommitChecks(["docs/knowledge/x.md"], {
      run: fakeRun(green),
      runTests: noTests,
    });
