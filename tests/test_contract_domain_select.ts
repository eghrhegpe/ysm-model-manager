...
check(
  goSet.length > 0 && goSet.length < all.length,
  `纯 go：子集应 >0 且 < 全量（${all.length}），实为 ${goSet.length}`,
);
...
check(
  frontSet.length > 0 && frontSet.length < all.length,
  `纯前端：子集应 >0 且 < 全量，实为 ${frontSet.length}`,
);
...
check(
  !has(frontSet, "test_cli_completion_parity.ts"),
  "前端子集不应含纯 go test_cli_completion_parity",
);

const dataSet = selectContractTests(["data"]);
check(dataSet.length === 3, `纯 data：应恰 3 个 schema 测试，实为 ${dataSet.length}`);
check(
  has(dataSet, "test_resource_schema.ts") &&
    has(dataSet, "test_creators_schema.ts") &&
    has(dataSet, "test_workshop_schema.ts"),
  "data 子集应含 3 个 schema 测试",
);

const docsSet = selectContractTests(["docs"]);
check(
  docsSet.length > 0 && docsSet.length < all.length,
  `纯 docs：子集应 >0 且 < 全量，实为 ${docsSet.length}`,
);
check(
  has(docsSet, "check-knowledge-drift-affected.ts"),
  "docs 子集应含 check-knowledge-drift-affected",
);
...
const MIXED = [
  "test_android_bridge_contract.ts",
  "test_cli_gui_flow_contract.ts",
  "test_config_syntax.ts",
  "test_cube_uv_quad_vertex.ts",
];
...
check(
  precise.length < all.length,
  `按文件裁剪：应小于全量（${all.length}），实为 ${precise.length}`,
);
check(
  has(precise, "test_commit_temp_index.ts"),
  "改 commit-temp-index 应触发 test_commit_temp_index",
);
check(
  !has(precise, "test_domain_classify.ts"),
  "改 commit-temp-index 不应触发 test_domain_classify",
);
// 精确化验证（ADR-156 后续）：原 scripts/ 宽哨兵已收敛为具体脚本清单
check(
  !has(precise, "test_scripts_json.ts"),
  "改 commit-temp-index 不应触发 test_scripts_json（已精确化）",
);
...
check(
  has(redlinesHit, "test_scripts_json.ts"),
  "改 check-redlines（11 个 --json 脚本之一）应触发 test_scripts_json",
);
check(
  has(redlinesHit, "test_redlines_changed_files.ts"),
  "改 check-redlines 应触发 test_redlines_changed_files",
);
...
check(
  !has(sidebarHit, "test_scripts_json.ts"),
  "改 gen-vitepress-sidebar 不应触发 test_scripts_json（精确化）",
);
...
for (const f of all) {
  check(
    Array.isArray(CONTRACT_TEST_DOMAINS[f]) && CONTRACT_TEST_DOMAINS[f].length > 0,
...
for (const f of Object.keys(CONTRACT_TEST_DOMAINS)) {
  check(allBasename.has(f), `${f} 在 CONTRACT_TEST_DOMAINS 但 tests/ 目录不存在（僵尸条目）`);
}
...
for (const f of all) {
  ...
  if (doms.includes("tests")) {
    check(
      Array.isArray(CONTRACT_TEST_TARGETS[f]) && CONTRACT_TEST_TARGETS[f].length > 0,
...
if (failures.length === 0) {
  console.log(
    `✅ test_contract_domain_select 全部通过（${assertCount} 组断言；全量 ${all.length}，go ${goSet.length} / frontend ${frontSet.length} / data ${dataSet.length} / docs ${docsSet.length}）`,
  );
  process.exit(0);
} else {
