 ...
 * 退出码：0 = 通过；1 = 失败。
 */
import { classify, DATA_FILES, planFromFiles } from "../scripts/_lib/domain-classify.ts";
...
assert(
  classify("frontend/src/views/app-preview/index.ts") === "frontend",
  "frontend/ 应归 frontend",
);
...
assert(
  pDocs.go === false && pDocs.frontend === false && pDocs.docs === true,
  "纯 docs：应只跑 docs 域，go/frontend 关闭",
);
...
assert(
  pData.data === true && pData.go === false && pData.frontend === false,
  "纯 data：应跑 data 域，不跑 go/frontend",
);
...
if (failures.length === 0) {
  console.log(
    `✅ test_domain_classify.mjs 全部通过（${DATA_FILES.size} 个 data 文件 + ${assertCount} 组断言）`,
  );
  process.exit(0);
} else {
