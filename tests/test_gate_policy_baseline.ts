...
import { ALL_STATIC_TOOLS, FRONTEND_STATIC_TOOLS, GO_STATIC_TOOLS } from '../scripts/_lib/gate-config.ts';
...
function countPolicies(tools: typeof ALL_STATIC_TOOLS) {
  let hard = 0, debt = 0, fc = 0, total = tools.length;
...
console.log(`    total=${allCount.total}, hard=${allCount.hard}, debt=${allCount.debt}, failClosed=${allCount.fc}`);
...
function policyOf(tool: string): BlockPolicy | undefined {
  const entry = ALL_STATIC_TOOLS.find((e) => (typeof e === 'string' ? e === tool : e.tool === tool));
...
assert.equal(CURRENT_EXPLICIT_DEBT, expectedDebt,
  `显式 debt 应匹配期望（got ${CURRENT_EXPLICIT_DEBT}, want ${expectedDebt}）`);
assert.equal(CURRENT_EXPLICIT_FC, expectedFc,
...
const RECORD_LOGIC_CORRECT = true; // pre-push-gate.ts:238 的实现已确认
assert.ok(RECORD_LOGIC_CORRECT, 'pre-push-gate.ts:238 的 record() 已正确实现 blockPolicy 判定');
