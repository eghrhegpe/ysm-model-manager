...
import {
  normPair, pairsFrom, matchDrift,
  isGoTestFile, isTestOnlyPair, involvesTest, filterTestDupes, classifyDupes,
...
assert.equal(isGoTestFile('go/download/download_zero_coverage_test.go'), true, '覆盖测试也算测试文件');
...
assert.equal(isGoTestFile('go/internal/testutil/testutil.go'), false,
...
console.log(`   ADR-144 扩展案例: ${classifiedReal.prod_prod.length} prod-prod + ${classifiedReal.prod_test.length} prod-test + ${classifiedReal.test_test.length} test-test`);

// 预期：8 个里只有 2 个 prod-prod（archive#maid_l0 + testutil 自引用），其余 6 个 test-test 应豁免
assert.equal(classifiedReal.prod_prod.length, 2,
  `8 个 added 里应 2 个 prod-prod（got ${classifiedReal.prod_prod.length}）——只有 archive#maid_l0 和 testutil 自引用`);
assert.equal(classifiedReal.test_test.length, 6,
...
const jscpdReport = {
  duplicates: [
    { firstFile: { name: 'go\\download\\download_edge_test.go' }, secondFile: { name: 'go\\download\\download_edge_test.go' } },
    { firstFile: { name: 'go\\geometry\\archive.go' }, secondFile: { name: 'go\\geometry\\maid_l0.go' } },
    { firstFile: { name: 'go\\sync\\sync_boundary_test.go' }, secondFile: { name: 'go\\sync\\sync_toggle_hashlock_test.go' } },
  ],
};
