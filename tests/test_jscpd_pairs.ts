...
import { matchDrift, normPair, pairsFrom } from "../scripts/_lib/jscpd-pairs.ts";
...
assert.equal(
  driftReal.filter((d) => d.type === "exact").length,
  1,
  "应恰 1 条 exact（classify.go 搬迁）",
);
assert.equal(
  driftReal.filter((d) => d.type === "partial").length,
  2,
...
console.log(
  `   normPair / pairsFrom / matchDrift(exact+partial) / ADR-144 回归 ${driftReal.length}/3 漂移识别`,
);
