 ...
 *
 * 锁「扫描域 = git 跟踪的 .md ∩ isScannable」这一不变量（2026-09-15，ADR-244）：
...
// ─── 2) 扫描域 ≡ git 跟踪文件 ∩ isScannable（构造性相等）──
const targets = listScanTargets();
assert.equal(targets.source, "git", "本仓 git 可用，扫描域必须走 git 权威路径（非 walk 退化）");

const trackedMd = gitPaths("ls-files", "-z").filter(isScannable);
assert.deepEqual(
  [...targets.rels].sort(),
  [...trackedMd].sort(),
  "扫描域必须恰好等于「git 跟踪文件 ∩ isScannable」",
);
assert.ok(
