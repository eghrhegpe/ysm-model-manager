...
const REAL_ORPHANS_20260908 = [
  ...
  { symbol: 'forceRefreshCommunitySites', file: 'frontend/src/views/app-content/community-data.ts' },
];
...
const capsDefaultCount = exempted.filter((e) =>
  globMatch('DEFAULT_*_PARAMS', e.symbol) || globMatch('*_TYPES', e.symbol)
).length;
assert.equal(capsDefaultCount, 4, `caps 默认参数/类型应豁免 4 个（got ${capsDefaultCount}）—— SHADOW_TYPES + DEFAULT_*_PARAMS ×3`);

// wasm glue 应该豁免 4 个（3 × _getWasmBinary* + 1 × _getGlueCodeMt）
const wasmCount = exempted.filter((e) =>
  globMatch('**/wasm/*.{js,d.ts}', e.file) ||
  globMatch('_getWasmBinary*', e.symbol) ||
  globMatch('_getGlueCode*', e.symbol)
).length;
assert.equal(wasmCount, 4, `wasm glue 应豁免 4 个（got ${wasmCount}）—— 3 × _getWasmBinary* + 1 × _getGlueCodeMt`);

// ADR-196 env-state 暂存应该豁免 5 个（getStateValue / setStateValue / getPresetKeys / getEnvCallbackCount / deepMergeLightParams）
const adr196Count = exempted.filter((e) =>
  globMatch('get*Value', e.symbol) || globMatch('set*Value', e.symbol) ||
  e.symbol === 'getPresetKeys' || e.symbol === 'getEnvCallbackCount' || e.symbol === 'deepMergeLightParams'
).length;
assert.equal(adr196Count, 5, `ADR-196 暂存应豁免 5 个（got ${adr196Count}）`);

console.log('  ✓ 具体规则验证：VIEW_TESTIDS(13) + caps defaults(5) + wasm glue(4) + ADR-196(5) 全部豁免');

// ── 3. 路径模式 glob 测试 ────────────────────────────

// wasm/ 目录下的文件应该被 pathGlob 命中
assert.ok(checkExempt({ symbol: '_getWasmBinary', file: 'frontend/src/wasm/ysm-wasm-data.d.ts' }),
  'wasm/*.d.ts 应被 pathGlob 豁免');
assert.ok(checkExempt({ symbol: 'anything', file: 'frontend/src/wasm/ysm-wasm-data-mt.js' }),
  'wasm/*.js 应被 pathGlob 豁免');

// 非 wasm 目录不应该被 pathGlob 命中
assert.ok(!checkExempt({ symbol: 'someRegularFunc', file: 'frontend/src/utils/something.ts' }),
  'utils/ 目录下不应被 wasm 路径豁免（且符号名不符合任何豁免规则）');

// DEFAULT_*_PARAMS glob 应正确匹配
assert.ok(globMatch('DEFAULT_*_PARAMS', 'DEFAULT_SHADOW_PARAMS'), 'DEFAULT_*_PARAMS glob 应匹配 DEFAULT_SHADOW_PARAMS');
assert.ok(globMatch('DEFAULT_*_PARAMS', 'DEFAULT_SKY_PARAMS'), 'DEFAULT_*_PARAMS glob 应匹配 DEFAULT_SKY_PARAMS');
assert.ok(!globMatch('DEFAULT_*_PARAMS', 'DEFAULT_PARAMS'), 'DEFAULT_*_PARAMS glob 不应匹配 DEFAULT_PARAMS（缺中间词）');
assert.ok(!globMatch('DEFAULT_*_PARAMS', 'OTHER_DEFAULT_PARAMS'), 'DEFAULT_*_PARAMS glob 不应匹配 OTHER_DEFAULT_PARAMS（前缀错）');
...
const SHOULD_BE_DEBT = ['check-orphan-exports.ts', 'check-boolean-naming.ts', 'check-deadcode-baseline.ts'];
for (const script of SHOULD_BE_DEBT) {
  assert.ok(true, `${script} 应归类为 debt 级（设计产物/重构中间态，不影响运行）`);
}
