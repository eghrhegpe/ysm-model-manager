...
import { ROOT, readText, relPosix, resolveImport, walk } from "../scripts/_lib/scan-files.ts";
...
try {
  ...
  assert(
    !all.includes("css/style.css"),
    `walk 应排除 css/（实际含 ${all.filter((f) => f.includes("css")).join(",")}）`,
  );
  ...
  const noTest = walk(tmp, { skipFile: /\.(test|spec)\./ }).map((p) =>
    path.relative(tmp, p).replace(/\\/g, "/"),
  );
...
// ── 2. resolveImport：扩展名补全边界（不依赖磁盘，仅 moduleSet）─────
{
  ...
  assert(
    resolveImport(from, "./dir", mods) === m("dir/index.ts"),
    "./dir 应补 index.ts（优先 ts）",
  );
  assert(resolveImport(from, "./nope", mods) === null, "不存在时应返回 null");
  assert(resolveImport(from, "three", mods) === null, "包导入（非相对）应返回 null");
  assert(
    resolveImport(from, "three/addons/controls/OrbitControls.js", mods) === null,
    "带路径的包导入应返回 null",
  );
  ...
  assert(
    resolveImport(path.join(upBase, "src", "mod.ts"), "../top", upMods) ===
      path.join(upBase, "top.ts"),
    "../ 上级路径应正确解析",
  );
}
...
assert(
  relPosix(path.join(ROOT, "scripts", "x.ts")) === "scripts/x.ts",
  `relPosix 应相对 ROOT 且正斜杠（got: ${relPosix(path.join(ROOT, "scripts", "x.ts"))}）`,
);
...
// ── 5. ripgrep 共享层（rg）：临时目录 fixture ─────────
{
  ...
  try {
    ...
    assert(
      hits.some((l) => l.includes("a.js")),
      `rg 应命中 a.js（got: ${hits.slice(0, 2).join(" | ")}）`,
    );
    assert(!hits.some((l) => l.includes("b.ts")), "rg 不应误命中 b.ts");

    const tsOnly = rg("const bar", ".rg-test-tmp", ["*.ts"]);
    assert(
      tsOnly.length === 1 && tsOnly[0].includes("b.ts"),
      `glob *.ts 应只命中 b.ts（got: ${tsOnly.join(" | ")}）`,
    );
...
// ── 6. parseRgLine：rg 输出行解析边界 ────────────────
{
  ...
  assert(
    f3 === "frontend/src/bus.ts" && l3 === 165 && c3 === "setBus",
    `相对路径解析失败（got: ${f3}:${l3} ${c3}）`,
  );

  // 无行号/非标准行 → 降级返回 [原行, 0, '']
  const [f4, l4, c4] = parseRgLine("some random output line");
  assert(
    f4 === "some random output line" && l4 === 0 && c4 === "",
    `非标准行应降级（got: ${f4}:${l4} ${c4}）`,
  );
  ...
  // 内容有首尾空白：应 trim
  const [_f6, , c6] = parseRgLine("a.js:9:  indented  ");
